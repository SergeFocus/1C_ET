...
Процедура ВыполнитьСменуОсновнойВалютыНаСервере()
	...
	Если СменитьВДокументах Тогда
		...
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОтчетКомиссионераДеньгиЗаТовар.Ссылка,
		|	ОтчетКомиссионераДеньгиЗаТовар.Ссылка.Дата КАК Дата
		|ИЗ Документ.ОтчетКомиссионера.ДеньгиЗаТовар КАК ОтчетКомиссионераДеньгиЗаТовар
		|ГДЕ (ОтчетКомиссионераДеньгиЗаТовар.Валюта = &ОсновнаяВалюта
		|			ИЛИ ОтчетКомиссионераДеньгиЗаТовар.Валюта = &ВалютаСмены)
		|	И ОтчетКомиссионераДеньгиЗаТовар.Ссылка.Проведен = ИСТИНА
		|ОБЪЕДИНИТЬ ВСЕ
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОтчетКомиссионераКомиссионноеВознаграждение.Ссылка,
		|	ОтчетКомиссионераКомиссионноеВознаграждение.Ссылка.Дата
		|ИЗ Документ.ОтчетКомиссионера.КомиссионноеВознаграждение КАК ОтчетКомиссионераКомиссионноеВознаграждение
		|ГДЕ (ОтчетКомиссионераКомиссионноеВознаграждение.Валюта = &ОсновнаяВалюта
		|			ИЛИ ОтчетКомиссионераКомиссионноеВознаграждение.Валюта = &ВалютаСмены)
		|	И ОтчетКомиссионераКомиссионноеВознаграждение.Ссылка.Проведен = ИСТИНА
		|УПОРЯДОЧИТЬ ПО Дата";
