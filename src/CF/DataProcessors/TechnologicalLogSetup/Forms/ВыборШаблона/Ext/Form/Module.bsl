...
Процедура ПриОткрытии(Отказ)
	...
	Перем Документ, Файлы, Представление, Описание, НовыйШаблон;
	...
	#Если ТонкийКлиент Тогда
		...
		Если НЕ ПустаяСтрока(ПутьКШаблонам) Тогда
			Документ = Новый ТекстовыйДокумент;
			Файлы = НайтиФайлы(ПутьКШаблонам, "config*.lct", ИСТИНА);
...
Процедура УдалитьШаблонСДиска(Команда)
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ОписаниеШаблона = Неопределено ИЛИ НЕ ОписаниеШаблона.Внешний Тогда
			Возврат;
		КонецЕсли;
		Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы действительно хотите удалить шаблон") +" " + ОписаниеШаблона.Представление + "?'; sys = ''";
		...
		Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
			Выполнить(" ОбратныйВызов = Новый ОписаниеОповещения(""УдалитьШаблонСДискаЗавершение"", Объект, ПараметрыВызова); 
