...
Функция ЗагрузитьФайлНастроекНаСервере(АдресХранилища)
	...
	Если Журнал.Количество() = 0 Тогда
		...
		Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("В загружаемом файле отсутствует элемент") + " <config>. " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Данный файл не является файлом настройки технологического журнала."));
		Возврат ЛОЖЬ;
	КонецЕсли;
