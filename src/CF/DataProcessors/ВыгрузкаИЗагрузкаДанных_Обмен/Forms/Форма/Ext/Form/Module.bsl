...
Функция   ВыгрузитьДанныеВФайл(ИмяФайлаTxt)	
	...
	Исключение 	
		...
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при записи файла") + ": " + ТекстОписаниеОшибки;
		Сообщение.Сообщить();
...
Процедура ВыгрузитьДанныеРуководителюЗавершениеФрагмент(Знач АвтоПериодДляВыгрузкиРуководителю, Знач РуководительПодтвердилПрошлуюВыгрузку)
	...
	Перем АдресФайла, ДиалогВыбФайла, ДиалогРасширение, ДиалогФильтр, ИмяФайлаTxt, ТекстОжидания;
...
Процедура ВыгрузитьРеализациюЗаПериодНаСервере(АдресФайла)
	...
	Попытка 
		...
		ПодсистемаИЭ.ЗаписатьВЖурналИмпортноЭкспортныхОпераций(АдресФайла, , Выб.Количество(), ИСТИНА, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выгрузка реализаций, списаний за период") + " " + Формат(Объект.ДатаНачала, "ДФ=dd.MM.yyyy") + " - " + Формат(Объект.ДатаОкончания, "ДФ=dd.MM.yyyy"));
	...
	Исключение 	
		ТекстОписаниеОшибки = ОписаниеОшибки();
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка при записи файла") + ": " + ТекстОписаниеОшибки;
		Сообщение.Поле 	= "ВыгрузитьРеализациюЗаПериод";
		Сообщение.УстановитьДанные(Объект);
		Сообщение.Сообщить();
...
Процедура ЗагрузитьДанныеОбОстаткахНаСервере(АдресФайла)
	...
	Исключение 	
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка при проведении документа") + ": " + ТекстОписаниеОшибки;
		Сообщение.Сообщить();		
	КонецПопытки;
...
Функция   ЗагрузитьДокумент(ВозвращатьЗначение = ЛОЖЬ, Знач ТипДокумента, сч, пропустить, ОпределенныйЭлемент =  Неопределено)
	...
	Если НЕ Врег(ЗначениеТип) = "NULL" тогда
			...
			Если НЕ отказ
				...
				исключение
					...
					попытка
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Провести документ не удалось") + ": " + ОбъектД + " " + ТекстОписаниеОшибки;
						Сообщение.Сообщить();
						
					Исключение
						ТекстОписаниеОшибки = ОписаниеОшибки();
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Записать документ не удалось") + ": " + ОбъектД + " " + ТекстОписаниеОшибки;
						Сообщение.Сообщить();
						пропустить = ИСТИНА;
					конецпопытки;
				конецпопытки;
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "" + сч + " " + ТекстОписаниеОшибки;
			Сообщение.Сообщить();
...
Процедура ЗагрузитьРеализациюЗаПериодНаСервере(Знач АдресФайла)
	...
	попытка                                        		
			...
			Если НЕ ЭтоКонецФайла тогда
				...
				Если НЕ ЗначениеЗаполнено(Номенклатура) 
					...
					Исключение 
						ТекстОписаниеОшибки = ОписаниеОшибки();
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи Номенклатуры") + ": " + НоменклатураОбъект + " " + ТекстОписаниеОшибки;
						Сообщение.Сообщить();
					КонецПопытки; 
				...
				Иначе
					...
					Если ЗначениеЗаполнено(КлиентНаименование)
						...
						Исключение // Записи КлиентОбъект
							ТекстОписаниеОшибки = ОписаниеОшибки();
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи Клиента") + ": " + КлиентОбъект + " - " + ТекстОписаниеОшибки;
							Сообщение.Сообщить();
		...
		ПодсистемаИЭ.ЗаписатьВЖурналИмпортноЭкспортныхОпераций(АдресФайла, , ТаблицаРасхода.Количество(), ЛОЖЬ, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Загрузка реализаций, списаний из файла."));
	...
	Исключение 	
		ТекстОписаниеОшибки = ОписаниеОшибки();
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка при чтении файла") + ": " + ТекстОписаниеОшибки;
		Сообщение.Сообщить();
...
Функция   ЗагрузитьРеквизит(ДействительноЗначениеДляРеквизита, ТипРеквизита, сч, пропустить, ЗначениеТип, ОпределенныйЭлемент = Неопределено);
	...
	Если Назначать Тогда
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "№: " + сч + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("для") + ": " + ТекДок.ПолучитьСтроку(сч) + ". " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Значения Тип") + ": " + ЗначениеТип + ". " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Значение Реквизита") + ": " + ЗначениеДляРеквизита + ". " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Тип Реквизита") + ": " + ТипРеквизита + ". " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Пропустить") + ": " + пропустить + " :" + ТекстОписаниеОшибки;
			Сообщение.Сообщить();
			сч = сч + 1;
	...
	Если НЕ запрещенопроводить
		...
		Исключение 	
			...
			Исключение 	
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка при записи") + ": " + ТекстОписаниеОшибки;
				Сообщение.УстановитьДанные(Объект);
				Сообщение.Сообщить();
...
Процедура ВыгрузитьЕжедневныеДанныеОРозничнойРеализацииЗаПериодВБухгалтерияПредприятия2(Команда)
	...
	ФормаВыбораДокументов = ПолучитьФорму("Обработка.ВыгрузкаИЗагрузкаДанных_Обмен.Форма.ВыборДокументовИзСписка", ПараметрыДляФормы);
	
	ФормаВыбораДокументов.ОткрытьМодально();
