...
Процедура ВыгрузитьРеализациюЗаПериодНаСервере(АдресФайла)
	...
	Если ВестиУчетРегионовДляКонтрагентовИПроизводителей
	...
	Пока Выб.Следующий() Цикл
		...
		ОтправитьВФайл(ФайлВыгрузки, Выб.НоменклатураОсновнойШтрихКод);
		...
		Пока не ЭтоКонецФайла Цикл // строки читаются до символа перевода строки	
			...
			если не ЭтоКонецФайла тогда
				...
				Если НЕ ЗначениеЗаполнено(Номенклатура) 
					...
					и Объект.СоздаватьНовуюНоменклатуру Тогда
				...
				Если не СтрокаТаблицы.Цена
					...
					если не НоваяНома
						...
						ТекущаяЦена = ОбщийМодульСервер.ПолучитьЦенуНаСервере(Номенклатура, объект.ВидЦенДляУстановкиПриЗагрузке, СтрокаТаблицы.Период, ложь, СтрокаТаблицы.Количество, , , , Объект.Ссылка);	
					КонецЕсли;
					...
					Если ТекущаяЦена = 0 Тогда
						...
						ОбщийМодульСервер.УстановитьЦенуИВсеЗависимые(объект.ВидЦенДляУстановкиПриЗагрузке, СтрокаТовара, , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Загружен автоматически") + " " + ТекущаяДата(), СтрокаТаблицы.Период, ложь, );
					КонецЕсли;
...
Функция   ОбнаружитьНоменклатуруПоРеквизитам(СтруктураРеквизитов)
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			...
			Результат = Справочники.Номенклатура.ТоварНаСумму.Ссылка ;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	ВестиУчетРегионовДляКонтрагентовИПроизводителей = ОбщийМодульПовтор.получитьЗначениеНастройкиИлиКонстанты("ВестиУчетРегионовДляКонтрагентовИПроизводителей");
	элементы.РегионНовыхКлиентов.Видимость 		= ВестиУчетПоКлиентам и ВестиУчетРегионовДляКонтрагентовИПроизводителей;
	...
	Если ВестиУчетРегионовДляКонтрагентовИПроизводителей Тогда
...
Функция   ПроверитьЭлемент(Ссылка, ТипСправочника)
	...
	если ЕстьОтборы тогда
		...
		Если НЕ Пропустить
			И ВестиУчетРегионовДляКонтрагентовИПроизводителей
...
Функция   ЗагрузитьРеквизит(ЗначениеРеквизита, ТипРеквизита, сч, пропустить, ЗначениеТип);
	...
	попытка
		...
		ИначеЕсли ТипРеквизита = "NULL" Тогда 
			...
			ИначеЕсли ТипТут = "ПУСТО" Тогда
				ЗначениеРеквизита = неопределено;				
			иначе				
				ЗначениеРеквизита = ЗагрузитьСправочник("СправочникСсылка." + типТут, сч, пропустить, Истина);
			КонецЕсли;
			
		ИначеЕсли НЕ Найти(ТипРеквизита, "СправочникСсылка") = 0 Тогда
			ЗначениеРеквизита = ЗагрузитьСправочник(ТипРеквизита, сч, пропустить);
			
		ИначеЕсли НЕ Найти(ТипРеквизита, "ДокументСсылка") = 0 Тогда
			ЗначениеРеквизита = ЗагрузитьДокумент(Истина, ТипРеквизита, сч, пропустить);
...
Функция   ЗагрузитьСправочник(Знач ТипСправочника, сч, Знач пропустить, ТипПрочитан = Ложь);
	...
	ИначеЕсли НЕ ЗначениеТип = "ПРОП"
		...
		Если не СпецКод = "" Тогда
			...
			если Запрос.Текст = "НШК" Тогда
				ОбъектСсылка = ОбщийМодульТоварСервер.ПолучитьНоменклатуруПоШтрихКоду(СпецКод, Ложь);				
...
Функция   ЗагрузитьДокумент(ВозвращатьЗначение = ложь, Знач ТипДокумента, сч, пропустить)
