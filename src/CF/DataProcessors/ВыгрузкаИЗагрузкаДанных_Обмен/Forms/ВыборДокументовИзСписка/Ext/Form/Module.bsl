...
// 	Если НЕ РекзльтатТаблица = неопределено Тогда
		...
		ДиалогВыбФайла.Заголовок				=	ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выберите файл для экспорта") + ":";
		ДиалогВыбФайла.ПолноеИмяФайла			=	АдресВыгрузки; // АДРЕС
		...
		Если ДиалогВыбФайла.Выбрать() Тогда
			АдресВыгрузки = ДиалогВыбФайла.ПолноеИмяФайла;
		...
		КонецЕсли; // когда файл АдресВыгрузки выбран
...
Процедура ВыгрузитьЕжедневныеДанныеОРозничнойРеализацииЗаПериодВБухгалтерияПредприятия2НаСервере()
	...
	Если НЕ ВсеДокументы.Количество() = 0 Тогда
		...
		|	ВыполненияРаботТовары.Номенклатура.Комментарий КАК Комментарий,
		...
		|	ОказанияУслугЗаВремя.Номенклатура.Комментарий,
		...
		|	РасходыТовараТовары.Номенклатура.Комментарий,
