...
Процедура ПроверитьСерии(Команда)
	...
	Если НЕ ПроверитьСерииНаСервере() Тогда
		...
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Серии указаны полностью.");
		Сообщение.Сообщить();			
	КонецЕсли;
...
Функция   ПроверитьСерииНаСервере(Отказ = ЛОЖЬ)
	...
	ИначеЕсли не ОбщийМодульПовтор.ТоварВедетсяПоСериям(Объект.Номенклатура) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Эта Номенклатура не учитывается по сериям!");
		Сообщение.Поле 	= "Номенклатура";
		Сообщение.УстановитьДанные(Объект);
		Сообщение.Сообщить();
	...
	Иначе
		...
		Для Каждого строкаСерии Из Объект.ОписаниеСерийНоменклатуры Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаСерии.Серия) Тогда			
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("В строке №") + строкасерии.номерстроки + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("не указана серия!");
				Сообщение.Поле 	= "ОписаниеСерийНоменклатурыСерия";
				Сообщение.УстановитьДанные(Объект);
				Сообщение.Сообщить();
			...
			ИначеЕсли не МассивИменСерий.Найти(СтрокаСерии.Серия) = Неопределено Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("В строке №") + строкасерии.номерстроки + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("повторяется серия") + ": " + СтрокаСерии.Серия;
				Сообщение.Поле 	= "ОписаниеСерийНоменклатурыСерия";
				Сообщение.УстановитьДанные(Объект);
				Сообщение.Сообщить();
			...
			Если ИспользоватьШтрихКоды
				...
				Если НЕ МассивШтрихКода.Найти(ШтрихКод) = Неопределено Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("В строке №") + строкасерии.номерстроки + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("повторяется Штрих-код") + ": " + ШтрихКод;
					Сообщение.Поле 	= "ОписаниеСерийНоменклатурыШтрихКод";
					Сообщение.УстановитьДанные(Объект);
					Сообщение.Сообщить();
				...
				Иначе					
					...
					Если ЗначениеЗаполнено(Номенклатура) Тогда
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("В строке №") + строкасерии.номерстроки + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Штрих-код чужой") + ": " + Номенклатура + " " + Серия;
						Сообщение.Поле 	= "ОписаниеСерийНоменклатурыШтрихКод";
						Сообщение.УстановитьДанные(Объект);
						Сообщение.Сообщить();
...
Процедура СформироватьСерииСНумерацией(Команда)
	...
	Если НЕ СтруктураВозврат = Неопределено Тогда
		...
		пока сч < КонечныйНомерСерии Цикл
			...
			СтрокаСерий.Серия = ПрефиксАвтоНумерации + ЛидирующиеНули + формат(сч, "ЧДЦ=; ЧРГ=' '; ЧГ=0") + ПостфиксАвтоНомурации;
			сч = сч + 1;			
		КонецЦикла;
