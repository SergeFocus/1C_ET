...
Процедура ОбновитьПримерыСерий()
	...
	Если ДобавлятьЛидирующиеНули Тогда
		...
		пока сч < (длинаконечногономера - длинаначальногономера) Цикл
			сч = сч + 1;
			ЛидирующиеНули = ЛидирующиеНули + "0";	
		КонецЦикла;
	КонецЕсли;
	
	ПримерПервогоНомераСерии = ПрефиксАвтоНумерации + ЛидирующиеНули + формат(НачальныйНомерСерии, "ЧДЦ=; ЧРГ=' '; ЧГ=0") + ПостфиксАвтоНомурации;
	ПримерПоследнегоНомераСерии  = ПрефиксАвтоНумерации + формат(КонечныйНомерСерии, "ЧДЦ=; ЧРГ=' '; ЧГ=0") + ПостфиксАвтоНомурации;
	
КонецПроцедуры // ОбновитьПримерыСерий

&НаКлиенте
Процедура ПостфиксАвтоНомурацииПриИзменении(Элемент)
	ОбновитьПримерыСерий();
КонецПроцедуры
...
Процедура СформироватьСерииСогласноУсловиям(Команда)
	...
	Если ВсегоНомеров > 9998 Тогда
		сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Номеров серий слишком много!"));
	Иначе
		...
		СтруктураВозврат.Вставить("ПостфиксАвтоНомурации", ПостфиксАвтоНомурации);
