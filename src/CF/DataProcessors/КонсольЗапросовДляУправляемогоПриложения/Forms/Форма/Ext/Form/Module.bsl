...
Процедура ПередЗакрытиемЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Ответ = КодВозвратаДиалога.Да Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтаФорма, Объект.ИмяФайла);
...
Процедура ТипВФормеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ТипВФормеЗавершениеВыбора", ЭтаФорма);
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	Если ПустаяСтрока(Элементы.ТекстЗапроса) Тогда 
		ТекстПредупреждения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Введите текст запроса.");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура СохранитьФайлЗапроса(ОписаниеОповещенияОЗакрытии)
	...
	Оповещение = Новый ОписаниеОповещения("СохранитьФайлЗапросаЗавершение", ЭтаФорма, ОписаниеОповещенияОЗакрытии);
...
Процедура ЗаполнитьПараметрыКлиент()
	...
	Если Не ПустаяСтрока(ТекущийЗапрос.Текст) Тогда
		...
		Если МассивПараметров.Количество() > 0 Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьПараметрыКлиентЗавершение", ЭтаФорма, ТекущийЗапрос);
	...
	Иначе
		ПоказатьПредупреждение(, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Текст запроса пустой."));
	КонецЕсли;
КонецПроцедуры
