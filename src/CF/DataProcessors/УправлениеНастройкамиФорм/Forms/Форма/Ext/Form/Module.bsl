...
Процедура СкопироватьВыполнитьОтметитьЗавершение(Результат, Пользователи) Экспорт
	...
	Если Результат Тогда
		...
		Если НЕ глВерсияПлатформы < 803040000 Тогда
			ТекстОповещения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("После копирования настроек пользователю форма у пользователя будет открываться с настройками, которые ему скопируются. При этом его собственные настройки будут потеряны.");
