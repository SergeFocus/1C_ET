...
Процедура ЗаполнитьУказанныйВидЦенИзДаннымиИзКарточкиНоменклатуры(Команда)
	...
	Иначе
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вид цен не указан!"), СтатусСообщения.Информация);
	КонецЕсли;
...
Процедура ЗаполнитьУказанныйВидЦенИзДаннымиИзКарточкиНоменклатурыНаСервере()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Исключение 
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи цены") + ": " + ЦеныНоменклатуры + " - " + ТекстОписаниеОшибки);
		КонецПопытки;		
	КонецЦикла;
...
Процедура ЗаполнитьЦеныВКарточкахНоменклатурыПоДаннымУказанногоВидаЦенНаСервере()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи цены") + ": " + НоменклатураОбъект + " - " + ТекстОписаниеОшибки);
		КонецПопытки;		
	КонецЦикла;
