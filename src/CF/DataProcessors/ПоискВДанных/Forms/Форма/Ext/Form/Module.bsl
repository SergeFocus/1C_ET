	...
	Если (ЭлементHTML.id = "FullTextSearchListItem") Тогда
		...
		Попытка
			...
			Если глВерсияПлатформы < 803040000  Тогда
				Выполнить(" ОткрытьЗначение(ВыбраннаяСтрока); ");
			Иначе
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() <> РежимПолнотекстовогоПоиска.Разрешить Тогда
		Отказ = ИСТИНА;
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Использование полнотекстового поиска не разрешено. Вы можете включить его в диалоге Управление полнотекстовым поиском"));
