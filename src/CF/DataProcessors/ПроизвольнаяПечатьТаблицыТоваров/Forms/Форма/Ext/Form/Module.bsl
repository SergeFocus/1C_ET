...
Процедура НаПечать(ТабДок)
	...
	ТабДок.ИмяПараметровПечати  = "ТаблицаТоваров" + СокрЛП(ИмяКомпьютера());
	...
	Иначе
		...
		Для Каждого ДокументДляПечати Из Объект.ИсточникиТаблицаДокументов Цикл
			...
			НаПечататьТаблицу(ТабДок, СчетчикДокументов);
...
&НаСервере
Процедура НаПечататьТаблицу(ТабДок, Знач СчетчикДокументов = 0);
		...
		Если НЕ ПланыПродаж Тогда
			...
			Если Объект.ВыводитьЦенуИСумму 
				...
				Если ЗначениеЗаполнено(Объект.ВидЦен)
					...
					Если ИспользоватьЕдиницыИзмеренияНоменклатуры Тогда
						Попытка 
							НоваяЦена = ОбщийМодульСервер.ПолучитьЦенуНаСервере(Номенклатура, Объект.ВидЦен, Объект.Дата + 1, ИСТИНА, ?(НЕ ОбщееКоличество = 0, СтрокаТовара.Количество, 0), , , , , ИСТИНА, СтрокаТовара.ЕдиницаИзмерения); // не учтена едизм	
						Исключение 	
							Попытка 
								НоваяЦена = ОбщийМодульСервер.ПолучитьЦенуНаСервере(Номенклатура, Объект.ВидЦен, Объект.Дата + 1, ИСТИНА, ?(НЕ ОбщееКоличество = 0, СтрокаТовара.Количество, 0), , , , , ИСТИНА); // не учтена едизм
							Исключение 	
							КонецПопытки;
	...
	Если ЗначениеЗаполнено(Объект.ПроизвольнаяПечатнаяФорма) Тогда
		...
		ОбщийМодульТекстСервер.СформироватьДокументПоПроизвольнойФорме(Объект.ПроизвольнаяПечатнаяФорма, , ТабДок, Объект, "Обработки", "ПроизвольнаяПечатьТаблицыТоваров", Новый Структура("Товары", Товары));
