		...
		Пока РезН.Следующий() Цикл
			...
			Если ЧтотоМенялось Тогда
				...
				Исключение // Записи ОбъектКонтрагент
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи справочника") + ": " + ОбъектКонтрагент + " - " + ТекстОписаниеОшибки;
					Сообщение.Сообщить();
		...
		Пока РезН.Следующий() Цикл
			...
			Если ЧтотоМенялось Тогда
				...
				Исключение // Записи ОбъектНоменклатура
					ТекстОписаниеОшибки = ОписаниеОшибки();
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи справочника:") + " " + ОбъектНоменклатура + " - " + ТекстОписаниеОшибки;
					Сообщение.Сообщить();
				...
				Если НЕ ВзДокумент.Пустой() Тогда
					...
					Пока РезДокумент.Следующий() Цикл
						...
						Исключение 
							ТекстОписаниеОшибки = ОписаниеОшибки();
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке отмены проведения документа:") + " " + РезСсылка + " " + ТекстОписаниеОшибки;
							Сообщение.Сообщить();
						КонецПопытки;  // записи  РезСсылка
			...
			Пока РезСправочник.Следующий() Цикл
				...
				Попытка 
					...
					Исключение 
						ТекстОписаниеОшибки = ОписаниеОшибки();
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи:") + " " + РезСсылка + " - " + ТекстОписаниеОшибки;
						Сообщение.Сообщить();
					КонецПопытки;  // записи  РезСсылка
				исключение
					прервать;
