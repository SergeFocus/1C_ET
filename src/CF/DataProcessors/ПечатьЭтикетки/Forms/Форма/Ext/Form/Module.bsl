...
Процедура ДанныеНоменклатурыПриИзмененииНаСервере()
	...
	Если ОбщийМодульТоварСервер.НаименованиеЭтойНоменклатурнойГруппыНазначается(Объект.НоменклатурнаяГруппа) Тогда
...
Процедура ЗакрытьИПередать(Знач ПечататьЦенникШтрихКод)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Номенклатура) Тогда
		...
		//И ЗначениеЗаполнено(Объект.НоменклатурнаяГруппа) Тогда // не все ах

		СтруктураНоменклатура = ОбщийМодульСерверНайтиНоменклатуруПоШтрихКоду(Объект.ШтрихКод);
		...
		Если НЕ ЗначениеЗаполнено(Объект.Номенклатура) Тогда
			...
			Если ИспользоватьСложныйМеханизмЦен
				...
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вид цен не указан!"), Объект, "ВидЦен");
				Отмена = ИСТИНА;
			КонецЕсли;
			...
			Если ВестиУчетСерийНоменклатуры Тогда
				Объект.СерияСуществующейНоменклатуры = ОбщийМодульТоварСервер.НайтиСериюНоменклатуры(Объект.Номенклатура, Объект.СерияНоменклатуры);
			КонецЕсли;
...
Процедура ЗакрытьИПередатьЗавершениеФрагмент(Знач ПечататьЦенникШтрихКод);
	...
	Если ПечататьЦенникШтрихКод Тогда
		// !!сюда!
		ДокументПечати = ОбщийМодульСерверНапечататьЭтикетку();
		Если НЕ ДокументПечати = Неопределено Тогда
			Если НЕ ЗначениеЗаполнено(ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("НаименованиеПринтераШтрихКодов")) Тогда
				НаименованиеДокумента = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Этикетка");
				ПараметрыФормыПечати = Новый Структура("ДокументПечати", ДокументПечати);
				ПараметрыФормыПечати.Вставить("НаименованиеДокумента", НаименованиеДокумента);
				ПараметрыФормыПечати.Вставить("НеПечататьНичегоДополнительно", ИСТИНА);
				ПараметрыФормыПечати.Вставить("Заголовок", НаименованиеДокумента);
	...
	Если Объект.ЭтоДобавлениеИзДокумента Тогда
		...
		СтруктураДляПечати.Вставить("ШтрихКод", Объект.ШтрихКод);
		СтруктураДляПечати.Вставить("Количество", Объект.Количество);
		СтруктураДляПечати.Вставить("Номенклатура", Объект.Номенклатура);
		СтруктураДляПечати.Вставить("СерияНоменклатуры", Объект.СерияСуществующейНоменклатуры);
		СтруктураДляПечати.Вставить("Цена", Объект.Цена);
		СтруктураДляПечати.Вставить("Склад", Объект.Склад);
		СтруктураДляПечати.Вставить("ЕдиницаИзмерения", Объект.ЕдиницаИзмерения);
		СтруктураДляПечати.Вставить("Дата", Объект.Дата);
		ЭтаФорма.Закрыть(СтруктураДляПечати);
	КонецЕсли;
...
&НаСервере
Функция   НапечататьЭтикеткиДляВсехСтрокТаблицыНаСервере(ОбщийДокументПечати, Знач ДляПозиций = ЛОЖЬ, Знач ДляСотрудников = ЛОЖЬ)
	...
	Если ДляСотрудников Тогда
		Для Каждого СтрокаТаблицы Из Объект.Сотрудники цикл
			...
			Если ЗначениеЗаполнено(Позиция) Тогда
				...
				ДокументПечати = ОбщийМодульСерверНапечататьЭтикеткуОбъектам(ШтрихКод, ТекстСШтрихКодом, СтрокаТаблицы.Количество, СокрЛП(Позиция));

				Если НЕ ДокументПечати = Неопределено
					...
					ОбщийДокументПечати.Вывести(ДокументПечати);
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		Для Каждого СтрокаТаблицы Из Объект.Контрагенты цикл
			...
			Если ЗначениеЗаполнено(Контрагент) Тогда
				...
				ТекстСШтрихКодом  = ШтрихКод;
				ДокументПечати = ОбщийМодульСерверНапечататьЭтикеткуОбъектам(ШтрихКод, ТекстСШтрихКодом, СтрокаТаблицы.Количество, СокрЛП(Контрагент));
...
&НаСервере
Функция   НапечататьЭтикеткиДляОрганизации(ОбщийДокументПечати)
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		ТекстСШтрихКодом  = ШтрихКод;
		ДокументПечати = ОбщийМодульСерверНапечататьЭтикеткуОбъектам(ШтрихКод, ТекстСШтрихКодом, 1, СокрЛП(Объект.Организация));
...
Процедура НоменклатурнаяГруппаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	ОбщийМодульКлиент.ПоискОшибкиПриВводеТекстаПользователем("НоменклатурныеГруппы", ДанныеВыбора, Текст, Объект.НоменклатурнаяГруппа);
...
Процедура ОбновитьТекстаЗаголовкаЭтикетки(Знач НеОбновлятьНоменклатуру = ЛОЖЬ)
	...
	Попытка
		...
		ИначеЕсли НЕ НеОбновлятьНоменклатуру
			...
			Номенклатура = ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(Объект.ШтрихКод, , ИСТИНА);
...
&НаСервере
Процедура ОбработатьПолученныйШтрихкодНаСервере(Знач ТекКод)

	Если НЕ ПустаяСтрока(ТекКод) Тогда
		РезультатОбработки = ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(ТекКод, ИСТИНА, ИСТИНА);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование"
		...
		Если ИмяСобытия = "ScanData" Тогда
			...
			ОбработатьПолученныйШтрихкодНаСервере(ШтрихКод);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция   ОбщийМодульСерверНапечататьЭтикеткуОбъектам(Знач ШтрихКод, Знач ТекстСШтрихКодом, Знач Количество = 1, ДокументПечати = Неопределено, Знач ТекстЗаголовка = "")
	...
	ОбщийМодульТоварСервер.НапечататьЭтикетку(СтруктураПечати, ДокументПечати);
...
&НаСервере
функция   ОбщийМодульСерверНайтиНоменклатуруПоШтрихКоду(ШтрихКод)

	возврат ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(ШтрихКод, ИСТИНА, ИСТИНА);
...
Процедура ПроизводительОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	ОбщийМодульКлиент.ПоискОшибкиПриВводеТекстаПользователем("Производители", ДанныеВыбора, Текст, Объект.Производитель);
...
Процедура СформироватьСерию(Команда)
	...
	Объект.СерияНоменклатуры = ОбщийМодульТоварСервер.СгенерироватьСерию(ФормулаГенерацииСерий, Объект.Номенклатура); //номенклатура может быть еще пустой
...
Процедура ШтрихКодПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Номенклатура) Тогда
		ОбработатьПолученныйШтрихкодНаСервере(Объект.ШтрихКод);
	КонецЕсли;
