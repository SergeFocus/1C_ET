...
Процедура ДобавитьПоПараметрОтбораНаСервере(Знач ПараметрОтбора)
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ПредставлениеТоваровКонтрагентами.Комментарий КАК СТРОКА(1000)), """") КАК Комментарий
...
Процедура ЗагрузитьДанныеКонтрагентаНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.КлиентПоставщик) Тогда
		...
		|	ВЫРАЗИТЬ(ПредставлениеТоваровКонтрагентами.Комментарий КАК СТРОКА(1000)) КАК Комментарий,
