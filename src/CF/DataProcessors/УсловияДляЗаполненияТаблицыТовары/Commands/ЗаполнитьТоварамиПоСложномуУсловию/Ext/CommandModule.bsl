...
Процедура ДозаполнитьНоменклатурой(ДокументСсылка, СтруктураОтбора)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Исключение
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи документа") + ": " + ДокументОбъект + " - " + ТекстОписаниеОшибки);
...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если НЕ ОбщийМодульСервер.ПроверитьДокументПроведен(ПараметрКоманды) Тогда
		...
		Если НЕ СтруктураОтбора = Неопределено Тогда
			...
			ОбщийМодульКлиент.ПересчитатьТаблицуТовары(ПараметрКоманды);
		КонецЕсли;
	Иначе
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Документ уже проведен. Изменений не будет."), СтатусСообщения.Информация);
	КонецЕсли;
