...
Функция ПолучитьВсеТипы(ЭлементДЗ)
	...
	Если ТипЗнч(ОбъектМД) <> Тип("ОбъектМетаданных") 
		...
		ВызватьИсключение("Внутренняя ошибка обработки выгрузки");
...
Процедура УстановкаСостоянияВыгружатьПриНеобходимости(ЭлементДЗ)
	...
	Для Каждого СсылочныйТип Из ВсеТипы Цикл
		...
		Если ТипИОбъект = Неопределено Тогда
			...
			ВызватьИсключение("Внутренняя ошибка. Неполное заполнение структуры ссылочных типов - " + СсылочныйТип);
		...
		Если СтрокаДЗ = Неопределено Тогда 
			...
			ВызватьИсключение("Внутренняя ошибка. Неполное заполнение дерева метаданных. Отсутствует объект, образующий тип - " + СсылочныйТип);
