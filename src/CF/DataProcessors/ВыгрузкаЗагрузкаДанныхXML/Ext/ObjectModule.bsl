	...
	Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		Исключение
			...
			Иначе
				...
				Попытка
					Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при загрузке данных.") + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Объект:") + " " + ЗаписанноеЗначение + ", " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Тип:") + " " + ТипЗнч(ЗаписанноеЗначение) + "
						|" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка") + ": " + ТекстОшибки, СтатусСообщения.Важное);
				Исключение
					Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при загрузке данных.") + "
						|" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка") + ": " + ТекстОшибки, СтатусСообщения.Важное);
				КонецПопытки;	
...
Процедура ВыполнитьВыгрузку(ИмяФайла, ТолькоПроверкаНедопустимыхСимволов = ЛОЖЬ) Экспорт
	...
	Иначе
		...
		Если НЕ ТолькоПроверкаНедопустимыхСимволов Тогда
			Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Начало выгрузки") + ": " + ТекущаяДатаСеанса());
		Иначе
			Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Начало проверки") + ": "  + ТекущаяДатаСеанса());
		КонецЕсли;
...
Функция ПолучитьВсеТипы(ЭлементДЗ)
	...
	Если ТипЗнч(ОбъектМД) <> Тип("ОбъектМетаданных") 
		...
		ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Внутреняя ошибка обработки выгрузки"));
...
Процедура УстановкаСостоянияВыгружатьПриНеобходимости(ЭлементДЗ)
	...
	Для Каждого СсылочныйТип Из ВсеТипы Цикл
		...
		Если ТипИОбъект = Неопределено Тогда
			...
			ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Внутреняя ошибка. Неполное заполнение структуры ссылочных типов") + " - " + СсылочныйТип);
		...
		Если СтрокаДЗ = Неопределено Тогда 
			...
			ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Внутреняя ошибка. Неполное заполнение дерева метаданных. Отсутствует объект, образующий тип") + " - " + СсылочныйТип);
