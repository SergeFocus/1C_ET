	...
	Попытка
		...
		Для Каждого СтрокаТаблицыВыгрузки Из СоставПолнойВыгрузки Цикл
			...
			Если СтрокаДереваМетаданных.ЭлементОписания.Менеджер = Неопределено Тогда
				ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выгрузка данных. Внутренняя ошибка"));
			КонецЕсли;
...
Процедура ВыполнитьВыгрузку(ИмяФайла, ТолькоПроверкаНедопустимыхСимволов = ЛОЖЬ) Экспорт
	...
	Если СоставПолнойВыгрузки.Количество() = 0
		...
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не задан состав выгрузки"));
		Возврат;
	...
	Иначе
		...
		Если НЕ ТолькоПроверкаНедопустимыхСимволов Тогда
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Начало выгрузки") + ": " + ТекущаяДатаСеанса());
		Иначе
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Начало проверки") + ": "  + ТекущаяДатаСеанса());
		КонецЕсли;
	...
	Если НЕ ТолькоПроверкаНедопустимыхСимволов Тогда
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выгружено объектов") + ": " + ВсегоОбработанныхЗаписей());
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Окончание выгрузки") + ": " + ТекущаяДатаСеанса());
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выгрузка данных успешно завершена"));
	Иначе
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Проверено объектов") + ": " + ВсегоОбработанныхЗаписей());
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Окончание проверки") + ": " + ТекущаяДатаСеанса());
	КонецЕсли;
...
Процедура ВыполнитьЗагрузку(ИмяФайла) Экспорт
	...
	Если Не ЧтениеXML.Прочитать()
		...
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Неверный формат файла выгрузки"));
		Возврат;
	...
	СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Начало загрузки") + ": " + ТекущаяДатаСеанса());
	...
	Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		Исключение
			...
			Иначе
				...
				Попытка
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при загрузке данных.") + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Объект:") + " " + ЗаписанноеЗначение + ", " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Тип:") + " " + ТипЗнч(ЗаписанноеЗначение) + "
						|" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка") + ": " + ТекстОшибки, СтатусСообщения.Важное);
				Исключение
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при загрузке данных.") + "
						|" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка") + ": " + ТекстОшибки, СтатусСообщения.Важное);
				КонецПопытки;	
	...
	СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Загружено объектов") + ": " + Загружено);
	СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Окончание загрузки") + ": " + ТекущаяДатаСеанса());
	СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Загрузка данных успешно завершена"));
...
Процедура ДобавитьВыгружаемыеОбъекты(СоставПолнойВыгрузки, СоставВспомогательнойВыгрузки, СтрокаДЗ)
	...
	Если (СтрокаДЗ.ЭлементОписания <> Неопределено) 
		...
		Если НЕ СтрокаДобавления = Неопределено Тогда
...
Процедура ЗаписьКонстанты(ЗаписьXML, МД_Константа, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов)
	...
	Исключение
		...
		Иначе
			...
			ИтоговаяСтрокаСообщения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при выгрузке константы:") + " " + Строка(МД_Константа.Имя) + "
			|" + СтрокаОписанияОшибки;
			
			ОбщегоНазначения.СообщитьПользователю(ИтоговаяСтрокаСообщения);
	...
	Пока ВыборкаИзРезультата.Следующий() Цикл
		...
		Исключение
			...
			Иначе
				...
				ИтоговаяСтрокаСообщения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при выгрузке набора записей регистра:") + " " + ДляЗапроса + ИмяОбъекта + "
				|" + СтрокаОписанияОшибки;
...
Процедура Инициализация() Экспорт
	...
	ДеревоМетаданных.Колонки.Добавить("Выгружать", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выгружать"));
	ДеревоМетаданных.Колонки.Добавить("ВыгружатьПриНеобходимости", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("При необходимости"));
	ДеревоМетаданных.Колонки.Добавить("Метаданные", , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Метаданные"));
	...
	ДеревоМетаданных.Колонки.Добавить("Развернут", , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Развернут"));
		...
		Если ЭтоЗапросПоОбъекту Тогда
			...
			Исключение
				...
				Иначе
					...
					ИтоговаяСтрокаСообщения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при выгрузке объекта:") + " " + Строка(Объект) + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Тип:") + " " + Строка(ТипЗнч(Объект)) + "
					|" + СтрокаОписанияОшибки;
...
Функция ПолучитьВсеТипы(ЭлементДЗ)
	...
	Если ТипЗнч(ОбъектМД) <> Тип("ОбъектМетаданных") 
		...
		ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Внутренняя ошибка обработки выгрузки"));		
	КонецЕсли;
...
Функция СформироватьТаблицуОшибок(ОбъектыВыгруженныеСОшибками)
	...
	Если ОбъектыВыгруженныеСОшибками.Количество() = 0 Тогда
		СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Проверка объектов на наличие недопустимых символов завершена. Ошибок не обнаружено."));
	Иначе
...
Процедура УстановкаСостоянияВыгружатьПриНеобходимости(ЭлементДЗ)
	...
	Для Каждого СсылочныйТип Из ВсеТипы Цикл
		...
		Если ТипИОбъект = Неопределено Тогда			
			ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Внутренняя ошибка. Неполное заполнение структуры ссылочных типов") + " - " + СсылочныйТип);			
		КонецЕсли;
		
		ОбъектМД = ТипИОбъект;
		СтрокаДЗ = ЭлементДЗПоОбъектуМД(ОбъектМД);
		Если СтрокаДЗ = Неопределено Тогда
			ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Внутренняя ошибка. Неполное заполнение дерева метаданных. Отсутствует объект, образующий тип") + " - " + СсылочныйТип);			
		КонецЕсли;
