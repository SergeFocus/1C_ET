...
Процедура ВидИзменяемыхОбъектовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//ОписаниеОповещения = Новый ОписаниеОповещения("ВидИзменяемыхОбъектовВыборСделан", Объект);
	//ОткрытьФорму(ПолноеИмяФормы("ВыборВидаОбъектов"), Новый Структура("ТекущийОбъект", ВидИзменяемыхОбъектов), , , , , ОписаниеОповещения);
	 форма = ПолучитьФорму("Обработка.ГрупповоеИзменениеОбъектов.Форма.ВыборВидаОбъектов", Новый Структура("ТекущийОбъект", ВидИзменяемыхОбъектов));
	 ВыбранныйОбъект = форма.открытьмодально();
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		...
		Иначе
			...
			//ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьИзменениеПроверкаОтбораВыполнена", Объект);
			//ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , НСтр("ru = 'Изменение элементов'"));
			
			РезультатВопроса = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение элементов"));
...
//					ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьИзменениеПроверкиВыполнены", Объект);
//					ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , НСтр("ru = 'Изменение элементов'"));
РезультатВопроса = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение элементов"));
ВыполнитьИзменениеПроверкиВыполнены(РезультатВопроса);
				Иначе
...
Процедура НастроитьПараметрыИзменения(Команда)
	...
	ОткрытьФорму(ПолноеИмяФормы("Настройка"), ПараметрыФормы, Объект);
...
Процедура ПерейтиВЖурналРегистрации(Команда)
	...
	//	ОткрытьФорму("Обработка.ЖурналРегистрации.Форма", ПараметрыФормы, Объект);
	//КонецЕсли;
КонецПроцедуры
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВопросПерейтиКРазблокированиюРеквизитовЗавершение", Объект);
...
Процедура РазрешитьРедактированиеРеквизитов()
	...
	Если ЕстьФормаРазблокированияРеквизитов Тогда
		...
		ОткрытьФорму(ПолноеИмяФормыРаботыСБлокируемымиРеквизитами, ПараметрыФормы, Объект);
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьРезультатИзмененияОтветПолучен", Объект, РезультатИзменения);
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение реквизитов элементов'"), , НСтр("ru = 'Изменение элементов завершено.'"));
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура НастройкаОтбораНажатие(Элемент)
	Если Не ПустаяСтрока(ВидИзменяемыхОбъектов) Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбранныхОбъектов", Объект);
...
Процедура ОбновитьСчетчикиИзменяемыхРеквизитов(Знач ТаблицаФормы = Неопределено)
	...
	//	ТабличнаяЧасть = Элементы[ТаблицаФормы.Имя];
	...
	//	КонецЦикла;
	//
	КоличествоИзменяемых = определитьКоличествоИзменяемых(ТаблицаФормы.Имя);
...
&НаСервере
Функция   определитьКоличествоИзменяемых(имят) //Экспорт
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Для Каждого ТабличнаяЧасть Из ТабличныеЧастиОбъекта Цикл
		...
		Попытка
			Для Каждого Реквизит Из Объект[ТабличнаяЧасть.Значение] Цикл
				Если Реквизит.Изменять Тогда
