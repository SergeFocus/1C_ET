...
Функция ВерсияБСПСоответствуетТребованиям()
	...
	Попытка
		...
		Возврат НомерВерсииВЧисло(ВерсияБСП) >= НомерВерсииВЧисло("2.2.1.20");
	Исключение
...
Процедура ВидИзменяемыхОбъектовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
		Выполнить(" ОписаниеОповещения = Новый ОписаниеОповещения(""ВидИзменяемыхОбъектовВыборСделан"", ЭтаФорма);
		| ОткрытьФорму(ПолноеИмяФормы(""ВыборВидаОбъектов""), Новый Структура(""ТекущийОбъект"", ВидИзменяемыхОбъектов), , , , , ОписаниеОповещения); ");
	КонецЕсли;
...
Функция ВидОбъектаПоТипу(Тип)
	...
	Иначе
		...
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Неверный тип значения параметра") + " (%1)", Строка(Тип));
	КонецЕсли;
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов()
	...
	ТекстВопроса = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Реквизит заблокирован, перейти к разблокированию реквизитов?");
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Да, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Да"));
	Кнопки.Добавить(КодВозвратаДиалога.Отмена, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Отмена"));
	ТекстОповещения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Реквизит заблокирован");

	Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
		Выполнить(" ОписаниеОповещения = Новый ОписаниеОповещения(""ВопросПерейтиКРазблокированиюРеквизитовЗавершение"", ЭтаФорма);
		| ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Да, ТекстОповещения); ");
	КонецЕсли;
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	ШаблонЗаголовка       = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение выделенных элементов") + " ""%1"" (%2)";
	Заголовок = ПодставитьПараметрыВСтроку(ШаблонЗаголовка, ТипПереданногоОбъекта, КоличествоОбъектов);
...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() Тогда
		ТекстВопроса = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменения не настроены. Выполнить перезапись элементов без изменений?");
		ТекстОповещения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение элементов");
		Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
			Выполнить(" ОписаниеОповещения = Новый ОписаниеОповещения(""ВыполнитьИзменениеПроверкиВыполнены"", ЭтаФорма);
			| ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , ТекстОповещения); ");
		КонецЕсли;
	Иначе
...
&НаСервере
Процедура ДобавитьДополнительныеРеквизитыИСведенияВНабор(УправлениеСвойствамиМодуль)

	Если Не КонтекстныйВызов Тогда
		ДобавитьСвойствоВТаблицуРеквизитов(УправлениеСвойствамиМодуль.СписокСвойствДляВидаОбъектов(ВидИзменяемыхОбъектов, "ДополнительныеРеквизиты"));
		ДобавитьСвойствоВТаблицуРеквизитов(УправлениеСвойствамиМодуль.СписокСвойствДляВидаОбъектов(ВидИзменяемыхОбъектов, "ДополнительныеСведения"), ЛОЖЬ);
	...
	Для Каждого ДанныеОбъекта Из ВыбранныеОбъекты(ИСТИНА).Строки Цикл
		...
		ДопРеквизиты = УправлениеСвойствамиМодуль.ПолучитьСписокСвойств(ОбъектДляИзменения, , ЛОЖЬ);
		...
		ДопСведения = УправлениеСвойствамиМодуль.ПолучитьСписокСвойств(ОбъектДляИзменения, ЛОЖЬ);
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение реквизитов элементов"), , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение элементов завершено."));
		Закрыть();
	...
	Если ОбработкаЗавершена Тогда
		ШаблонСообщения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменения выполнены во всех выбранных элементах") + " (%2).";
	Иначе
		Если Объект.ИзменятьВТранзакции Или ТекущееСостояниеИзменения.КоличествоИзмененных = 0 Тогда

			ШаблонСообщения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменения не выполнены.");
		Иначе
			ШаблонСообщения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменения выполнены частично.") + Символы.ПС +
			ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменено") + ": %1; " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не удалось изменить") + ": %3";
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 Тогда
			...
			СтрокаТаблицы.Объект  = ".." + ПодставитьПараметрыВСтроку(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("и другие элементы") + " (%1)", КоличествоПропущенных);
			СтрокаТаблицы.Причина = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Пропущены, так как не были изменены один или более элементов.");
		КонецЕсли;
	КонецЕсли;
	...
	Элементы.ФормаНазад.Видимость = Не КонтекстныйВызов;
...
&НаСервере
Процедура ЗагрузитьМетаданныеОбъекта()
...
Процедура ЗаполнитьРеквизитыОбъекта(БлокируемыеРеквизиты, НеРедактируемые, ФильтруемыеРеквизиты)
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		УправлениеСвойствамиМодуль = ОбщийМодуль("УправлениеСвойствами");
		Если УправлениеСвойствамиМодуль <> Неопределено Тогда
			ИспользуютсяДопРеквизиты = УправлениеСвойствамиМодуль.ИспользоватьДопРеквизиты(МенеджерОбъекта.ПустаяСсылка());
			ИспользуютсяДопСведения  = УправлениеСвойствамиМодуль.ИспользоватьДопСведения (МенеджерОбъекта.ПустаяСсылка());

			Если ИспользуютсяДопРеквизиты Или ИспользуютсяДопСведения Тогда
				ДобавитьДополнительныеРеквизитыИСведенияВНабор(УправлениеСвойствамиМодуль);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьТабличныеЧастиОбъектов(ЗаблокированныеРеквизиты, НередактируемыеРеквизиты, ФильтруемыеРеквизиты)
	...
	Для Каждого ТаблицаОбъекта Из ТаблицыОбъекта Цикл
		...
		Для Каждого ОписаниеРеквизита Из ОписаниеТабличнойЧасти.Реквизиты Цикл
			...
			Реквизит = ЭтаФорма[ТаблицаОбъекта.Ключ].Добавить();
			Реквизит.Имя = ОписаниеРеквизита.Имя;
			Реквизит.Представление = ОписаниеРеквизита.Синоним;
			// Реквизит.ДопустимыеТипы = XMLСтрока(Новый ХранилищеЗначения(ОписаниеРеквизита.Тип));
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		Если КоличествоОбъектовДляОбработки = 0 Тогда
			ТекстПредупреждения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не указаны элементы для изменения");
		...
		Иначе
			ТекстВопроса = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Отбор не задан. Изменить все элементы?");
			ТекстОповещения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение элементов");

			Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
				Выполнить(" ОписаниеОповещения = Новый ОписаниеОповещения(""ВыполнитьИзменениеПроверкаОтбораВыполнена"", ЭтаФорма);
				| ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , ТекстОповещения); ");
			КонецЕсли;
		КонецЕсли;
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	Иначе
		НаименованиеЗадания = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Групповое изменение элементов");
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			ПоказатьОповещениеПользователя(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение выделенных элементов"), ,ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Пожалуйста подождите, обработка может занять некоторое время.."));
	...
	Иначе
		...
		Состояние(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Обрабатываются элементы.."), 0, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение выделенных элементов"));
		ПоказыватьПроцентОбработанных = ИСТИНА;
	КонецЕсли;
...
Функция ИзменяемыеРеквизиты(ИмяТабличнойЧасти = "РеквизитыОбъекта")
	...
	ТаблицаРеквизитов = ЭтаФорма[ИмяТабличнойЧасти];
...
Функция КлючНастроек(Знач Строка)
	...
	Если ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() > 803040000 Тогда
		Если СтрДлина(Строка) > 128 Тогда // ключ более 128 символов вызовет исключение при обращении к хранилищу настроек
			...
			Выполнить(" ХешированиеДанных = Новый ХешированиеДанных(ХешФункция.MD5); ");
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = "РЕГИСТРРАСЧЕТА" Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение ПодставитьПараметрыВСтроку(
				ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Неизвестный тип объекта метаданных") + " ""%1"" ", ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
...
Процедура Назад(Команда)
	...
	Элементы.ФормаИзменить.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменить реквизиты");
	ОбновитьНадписи();
...
Процедура НастроитьПараметрыИзменения(Команда)
	...
	ОткрытьФорму(ПолноеИмяФормы("Настройка"), ПараметрыФормы, ЭтаФорма);
...
Процедура НастройкаОтбораНажатие(Элемент)
	...
	Если Не ПустаяСтрока(ВидИзменяемыхОбъектов) Тогда
		Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
			Выполнить(" ОписаниеОповещения = Новый ОписаниеОповещения(""ПриЗакрытииФормыВыбранныхОбъектов"", ЭтаФорма);
			| ОткрытьФорму(ПолноеИмяФормы(""ВыбранныеЭлементы""), Новый Структура(""ВидИзменяемыхОбъектов, Настройки"", ВидИзменяемыхОбъектов, КомпоновщикНастроек.Настройки), , , , , ОписаниеОповещения); ");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		Если ПустаяСтрока(ТекстСообщенияОбОшибке) Тогда
			...
			ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выбрано элементов:") + " %1", КоличествоВыбранныхОбъектов);
		Иначе
			ЕстьОшибкиВОтборе = ИСТИНА;
			ТекстНадписи = ПодставитьПараметрыВСтроку(
			ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Элементы не выбраны.") + " %1", ТекстСообщенияОбОшибке);
		КонецЕсли;
	Иначе
		ТекстНадписи = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменять все элементы");
	КонецЕсли;
...
Процедура ОбновитьСчетчикиИзменяемыхРеквизитов(Знач ТаблицаФормы = Неопределено)
	...
	Для Каждого ТаблицаФормы Из СписокТаблиц Цикл
		ТабличнаяЧасть = ЭтаФорма[ТаблицаФормы.Имя];
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		Если Объект.ИзменятьВТранзакции Тогда
			...
			ТекстПредупреждения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("При изменении элементов обнаружены ошибки - изменения отменены.");
			ПодключитьОбработчикОжидания("ЗавершитьИзменениеОбъектов", 0.1, ИСТИНА);

			Возврат; // досрочный выход Из цикла и процедуры
		КонецЕсли;

		ТекстВопроса = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("При изменении элементов (группы элементов) возникли ошибки.") + "
		|" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Прервать изменение элементов и перейти к просмотру ошибок?" + "|'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Прервать, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Прервать"));
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Продолжить"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Больше не спрашивать"));
		ТекстОповещения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибки при изменении элементов");

		Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
			Выполнить(" ОписаниеОповещения = Новый ОписаниеОповещения(""ОбработатьРезультатИзмененияОтветПолучен"", ЭтаФорма, РезультатИзменения);
			| ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, ТекстОповещения); ");
		КонецЕсли;
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Обрабатываются элементы.."), ТекущийПроцент, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение выделенных элементов"));
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.ГрупповоеИзменениеОбъектов.Форма.Настройка") Тогда
		...
		Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
			...
			Если ПерезаполнитьСтруктуруРеквизитовОбъекта Тогда
				ИнициализироватьКомпоновщикНастроек();
				ЗагрузитьМетаданныеОбъекта();
			КонецЕсли;
...
// // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // //
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ ОбъектыКоторыеНеУдалосьИзменить
&НаКлиенте
Процедура ОбъектыКоторыеНеУдалосьИзменитьПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТипЗнч(Элемент.ТекущиеДанные.Объект) <> Тип("Строка") Тогда
		Если глВерсияПлатформы < 803040000  Тогда
			Выполнить(" ОткрытьЗначение(Элемент.ТекущиеДанные.Объект); ");
		Иначе
			Выполнить(" ПоказатьЗначение(, Элемент.ТекущиеДанные.Объект); ");
		КонецЕсли;
	КонецЕсли;
...
Функция ОписанияКолонокТаблицыРеквизитов()
	...
	КолонкиТаблиц.Колонки.Добавить("Имя", , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Имя"));
	КолонкиТаблиц.Колонки.Добавить("Тип", , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Тип"));
	КолонкиТаблиц.Колонки.Добавить("Представление", , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Представление"));
	...
	ОписаниеКолонки.Тип = Новый ОписаниеТипов("Строка");
	ОписаниеКолонки.Представление = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Реквизит");
	...
	ОписаниеКолонки.Представление = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Новое значение");
	ОписаниеКолонки.ВидПоля = ВидПоляФормы.ПолеВвода;
...
Процедура Подключаемый_ПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущийЭлемент = Элементы.РеквизитыОбъектаЗначение И Элемент.ТекущиеДанные.ЗаблокированныйРеквизит Тогда
		Отказ = ИСТИНА;
...
Функция ПодставитьПараметрыВСтрокуАльтернативныйАлгоритм(Знач СтрокаПодстановки,
	...
	Пока Позиция > 0 Цикл
		...
		Если ПустаяСтрока(ПодставляемыйПараметр) Тогда
			Результат = Результат + "%";
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПерейтиВЖурналРегистрации.Видимость = ПравоДоступа("Администрирование", Метаданные) И ПодсистемаСуществует("СтандартныеПодсистемы.БазоваяФункциональность");
	...
	Иначе
		Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Групповое изменение реквизитов");
	КонецЕсли;
...
Функция ПрочитатьXMLВТаблицу(Знач XML)
	...
	Если Не Чтение.Прочитать() Тогда
		...
		ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Пустой XML"));
	ИначеЕсли Чтение.Имя <> "Items" Тогда
		ВызватьИсключение(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка в структуре XML"));
	КонецЕсли;
...
Процедура РазрешитьРедактированиеРеквизитов()
	...
	Если ЕстьФормаРазблокированияРеквизитов Тогда
		...
		ОткрытьФорму(ПолноеИмяФормыРаботыСБлокируемымиРеквизитами, ПараметрыФормы, ЭтаФорма);
	Иначе
		...
		Если ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
			...
			Если ЗапретРедактированияРеквизитовОбъектовКлиентМодуль <> Неопределено Тогда
				Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
					Выполнить(" ЗапретРедактированияРеквизитовОбъектовКлиентМодуль.ПроверитьСсылкиНаОбъект(Новый ОписаниеОповещения(""РазрешитьРедактированиеРеквизитовЗавершение"", ЭтаФорма, ЗаблокированныеРеквизитыСтроки), МассивСсылок, СинонимыРеквизитов); ");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СброситьНастройкиИзменения()
	...
	Для Каждого ТабличнаяЧасть Из ТабличныеЧастиОбъекта Цикл
		Для Каждого Реквизит Из ЭтаФорма[ТабличнаяЧасть.Значение] Цикл
			Реквизит.Значение = Неопределено;
			Реквизит.Изменять = ЛОЖЬ;
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
		Пояснение = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("реквизит") + " " + Пояснение;
	ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
		Пояснение = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("реквизиты") + " " + Пояснение;
	КонецЕсли;

	Если Не ПустаяСтрока(Пояснение) Тогда
		Пояснение = Пояснение + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("в выбранных элементах");
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			...
			ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("реквизиты") + " (%1)", ИзменяемыеРеквизиты.Количество());
		Иначе
			Для Каждого Реквизит Из ИзменяемыеРеквизиты Цикл
				Если Не ПустаяСтрока(Пояснение) Тогда
					Пояснение = Пояснение + ", ";
				...
				Если ИзменяемыеРеквизиты.Найти(Реквизит) = 0 Тогда
					Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
						Пояснение = Пояснение + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("реквизит") + " ";
					ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
						Пояснение =  Пояснение + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("реквизиты") + " ";
					КонецЕсли;
				КонецЕсли;
		...
		Пояснение = Пояснение + " "
		+ ПодставитьПараметрыВСтроку(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("в табличной части") + " ""%1""", ТабличнаяЧасть.Ключ);
	КонецЦикла;

	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда
				Пояснение = Пояснение + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("в тех строках выбранных элементов, которые удовлетворяют условиям отбора")
			Иначе
				Пояснение = Пояснение + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("во всех строках выбранных элементов")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		Если Не ПустаяСтрока(Пояснение) Тогда
			Пояснение = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменить") + " " + Пояснение + ".";
		Иначе
			Пояснение = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выполнить перезапись выбранных элементов.");
		КонецЕсли;
	Иначе
		Пояснение = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не выбраны элементы, реквизиты которых необходимо изменить.");
	КонецЕсли;
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Если НачалоИзменения Тогда
		Элементы.ФормаИзменить.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Прервать");
	Иначе
		Если ОбъектыКоторыеНеУдалосьИзменить.Количество() > 0 Тогда

			Элементы.ФормаИзменить.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Повторить изменение");
		Иначе
			Элементы.ФормаИзменить.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Закрыть");
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьНастройкуИзменения(Знач Настройка)
	...
	Для Каждого ТабличнаяЧасть Из Настройка.ТабличныеЧасти Цикл
		Для Каждого ИзменяемыйРеквизит Из ТабличнаяЧасть.Значение Цикл
			...
			Если Элементы.Найти("ТабличнаяЧасть" + ТабличнаяЧасть.Ключ) <> Неопределено Тогда
				НайденныеСтроки = ЭтаФорма["ТабличнаяЧасть" + ТабличнаяЧасть.Ключ].НайтиСтроки(СтруктураПоиска);
				Если НайденныеСтроки.Количество() > 0 Тогда
					НайденныеСтроки[0].Значение = ИзменяемыйРеквизит.Значение;
					НайденныеСтроки[0].Изменять = ИСТИНА;
				КонецЕсли;
	...
	Если ЕстьЗаблокированные Тогда
			ТекстПредупреждения = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Некоторые реквизиты заблокированы для изменения, изменения не установлены.");
		Если глВерсияПлатформы < 803040000  Тогда
			Выполнить(" Предупреждение(ТекстПредупреждения); ");
		Иначе
			Выполнить(" ПоказатьПредупреждение( , ТекстПредупреждения); ");
...
&НаКлиенте
Процедура ОбъектыКоторыеНеУдалосьИзменитьПриИзменении(Элемент)
	глВремяПоследнегоСобытия = ТекущаяДата();
КонецПроцедуры
