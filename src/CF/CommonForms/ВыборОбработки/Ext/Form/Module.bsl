...
Процедура ВосстановлениеПредопределенныхЭлементовНаСервере()
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.СтрокаОшибок) Тогда;
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Восстановлены предопределенные элементы, не сопоставленные") + ": " + Символы.ПС + ДополнительныеПараметры.СтрокаОшибок);
	Иначе
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Все предопределенные данные сопоставлены"));
	КонецЕсли;
...
КонецПроцедуры // ВыгрузитьШтрихКодКОличество
...
Процедура ВыполнитьПрограммныйКод1СНаКлиенте(Знач ПрограммныйКод1СДляВыполнения, ПереданноеЗначение = Неопределено)
	...
	Если НЕ ПустаяСтрока(ПрограммныйКод1СДляВыполнения) Тогда
		Попытка
...
Процедура ОчиститьВсеЗаписиОТекстахНаВсехЯзыкахНажатиеЗавершениеНаСервере() Экспорт
	...
	РегистраСведенийНаборЗаписей = РегистрыСведений.ТекстыНаДругихЯзыках.СоздатьНаборЗаписей();
	РегистраСведенийНаборЗаписей.Записать(ИСТИНА);
	
	РегистраСведенийНаборЗаписей = РегистрыСведений.ЗначенияНаДругихЯзыках.СоздатьНаборЗаписей();
	РегистраСведенийНаборЗаписей.Записать(ИСТИНА);
...
Процедура ОчиститьВсеЛичныеНастройкиПользователейНаСервере()
	...
	РегистраСведенийНаборЗаписей = РегистрыСведений.НастройкиПользователей.СоздатьНаборЗаписей();
	РегистраСведенийНаборЗаписей.Прочитать();
	РегистраСведенийНаборЗаписей.Очистить();
	РегистраСведенийНаборЗаписей.Записать(ИСТИНА);
...
Процедура ОчиститьВсеШтрихКодыНаСервере()
	...
	РегистраСведенийНаборЗаписей = РегистрыСведений.ШтрихКоды.СоздатьНаборЗаписей();
	РегистраСведенийНаборЗаписей.Записать(ИСТИНА);
...
Процедура ОчиститьСообщенияПользователейНаСервере()
	...
	РегистраСведенийНаборЗаписей = РегистрыСведений.СообщенияПользователюОтСистемы.СоздатьНаборЗаписей();
	РегистраСведенийНаборЗаписей.Очистить();
	РегистраСведенийНаборЗаписей.Записать(ИСТИНА);
...
Процедура ПеренестиНоменклатуруПоПапкамПоПервомуСловуНаименованияНаСервере()
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ГруппаПоПервомуСлову = ОбщийМодульТоварСервер.ПолучитьНаименованиеГруппыПоПервомуСлову(ВыборкаДетальныеЗаписи.наименование);
...
Процедура ПеренумерацияИСжатиеСправочникаНаСервере(Знач ИмяСправочника)
	...
	Если НЕ ВзКоличествоИмяСправочника.Пустой() Тогда
		...
		ДлинаКодаЭлемента =	метаданные.Справочники[ИмяСправочника].ДлинаКода;
		СтрокаНулей = "";
		сч = 1;
		пока сч < ДлинаКодаЭлемента цикл
			СтрокаНулей = СтрокаНулей + "0";
			сч = сч + 1;
		КонецЦикла;
		
		нкод = СтрокаНулей + "1";
		страда = стрдлина(СтрокаНулей)+1;
			...
			Если НЕ ММ.предопределенный Тогда
				...
				пока ИСТИНА цикл
					...
					Иначе
						эточисло = число(нкод);
						эточисло = эточисло + 1;
						нкод = прав(СтрокаНулей + стрзаменить(СокрЛП(эточисло), символы.НПП, ""), страда);
					КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		Элементы.СначалаНаКлиенте.Видимость = ЛОЖЬ;
		Элементы.ПослеСноваНаКлиенте.Видимость = ЛОЖЬ;
		Элементы.ВЭтомРежимеПрограммаВыполняетсяТолькоНаСервере.Видимость = ИСТИНА;
	#КонецЕсли
	
	ВремяНачалаРаботыПользователя = глВремяНачалаРаботыПользователя;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Отказ Тогда
		...
		Элементы.ЗначениеСтрока.Заголовок		= "ЗначениеСтрока";
		...
		Элементы.ЗначениеДата.Заголовок			= "ЗначениеДата";
		...
		Элементы.ЗначениеЧисло.Заголовок		= "ЗначениеЧисло";
		...
		Элементы.ЗначениеБулево.Заголовок		= "ЗначениеБулево";
		...
		Элементы.ДокументДляПечати.Заголовок	= "ДокументДляПечати";
		СтрокаСправкиККоду.ОписаниеПрименения 	= ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Доступный табличный документ");
		СтрокаСправкиККоду = СправкаККоду.Добавить();
		СтрокаСправкиККоду.ИмяПрограммное 		= "ТекстДок";
		Элементы.ТекстДок.Заголовок 			= "ТекстДок";
		...
		Элементы.СписЗнач.Заголовок				= "СписЗнач";
		...
		Элементы.ЗначениеСсылка.Заголовок		= "ЗначениеСсылка";
		...
		Элементы.ЗначениеПроизвольное.Заголовок	= "ЗначениеПроизвольное";
...
Функция   ПроверитьНаличиеНовойВерсииКонфигурацииНаСервере()
	...
	ВопросКСерверу = ОбщийМодульСервер.ЕстьНоваяВерсияКонфигурацииЭТ(ИСТИНА);
...
Процедура СвязатьСОдноименнойНоменклатурнойГруппойНаСервере()
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			НоменклатурнаяГруппа = ОбщийМодульПовторВТеченииСервера.НайтиНоменклатурнуюГруппуПоНаименованию(СокрЛП(ВыборкаДетальныеЗаписи.родитель), ВыборкаДетальныеЗаписи.Наименование, ИСТИНА);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если НЕ ПустаяСтрока(ОсновнойШтрихКод) Тогда
				...
				ЗаписьШтрихкода.Активность = ИСТИНА;
				ЗаписьШтрихкода.Номенклатура = ВыборкаДетальныеЗаписи.ссылка;
				ЗаписьШтрихкода.ТипШтрихКода = ТипШтрихКодов;
				ЗаписьШтрихкода.ШтрихКод = ОсновнойШтрихКод;
				Попытка
					ЗаписьШтрихкода.Записать(ИСТИНА);
				Исключение
					ТекстОписаниеОшибки = ОписаниеОшибки();
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи в регистр сведений") + ": " + ЗаписьШтрихкода + " - " + ТекстОписаниеОшибки);
				КонецПопытки;
			КонецЕсли;
...
Процедура СкопироватьВсеШтрихКодыВДополнительныеНаСервере()
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ЗаписьДопШтрихкода = РегистрыСведений.ДополнительныеШтрихКоды.СоздатьМенеджерЗаписи();
			ЗаписьДопШтрихкода.Активность 			= ИСТИНА;
			ЗаписьДопШтрихкода.ЕдиницаИзмерения 	= ВыборкаДетальныеЗаписи.ЕдиницаИзмерения;
			ЗаписьДопШтрихкода.Номенклатура		= ВыборкаДетальныеЗаписи.Номенклатура;
			ЗаписьДопШтрихкода.СерияНоменклатуры	= ВыборкаДетальныеЗаписи.СерияНоменклатуры;
			ЗаписьДопШтрихкода.ТипШтрихКода		= ВыборкаДетальныеЗаписи.ТипШтрихКода;
			ЗаписьДопШтрихкода.ШтрихКод			= ВыборкаДетальныеЗаписи.ШтрихКод;
			Попытка
				ЗаписьДопШтрихкода.Записать(ИСТИНА);
			Исключение
			КонецПопытки;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если ВестиУчетНоменклатурыВРазрезеНоменклатурныхГрупп
				...
				ОбъектНоменклатура.НоменклатурнаяГруппа = ОбщийМодульПовторВТеченииСервера.НайтиНоменклатурнуюГруппуПоНаименованию(СокрЛП(ВыборкаДетальныеЗаписи.родитель), ВыборкаДетальныеЗаписи.Наименование);
			КонецЕсли;
...
Процедура УдалениеДублейНоменклатурыПоШтрихКодуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Состояние(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Поиск дублей номенклатуры по штрихкоду.."), , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ждите.."));
		СтруктураНастроек = ПодготовкаКУдалению();
		Состояние(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Перенос ссылок для дублей номенклатуры по штрихкоду.."), , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ждите.."), 10);
		УдалениеЗадублированнойНоменклатурыНаСервере(ИСТИНА);
		Состояние(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Завершение поиска дублей номенклатуры по штрихкоду.."), , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ждите.."), 90);
...
Процедура УдалениеЗадублированнойНоменклатурыНаСервере(Знач ПоШтрихКоду = ЛОЖЬ)
	...
	|	ИЗ РегистрНакопления.Товары.Остатки КАК ТоварыОстатки) КАК ВложенныйЗапрос
	...
	|	Номенклатура.Наименование
	|ИЗ Справочник.Номенклатура КАК Номенклатура
	|ГДЕ Номенклатура.Предопределенный = ЛОЖЬ
	|	И Номенклатура.ЭтоГруппа = ЛОЖЬ
	|УПОРЯДОЧИТЬ ПО Номенклатура.ПометкаУдаления УБЫВ";
	...
	|ГДЕ ТоварыПереданныеНаКомиссию.Номенклатура = &Номенклатура
...
&НаСервере
Процедура ПометитьДублиПоАртикулуИПроизводителюНаСервере()
	...
	МассивСораненных = Новый Массив;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Номенклатура.Ссылка КАК Оригинал,
	|	ВложенныйЗапрос.Ссылка КАК Дубль
	|ИЗ Справочник.Номенклатура КАК Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			Номенклатура.Ссылка КАК Ссылка,
	|			Номенклатура.Артикул КАК Артикул,
	|			Номенклатура.Производитель КАК Производитель
	|		ИЗ Справочник.Номенклатура КАК Номенклатура) КАК ВложенныйЗапрос
	|		ПО Номенклатура.Артикул = ВложенныйЗапрос.Артикул
	|			И Номенклатура.Производитель = ВложенныйЗапрос.Производитель
	|ГДЕ НЕ Номенклатура.Ссылка = ВложенныйЗапрос.Ссылка";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		МассивСораненных.Вставить(ВыборкаДетальныеЗаписи.Оригинал);
		Если МассивСораненных.Найти(ВыборкаДетальныеЗаписи.Дубль) = Неопределено Тогда
			ОбъектКУдалению = ВыборкаДетальныеЗаписи.Дубль.ПолучитьОбъект();
			ОбъектКУдалению.ПометкаУдаления = ИСТИНА;
			ОбъектКУдалению.Записать();
		КонецЕсли;
	КонецЦикла;
	
	Если УстановленМонопольный Тогда
		Попытка
			УстановитьМонопольныйРежим(ЛОЖЬ);
		Исключение
		КонецПопытки;
...
&НаКлиенте
Процедура ПометитьДублиПоАртикулуИПроизводителю(Команда)
	
	Если Вопрос(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы готовы пометить дубли на удаление?"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		ПометитьДублиПоАртикулуИПроизводителюНаСервере();
	КонецЕсли;
