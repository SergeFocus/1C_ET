...
Процедура ПоискИУдалениеПомеченныхЭлементовИДокументовНаСервере()
	...
	Исключение
		...
		Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не удалось удалить помеченные на удаление объекты.") + Символы.ПС + ТекстОписаниеОшибки, СтатусСообщения.ОченьВажное);
	КонецПопытки;
...
Процедура УдалениеВсехЗаметокНаСервере()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Исключение
			...
			Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка очистки регистра сведений") + ": " +  + " " + ТекстОписаниеОшибки;
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЦикла;
	...
	Если НЕ ВзКоличествоИмяСправочника.Пустой() Тогда
		...
		для М = 1 по кво цикл
			...
			Если НЕ ММ.предопределенный Тогда
				...
				Исключение 
					ТекстОписаниеОшибки = ОписаниеОшибки();
					Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не удалось записать") + " " + ммм + " " + ТекстОписаниеОшибки);
					ПРЕРВАТЬ;
				конецпопытки;
							...
							Если НЕ РезультатЗапроса4.Пустой() Тогда
								...
								Пока ВыборкаДетальныеЗаписи4.Следующий() Цикл
									...
									Исключение 
										ТекстОписаниеОшибки = ОписаниеОшибки();
										Сообщение = Новый СообщениеПользователю;
										Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи в регистр сведений цены") + ": " + Цена + " - " + ТекстОписаниеОшибки;
										Сообщение.Сообщить();
									КонецПопытки; 
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Исключение 
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи номенклатуры") + ": " + ОбъектНоменклатура + " - " + ТекстОписаниеОшибки;
				Сообщение.Сообщить();
...
Процедура ВыгрузитьШтрихКодЦена(Команда)
	...
	ДиалогВыбФайла.Показать(Новый ОписаниеОповещения("ВыгрузитьШтрихКодЦенаЗавершение1", ЭтаФорма, Новый Структура("ДиалогВыбФайла", ДиалогВыбФайла)));
...
Процедура ВыгрузитьШтрихКодЦенаЗавершение1(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		...
		ПоказатьВводЗначения(Новый ОписаниеОповещения("ВыгрузитьШтрихКодЦенаЗавершение", ЭтаФорма, Новый Структура("ДиалогВыбФайла, ВидЦен", ДиалогВыбФайла, ВидЦен)), ВидЦен, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Укажите вид цен"), тип("СправочникСсылка.ВидыЦен"));
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если НЕ ПустаяСтрока(ОсновнойШтрихКод) Тогда
				...
				Исключение // Записи в регистр сведений 
					ТекстОписаниеОшибки = ОписаниеОшибки();
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи в регистр сведений") + ": " + ШК + " - " + ТекстОписаниеОшибки;
					Сообщение.Сообщить();
				КонецПопытки; // Записи в регистр сведений
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Отказ Тогда
		...
		СтрокаСправкиККоду = СправкаККоду.Добавить();
		СтрокаСправкиККоду.ИмяПрограммное 		= "ТабДок";
...
&НаКлиенте
Процедура ВыполнитьПрограммныйКод1СПоНомеру(ПрограммныйКод1СНаСервере, ПрограммныйКод1СНаКлиентеСначала, ПрограммныйКод1СНаКлиентеОкончание)
...
Процедура Пользователи(Команда)
	...
	ФормаСписка = ПолучитьФорму("Справочник.Пользователи.ФормаСписка");
	ФормаСписка.Открыть();
...
Процедура ПроизвольнаяПечатьТаблицы(Команда)
	...
	ФормаОбработки = ПолучитьФорму("Обработка.ПроизвольнаяПечатьТаблицыТоваров.Форма.Форма");
	ФормаОбработки.Открыть();
...
Процедура ОператорСообщить(Команда)
	ПрограммныйКод1С = ПрограммныйКод1С + Символы.Таб + "Сообщить("""");" + Символы.ПС;
КонецПроцедуры
...
Процедура ОператорПопытка(Команда)
	...
	|
	|Исключение 	
	|Сообщить(""Произошла ошибка: "" + ТекстОписаниеОшибки, СтатусСообщения.ВажнаяИнформация);
...
Процедура ОператорСообщитьКлиент(Команда)
	ПрограммныйКод1СКлиент = ПрограммныйКод1СКлиент + Символы.Таб + "Сообщить("""");" + Символы.ПС;
КонецПроцедуры

&НаКлиенте
Процедура ОператорСообщитьКлиентПосле(Команда)
	ПрограммныйКод1СКлиентОкончание = ПрограммныйКод1СКлиентОкончание + Символы.Таб + "Сообщить("""");" + Символы.ПС;
КонецПроцедуры
...
Процедура ВыгрузитьКодВФайл(Команда)
	...
	ДиалогРасширение = "*";
	ДиалогВыбФайла 	 = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	ДиалогВыбФайла.Заголовок				=	ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выберите файл для записи кода") + ":";
	...
	ДиалогВыбФайла.Показать(Новый ОписаниеОповещения("ВыгрузитьКодВФайлЗавершение", ЭтаФорма, Новый Структура("ДиалогВыбФайла", ДиалогВыбФайла)));
...
Процедура ЗагрузитьКодИзФайла(Команда)
	...
	ДиалогВыбФайла.Заголовок				=	ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выберите файл для импорта программного кода") + ":";
	ДиалогВыбФайла.ПолноеИмяФайла			=	АдресФайла; //АДРЕС
	ДиалогВыбФайла.Фильтр					=	ДиалогФильтр;
	ДиалогВыбФайла.Расширение				=	ДиалогРасширение;
	ДиалогВыбФайла.МножественныйВыбор		=	Ложь;
	...
	ДиалогВыбФайла.Показать(Новый ОписаниеОповещения("ЗагрузитьКодИзФайлаЗавершение", ЭтаФорма, Новый Структура("ДиалогВыбФайла", ДиалогВыбФайла)));
...
Процедура ПереформироватьАртикулыНоменклатурыНаСервере()
	...
	Если ЗначениеЗаполнено(ФормулаФормированияАртикула) Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Внесены изменения") + ": " + Счетчик);
		КонецЕсли;
		
	Иначе
		Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Формула артикула не задана"));
	КонецЕсли;
...
Процедура ОчиткаРегистраВерсионированияНаСервере()
	...
	Иначе
		Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("У Вас недостаточно прав для этого!"));
	КонецЕсли;
