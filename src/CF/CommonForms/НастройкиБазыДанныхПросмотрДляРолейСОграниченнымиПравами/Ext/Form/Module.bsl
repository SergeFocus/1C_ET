...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если Не Отказ Тогда
		...
		если ПараметрыСеанса.НеМожетМенятьЦены тогда
			элементы.ВидЦенРасходованияПоУмолчанию.Доступность 	= Ложь;	
			элементы.РеализаторНеМожетМенятьЦены.Доступность 	= Ложь;	
		КонецЕсли;
		...
		Элементы.МеханизмЛьготнойЦены.Видимость =ПараметрыСеанса.ВключитьМеханизмЛьготнойЦеныПоКоличествуПриПродаже ;
...
Процедура ЗапретитьИзмененияВПрошломПериодеПриИзменении(Элемент)
	...
	ЗапретитьИзмененияВПрошломПериоде();
	ЗапретитьИзмененияВПрошломПериодеП = НаборКонстант.ЗапретитьИзмененияВПрошломПериодеП;
