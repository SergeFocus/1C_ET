...
&НаСервере
Процедура ЗаполнитьЧтоНового(Знач ТолькоВПоследнейВерсии = ЛОЖЬ)
	...
	Если НЕ МассивНеобходимыхОбластей.Количество() = 0 Тогда
		...
		Если НЕ ОбщийМодульПовтор.ПолнаяВерсияКонфигурации() Тогда
...
Процедура ОбновитьИменаПринтеров()
	...
	#Если Клиент Тогда
		Попытка
			...
			Пока НомерСимвола < Принтеры.Count()-1 Цикл
				ИмяПринтера = СокрЛП(Принтеры.Item(НомерСимвола+1));
		...
		Если НЕ ПомеченныеКоличество = 0 Тогда
			...
			Для Каждого Помеченный Из Помеченные Цикл
				...
				Пока ЖурналСобытий.Следующий() Цикл
					ЗаписиЖурнала = ЖурналСобытий.ПолучитьМенеджерЗаписи();
					ЗаписиЖурнала.Удалить();
				КонецЦикла;
