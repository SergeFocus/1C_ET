...
Процедура ЗаказПоставщику(Команда)
	...
	Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетПоПоставщикам", ИСТИНА) Тогда
		ОткрытьФорму("Справочник.Поставщики.ФормаВыбора", Новый Структура("Поставщик", Поставщик), ЭтаФорма, , , , Новый ОписаниеОповещения("ЗаказПоставщикуЗавершение", ЭтаФорма, Новый Структура("Поставщик", Поставщик)));
...
Процедура НовыйКлиентВыбратьТоварЗаказ(Команда)
	...
	НовыйКонтрагент = ОбщийМодульСервер.НайтиИлиСоздатьНовыйЭлементИлиДокумент(ИСТИНА, "Клиенты", ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Новый Клиент"));
...
Процедура НовыйКлиентВыбратьТоварЗаказФрагмент(Знач Количество, Знач НовыйКонтрагент, Знач РезультатНеОтменен)
	...
	Если РезультатНеОтменен Тогда
		...
		ФормаДокумента = ПолучитьФорму("Документ.РасходыТовара.Форма.ФормаДокументаЗаказ", ПараметрыФормы);
...
Процедура ОтчетыКомиссионеровФрагмент()
	...
	ОткрытьФорму("Документ.ОтчетыКомиссионеров.Форма.ФормаДокумента", ПараметрыФормы);
