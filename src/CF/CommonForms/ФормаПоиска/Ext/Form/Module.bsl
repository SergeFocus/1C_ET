...
Процедура СформироватьСтрокуНавигации(ВсегоСтраниц, ТекущаяПозиция, РазмерПорции, ПолноеКоличество, Запись)
    ...
    Если ТекущаяПозиция > 0 Тогда
        ...
        AElem.ТекстовоеСодержимое = НСтр(" ru = ""Предыдущая""", "ru");
    ...
    Иначе
        ...
        DivElem.ТекстовоеСодержимое = НСтр(" ru = ""Предыдущая""", "ru");
    ...
    Если ТекущаяПозиция + РазмерПорции < ПолноеКоличество Тогда
        ...
        AElem.ТекстовоеСодержимое = НСтр(" ru = ""Следующая""", "ru");
        TDElem.ДобавитьДочерний(AElem);
    ...
    Иначе
        ...
        DivElem = ДокументHTML.СоздатьЭлемент("DIV");
        DivElem.УстановитьАтрибут("class", "gray");
        DivElem.ТекстовоеСодержимое = НСтр(" ru = ""Следующая""", "ru");
...
|</head>
|<body>
|<p style=font-size:12pt;>По запросу <b>&quot;""", "ru") + Стр + НСтр(" ru = ""&quot;</b> ничего не найдено.</p>
|<p class=green>Рекомендации:</p>
|<ul>
|<li><b>Упростите запрос, исключив из него какое-либо слово.</b>
|<li><b>Воспользуйтесь поиском по началу слова.</b><br>
|Используйте звездочку (*) в качестве окончания.<br>
|Например, поиск стро* найдет все документы,
|которые содержат слова, начинающиеся на стро &ndash;
|Журнал &quot;Строительство и ремонт&quot;,
|&quot;ООО СтройКомплект&quot; и&nbsp;т.д.
|<li><b>Воспользуйтесь нечетким поиском</b>.<br>
|Используйте решетку (#).<br>
|Например, Сентинель#2
|найдет все документы, содержащие такие слова, которые отличаются от слова Сентинель
|на одну или две буквы.
|<li><b>Ищите синонимы.</b><br>
|Используйте восклицательный знак (!).<br>
|Например, поиск !инструкция найдет все документы в которых встречаются
|слова инструкция,
|предписание,
|указание.
|</ul>
