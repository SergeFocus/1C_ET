...
Процедура Далее(Команда)
	...
	Если Элементы.ГруппаСтраниц.ТекущаяСтраница = Элементы.ОсновныеНастройки Тогда
		Элементы.ГруппаСтраниц.ТекущаяСтраница = Элементы.ГруппаНастроекУчетаНоменклатуры;
	ИначеЕсли Элементы.ГруппаСтраниц.ТекущаяСтраница = Элементы.ГруппаУчетДенег Тогда
...
Процедура ОбновитьТекущийЯзыкИнтерфейса(Знач ВыберитеВашЯзыкБК = Неопределено)
	...
	Если НЕ Русский Тогда
		...
		ИначеЕсли  ВыберитеВашЯзыкБК = Справочники.Языки.Чешский
...
Процедура ПриЗакрытииНаСервере()
	...
	Если НЕ ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("НеОтправлятьСтатистикуАвторуПрограммыПриПервомЗапускеИОбновлении") Тогда
		...
		СтруктураПисьма.Вставить("РегистрационныйКод", ОбщийМодульПовтор.РегистрационныйКодКонфигурации());
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)     // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Константы.ПодсистемаИЭИмпортЭкспорт.Установить(ИСТИНА);
	
	Если Константы.ЧислоМинутПослеПоследнейОперацииКоторыеСчитаютсяРабочими.Получить() = 0 Тогда
	...
	Если ПустаяСтрока(ТекРегКод) Тогда
		ТекРегКод = ОбщийМодульСервисСервер.РегистрационныйКодКонфигурации();
	...
	Если НЕ ЗаконченаПервоначальнаяНастройка Тогда
		...
		Если НЕ Найти(ТекущийКодЛокализации, "EN") = 0 Тогда
			Если НЕ ЗначениеЗаполнено(ВыберитеВашЯзыкТут) Тогда
		...
		Константы.ПользователейНесколько.Установить(ОбщийМодульПовтор.ПолнаяВерсияКонфигурации());
	...
	НаименованиеВашегоОсновногоСклада 		 = ПредопределенноеЗначение("Справочник.Склады.ОсновнойСклад").Наименование;
