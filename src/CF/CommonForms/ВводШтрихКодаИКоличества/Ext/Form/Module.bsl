...
Процедура АртикулПриИзмененииНаСервере()
	...
	Если НЕ ПустаяСтрока(артикул) Тогда
		...
		Если НЕ ЗначениеЗаполнено(НоменклатураПоАртикулу)
			...
			НоменклатураПоАртикулу = ОбщийМодульТоварСервер.НайтиНоменклатуруПоПредставлениюКонтрагента(КлиентПоставщик, ПредопределенноеЗначение("Перечисление.ПоляПредставленийНоменклатуры.Артикул"), артикул);
		КонецЕсли;
...
Процедура Готово(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Номенклатура) Тогда
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Номенклатура не указана"), , "Номенклатура");
	ИначеЕсли ОбщийМодульПовтор.ТоварВедетсяПоСериям(Номенклатура)
		И НЕ ЗначениеЗаполнено(СерияНоменклатуры) Тогда

		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Серия товара не указана"), , "СерияНоменклатуры");
	Иначе
...
Процедура НоменклатураОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	ОбщийМодульКлиент.ПоискОшибкиПриВводеТекстаПользователем("Номенклатура", ДанныеВыбора, Текст, Номенклатура);
...
Процедура ОбновитьПоляКлючейУчета()
	...
	Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьКлючиУчетаНоменклатурыИИхСвязанныеСписки") Тогда
		...
		ТаблицаКлючей.Колонки.Добавить("ЗначениеПоУмолчанию");
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				СтрокаТаблицыКлючей.ЗначениеПоУмолчанию = ВыборкаДетальныеЗаписи.ЗначениеПоУмолчанию;
			...
			Для Каждого СтрокаТаблицыКлючей Из ТаблицаКлючей Цикл
				...
				Если ГруппаКлюча = Неопределено Тогда
					...
					Если НЕ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803040000 Тогда
						Выполнить(" ГруппаКлюча.Поведение = ПоведениеОбычнойГруппы.Свертываемая; ");
					КонецЕсли;
					...
					ИначеЕсли ТипДополнительногоРеквизита = Перечисления.ТипыДополнительныхРеквизитов.СсылкаНаЭлементСправочника Тогда
						Если ЗначениеЗаполнено(СтрокаТаблицыКлючей.ЗначениеПоУмолчанию) Тогда
							ЗначениеКлючаУчета = СтрокаТаблицыКлючей.ЗначениеПоУмолчанию;
						Иначе
							ЗначениеКлючаУчета = Неопределено;
						КонецЕсли;
					...
					НовыйЭлемент.ВыбиратьТип 	= ВыбиратьТип;
...
Процедура ОбновитьТекстЗапросаНаСервере()
	...
	|	" + ?(СОстатками, " ЕСТЬNULL(ВложенныйЗапрос.КоличествоОстаток, 0) ", "0") + " КАК Остаток
	|ИЗ
	...
	Если ВестиУчетНоменклатурыВРазрезеНоменклатурныхГрупп
		...
		Если НЕ НаименованияДополнительныхРеквизитов = Неопределено Тогда
			СчетчикВЦикле = 1;
			Для Каждого ДопРеквизит Из НаименованияДополнительныхРеквизитов Цикл
				Если СчетчикВЦикле <= 10 Тогда
					СокрЛПсч = СокрЛП(СчетчикВЦикле);
					ЭтоСтрока = ЛОЖЬ;
				...
				СчетчикВЦикле = СчетчикВЦикле + 1;

			КонецЦикла;

			Пока СчетчикВЦикле <= 10 Цикл
				СокрЛПсч = СокрЛП(СчетчикВЦикле);
...
Процедура СОстаткамиПриИзменении(Элемент)
	...
	Если НЕ СОстатками Тогда
		ТолькоСОстатками = ЛОЖЬ;
	КонецЕсли;
