...
&НаКлиенте
Процедура ДетализироватьИнформацияРуководителюПриИзменении(Элемент)
	ОбщийМодульКлиент.ДобавитьСобытиеЖурнала(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сменил") + " " + Элемент.Имя, 2);
КонецПроцедуры
	...
	Для Каждого Область Из МакетЧтоНового.Области Цикл
		...
		Если НЕ ПустаяСтрока(ИмяОбласти) Тогда
			Попытка
				...
				Если НЕ ТолькоВПоследнейВерсии
					ИЛИ НомерОбласти > ЧислоВерсияПриИнсталляции Тогда
...
Процедура ОбновитьВидимостьЭлементов()
	...
	Элементы.ВидЦенДляРегистрацииФактическойЦеныПриобретения.Видимость = НаборКонстант.ИспользоватьСложныйМеханизмЦен;

	ДоступноНазначатьРазличныеСпособыФормированияЦеныДляНоменклатурныхГрупп = ПолнаяВерсия
	И НаборКонстант.ИспользоватьСложныйМеханизмЦен
	И НаборКонстант.ВестиУчетНоменклатурыВРазрезеНоменклатурныхГрупп;
...
Процедура ОбновитьИнформационныеНадписи(Знач ТолькоПереименование = ЛОЖЬ)
	...
	Элементы.Инструкция.Заголовок 			= ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Инструкция");
...
Процедура ПоведениеПрограммыИзмена()
	...
	ВидимостьЭлементов 	= ЗначениеЗаполнено(Поведение)
	И НЕ Поведение = Перечисления.ИгнорироватьРазрешитьЗапретить.Игнорировать;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)     // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Все что угодно, в зависимости от производственной необходимости: от размера одежды, до имени домашнего животного.");

		Элементы.ОписаниеМеханизмаОтсрочки.Заголовок =
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Отсрочка позволяет разделить долги клиентов и долги перед поставщиками на такие, которые уже просрочены и такие, по которым оплата еще только ожидается.");
		...
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оборудование, инструменты, недвижимость, земля, транспорт и т.д.");
		...
		ТекстИнструкции = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("На закладках по разделам распределены элементы настроек.") + Символы.ПС + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Некоторые настройки включают или выключают элементы конфигурации.") + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Другие задают поведение программы конфигурации.") + Символы.ПС + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Настройки внешнего вида задаются в отдельной форме.") + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Настройки обмена и оборудования в отдельной форме.") + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Настройки значений для подстановки по-умолчанию также отдельно.") + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Эти формы настроек доступны по командам из командной панели.") + Символы.ПС + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Здесь устанавливаются общие настройки для всех пользователей.") + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Некоторые настройки могут быть заданы индивидуально для каждого пользователя (личные).") + Символы.ПС;

		Элементы.СмотритеТакЖе.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Смотрите также страницу с инструкциями на сайте конфигурации") + ": ";

		Элементы.Декорация4.Заголовок = ТекстИнструкции;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗапретитьЗаписьКлиентовБезУказанияГруппыПриИзменении(Элемент)
	ОбщийМодульКлиент.ДобавитьСобытиеЖурнала(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сменил") + " " + Элемент.Имя + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("на") + " " + СокрЛП(НаборКонстант.ЗапретитьЗаписьКлиентовБезУказанияГруппы), 2);
КонецПроцедуры

&НаКлиенте
Процедура ЗапретитьЗаписьНоменклатурыБезУказанияГруппыПриИзменении(Элемент)
	ОбщийМодульКлиент.ДобавитьСобытиеЖурнала(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сменил") + " " + Элемент.Имя + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("на") + " " + СокрЛП(НаборКонстант.ЗапретитьЗаписьНоменклатурыБезУказанияГруппы), 2);
КонецПроцедуры

&НаКлиенте
Процедура КомандаДругая(Команда)

	Если Элементы.Декорация1.Видимость = ИСТИНА Тогда
		Элементы.Декорация1.Видимость = ЛОЖЬ;
		Элементы.Декорация2.Видимость = ИСТИНА;
	ИначеЕсли Элементы.Декорация2.Видимость = ИСТИНА Тогда
...
&НаКлиенте
Процедура СтраницаОбновленияКонфигурации1Нажатие(Элемент)

	Если глВерсияПлатформы < 806010000 Тогда
		Выполнить(" ЗапуститьПриложение(""http://sites.google.com/site/elementarytrade/instrukcii-v-kartinkah""); ");
	Иначе
		Выполнить(" НачатьЗапускПриложения(Неопределено, ""http://sites.google.com/site/elementarytrade/instrukcii-v-kartinkah""); ");
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура СтраницаОбновленияКонфигурации2Нажатие(Элемент)

	Если глВерсияПлатформы < 806010000 Тогда
		Выполнить(" ЗапуститьПриложение(""http://groups.google.com/d/forum/elemental-trade-forum""); ");
	Иначе
		Выполнить(" НачатьЗапускПриложения(Неопределено, ""http://groups.google.com/d/forum/elemental-trade-forum""); ");
	КонецЕсли;
...
Процедура ДекорацияЛогоНажатие(Элемент)
	...
	Элементы.ДекорацияЛого.Видимость = ЛОЖЬ;
	Элементы.ДекорацияЛогоАниме.Видимость = ИСТИНА;

КонецПроцедуры

&НаКлиенте
Процедура ВестиУчетРабочегоВремениСотрудниковПриИзменении(Элемент)
	СообщитьОСменеФункциональнойОпции(Элемент.Имя);
КонецПроцедуры

&НаКлиенте
Процедура ВыводитьЗапросОТомЧтоСотрудникПользовательНаРабочемМестеПриИзменении(Элемент)
	ОбщийМодульКлиент.ДобавитьСобытиеЖурнала(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сменил") + " " + Элемент.Имя + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("на") + " " + СокрЛП(НаборКонстант.ВыводитьЗапросОТомЧтоСотрудникПользовательНаРабочемМесте), 2);
КонецПроцедуры

&НаКлиенте
Процедура ЧислоМинутПослеПоследнейОперацииКоторыеСчитаютсяРабочимиПриИзменении(Элемент)
	
	Если НаборКонстант.ЧислоМинутПослеПоследнейОперацииКоторыеСчитаютсяРабочими < 2 Тогда
		НаборКонстант.ЧислоМинутПослеПоследнейОперацииКоторыеСчитаютсяРабочими = 2;
	КонецЕсли;
	
	ОбщийМодульКлиент.ДобавитьСобытиеЖурнала(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сменил") + " " + Элемент.Имя + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("на") + " " + СокрЛП(НаборКонстант.ЧислоМинутПослеПоследнейОперацииКоторыеСчитаютсяРабочими), 2);
