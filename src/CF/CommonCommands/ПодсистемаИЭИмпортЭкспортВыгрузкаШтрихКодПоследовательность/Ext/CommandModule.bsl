...
Процедура ВыгрузитьШтрихКодКОличество(Знач Адрес, Знач ПараметрКоманды)
	...
	Для Каждого СтрокаТовара Из ПараметрКоманды[ИмяТаблицы] Цикл
		...
		Для СчетчикКоличества = 1 По Количество Цикл
...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если НЕ Отказ Тогда
		КвоСтрок = ?(ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.ТаблицыДанных"), 1, ПодсистемаИЭИмпортЭкспортФС.параметркомандыТоварыколичество(ПараметрКоманды));
		Если НЕ КвоСтрок = Неопределено 
			...
			Если ДиалогВыбФайла.Выбрать() Тогда
				...
				ОбщийМодульКлиент.ДобавитьСобытиеЖурнала(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выгрузил последовательность Штрих-кодов в файл"), 2, ПараметрКоманды);
