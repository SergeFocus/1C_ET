...
Процедура ВыяснитьТекущийКурс(Знач ДляПериода = ЛОЖЬ)
	...
	Элементы.На1Валюту.Заголовок = СокрЛП(НаименованиеОсновнойВалюты) + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("на") + " 1 " + запись.Валюта;
...
Процедура ОбратныйПересчетКурса()
	...
	Если ЗначениеЗаполнено(запись.Валюта) 
		...
		Элементы.ДекорацияОбратногоПересчета.Заголовок = "" + СокрЛП(Запись.Валюта) + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("за") + " 1 " + НаименованиеОсновнойВалюты;
...
Процедура ПослеЗаписиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		РассчитатьВсеЗависимыеЦеныЗдесь(Запись.Валюта, Запись.Период);	
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура РассчитатьВсеЗависимыеЦеныЗдесь(Знач Валюта, Знач Период)	
...
Процедура РассчитатьВсеЗависимыеЦены(Команда)
	...
	Если НЕ ОТказ 
        ...
        Возврат;
	Иначе
	    ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение цен Вам недоступно!"));
	КонецЕсли;
	
	РассчитатьВсеЗависимыеЦеныФрагмент();
...
&НаКлиенте
Процедура РассчитатьВсеЗависимыеЦеныФрагмент()
