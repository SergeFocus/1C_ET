...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запись.ИсходныйКлючЗаписи.Пустой() Тогда
		Если НЕ ПодключаемоеОборудованиеДСервер.ПроверитьУникальностьШтрихКода(Запись.ШтрихКод, Запись.ТипШтрихКода) Тогда
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ШтрихКод не уникален!"));
			Отказ = ИСТИНА;
		КонецЕсли;
	КонецЕсли;

	Если НЕ ПроверитьТипШтрихКодаПередЗаписью() Тогда
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ШтрихКод не соответствует данному типу"));
		Отказ = ИСТИНА;
	КонецЕсли;
...
&НаСервере
Процедура ПолученШтрихКодИзСШК(Знач ШтрихКод)
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		Если Не ПустаяСтрока(Параметры.ШтрихКод) Тогда
