...
&НаКлиенте
Процедура ДобавитьЗаписьНаКлиенте(Знач ТекКод)

	ТекстВопроса = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Штрих-код") + " " + ТекКод + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("не зарегистрирован. Создать новую запись?");
	Ответ = Неопределено;
...
&НаСервере
Функция   ОбработатьПолученныйШтрихкод(Знач ТекКод)
