...
// sza151028-0522 
// sza151025-0513 доп расх руками
// sza151022-1316 допвидцен по складу
// sza150612-0303 вауч
// sza150225-1217 
// sza150216-0230 
// sza150111-0238 НЧ
...
Процедура ВзятьСуммуПоДокументуНаСервере()
	...
	Если ЕстьДополнительныеРасходы Тогда
		...
		Если Объект.ДополнительныеРасходы.Количество() = 0 Тогда
			СтрокаДопРасходов = Объект.ДополнительныеРасходы.Добавить();
		Иначе
			СтрокаДопРасходов = Объект.ДополнительныеРасходы[0];
		КонецЕсли;
		
		СтрокаДопРасходов.Валюта = ВалютаПоступления;
		СтрокаДопРасходов.Курс   = ОбщийМодульСервер.ПолучитьТекущийКурс(ВалютаПоступления, ?(СтрокаДопРасходов.ДатаРасхода = '00010101000000' , Объект.Дата, СтрокаДопРасходов.ДатаРасхода), ЛОЖЬ);
		СтрокаДопРасходов.Сумма  = ОбщийМодульСервер.ПоКурсу(Объект.Товары.Итог("Сумма"), ВалютаПоступления, ВалютаЦены, ?(СтрокаДопРасходов.ДатаРасхода = '00010101000000' , Объект.Дата, СтрокаДопРасходов.ДатаРасхода), СтрокаДопРасходов.Курс);
...
Процедура ВзятьСуммуСУчетомДолгаКлиентаНаСервере(Знач иСуммуДолга = ЛОЖЬ)
	...
	Если ЕстьДополнительныеРасходы Тогда
		Если иСуммуДолга Тогда
			...
			СтрокаДопРасходов 		 = Объект.ДополнительныеРасходы.Добавить();
			СтрокаДопРасходов.Сумма  = -ТекущийДолг;
			СтрокаДопРасходов.Валюта = Объект.Валюта;
			СтрокаДопРасходов.Курс   = Объект.Курс;
			СтрокаДопРасходов.НеРаспределятьНаСтоимость = ИСТИНА;
			
			Если ТекущийДолг > 0 Тогда
				СтрокаДопРасходов.ОСтроке = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оплата долга");
			Иначе
				СтрокаДопРасходов.ОСтроке = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Возврат долга");
			КонецЕсли;
			
			Если ТипЗнч(Объект.КлиентПоставщик) = Тип("СправочникСсылка.Клиенты") Тогда
				СтрокаДопРасходов.Статья  = Справочники.СтатьиДвиженияДенег.ДоходОтРеализации;
				СтрокаДопРасходов.ОСтроке = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сторно") + " " + СтрокаДопРасходов.остроке;
			Иначе
				СтрокаДопРасходов.Статья  = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенег.РасходыНаПриобретениеТоваров");
			КонецЕсли;       				
...
Процедура ВидЦенПриИзмененииНаСервере(Знач ПриНачальнойЗагрузке = ЛОЖЬ)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦен)
		...
		Если НЕ Объект.ДополнительныеРасходы.Количество() = 0  Тогда
			...
			СтрокаДопРасходов.Валюта = ВидЦенВалютаЦены;
			СтрокаДопРасходов.Курс   = ОбщийМодульСервер.ПолучитьТекущийКурс(СтрокаДопРасходов.Валюта, ?(СтрокаДопРасходов.ДатаРасхода = '00010101000000' , Объект.Дата, СтрокаДопРасходов.ДатаРасхода), ЛОЖЬ);
			
			Если ЗначениеЗаполнено(СтрокаДопРасходов.ХранилищеДенег)
				И НЕ СтрокаДопРасходов.ХранилищеДенег.Валюта = ВидЦенВалютаЦены Тогда
				
				СтрокаДопРасходов.ХранилищеДенег = ПредопределенноеЗначение("Справочник.ХранилищаДенег.ПустаяСсылка");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнительныеРасходыНаСервере()
	...
	Если НЕ ЗапретитьВозможностьРегистрироватьДополнительныеРасходыПриПриходованииТоваров
		...
		СтрокаДопРасходов 		 = Объект.ДополнительныеРасходы.Добавить();
		СтрокаДопРасходов.Валюта = Объект.Валюта;
		СтрокаДопРасходов.сумма  = Объект.ВыбылоДенег;
		СтрокаДопРасходов.ХранилищеДенег = Объект.ХранилищеДенег;
		
		Если НЕ ЗначениеЗаполнено(СтрокаДопРасходов.ХранилищеДенег)
			ИЛИ НЕ СтрокаДопРасходов.ХранилищеДенег.Валюта = Объект.Валюта Тогда
			
			Если ЗначениеЗаполнено(Объект.Склад)
				И ЗначениеЗаполнено(Объект.Склад.ХранилищеДенег) Тогда
				
				СтрокаДопРасходов.ХранилищеДенег = Объект.Склад.ХранилищеДенег;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(СтрокаДопРасходов.ХранилищеДенег)
				ИЛИ НЕ СтрокаДопРасходов.ХранилищеДенег.Валюта = Объект.Валюта Тогда
				
				СтрокаДопРасходов.ХранилищеДенег = ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ОсновноеХранилищеДенег");
				Если НЕ ЗначениеЗаполнено(СтрокаДопРасходов.ХранилищеДенег) Тогда
					СтрокаДопРасходов.ХранилищеДенег = ПредопределенноеЗначение("Справочник.ХранилищаДенег.ОсновнаяКассаВОсновнойВалюте");
				КонецЕсли;		
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаДопРасходов.ХранилищеДенег)
				И НЕ СтрокаДопРасходов.ХранилищеДенег.Валюта = Объект.Валюта Тогда
		...
		СтрокаДопРасходов.Курс	  = Объект.Курс;
		СтрокаДопРасходов.Статья  = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенег.РасходыНаПриобретениеТоваров");
		СтрокаДопРасходов.ОСтроке = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("За товар");
		СтрокаДопРасходов.НеРаспределятьНаСтоимость = ИСТИНА;
...
Процедура ОбновитьКурсСтрок()
	...
	Для Каждого СтрокаДопРасходов из Объект.ДополнительныеРасходы цикл
		Если ЗначениеЗаполнено(СтрокаДопРасходов.Валюта) Тогда
			СтрокаДопРасходов.Курс = ОбщийМодульПовтор.ПолучитьТекущийКурс(СтрокаДопРасходов.Валюта, ?(СтрокаДопРасходов.ДатаРасхода = '00010101000000' , Объект.Дата, СтрокаДопРасходов.ДатаРасхода));
		КонецЕсли;	
	КонецЦикла;
...
Процедура ПересчитатьСуммуПрочихРасходов(Знач БезИзменений = ЛОЖЬ)
	...
	Для Каждого СтрокаДопРасходов из Объект.ДополнительныеРасходы Цикл
		...
		Если НЕ БезИзменений Тогда
			Если СтрокаДопРасходов.Курс = 0 Тогда
				СтрокаДопРасходов.Курс = 1;	
			КонецЕсли;	
		КонецЕсли;
		
		ИтогоРасходов = ИтогоРасходов + СтрокаДопРасходов.Сумма * ?(СтрокаДопРасходов.Курс = 0, 1, СтрокаДопРасходов.Курс);
	КонецЦикла;
...
Процедура ПриИзмененииНоменклатуры(Знач ИзКоличества = ЛОЖЬ, СтрокаТовара = Неопределено, Знач НеМенятьЦену = ЛОЖЬ) 
	...
	Если РегистрироватьВидЦенПриПоступлении Тогда		
		РасчитатьДополнительнуюЦенуДляСтрокиНаКлиенте(СтрокаТовара);
	КонецЕсли;
...
Функция   ПроверитьНаОтказ()
	...
	Если НЕ отказ
		...
		Иначе
			...
			СуммаДенег = 0;
			Для Каждого СтрокаДопРасходов Из Объект.ДополнительныеРасходы Цикл
				СуммаДенег = СтрокаДопРасходов.Сумма * ?(СтрокаДопРасходов.Курс = 0, 1, СтрокаДопРасходов.Курс);
				
				Отказ = ОбщийМодульСервер.ПроверитьОстатокДенежныхСредств(СуммаДенег, 
				?(СтрокаДопРасходов.ДатаРасхода = '00010101000000' , Объект.Дата, СтрокаДопРасходов.ДатаРасхода), ,
				СтрокаДопРасходов.Валюта, СтрокаДопРасходов.ХранилищеДенег, , Объект.Ссылка);
...
Процедура ДополнительныеРасходыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		СтрокаДопРасходов = Элементы.ДополнительныеРасходы.ТекущиеДанные;
		
		Если НЕ СтрокаДопРасходов = Неопределено Тогда			
			СтрокаДопРасходов.Валюта = Объект.Валюта;
			СтрокаДопРасходов.Курс   = ОбщийМодульПовтор.ПолучитьТекущийКурс(СтрокаДопРасходов.Валюта, ?(СтрокаДопРасходов.ДатаРасхода = '00010101000000', Объект.Дата, СтрокаДопРасходов.ДатаРасхода));
		КонецЕсли;		
	КонецЕсли;
...
Процедура ПересчитатьРаспределениеДопРасходов(Знач ТолькоРасчет = ЛОЖЬ)
	...
	Для Каждого СтрокаДопРасходов ИЗ Объект.ДополнительныеРасходы Цикл
		...
		Если НЕ СтрокаДопРасходов.НеРаспределятьНаСтоимость Тогда
			СуммаДополнительныхРасходовКРаспределению = СуммаДополнительныхРасходовКРаспределению + СтрокаДопРасходов.Сумма * СтрокаДопРасходов.Курс;
		КонецЕсли;
			...
			Иначе			
				...
				Если РаспрДопРасхПропорцКоличествуАНеСумме Тогда
					Если НЕ ТовараВКоличестве = 0 тогда
						СуммаДопРасх = СуммаДополнительныхРасходовКРаспределению * (СтрокаТовара.Количество / ?( КоличествоБазыНоменклатурыНаКоторуюРаспотраняетсяРасределениеДопРасходов = 0,  1,  КоличествоБазыНоменклатурыНаКоторуюРаспотраняетсяРасределениеДопРасходов));	 // без едизм
					КонецЕсли;
				...
				ИначеЕсли РаспредДопРасходыПропорцНаЕдНоменклатурыНезависОтСтоимости Тогда // Распределять дополнительные расходы пропорционально на единицу номенклатуры независимо от ее стоимости и количества
					Если НЕ ТовараВКоличестве = 0 тогда
						СуммаДопРасх = СуммаДополнительныхРасходовКРаспределению / ЧислоСтрокТоваровНаКоторыеРаспределяетсяВаловаяПрибыль;	 // без едизм
					КонецЕсли;
					
				Иначе
					Если НЕ СуммаБазыНоменклатурыНаКоторуюРаспотраняетсяРасределениеДопРасходов = 0 Тогда											
						СуммаДопРасх = СуммаДополнительныхРасходовКРаспределению * СуммаТов / СуммаБазыНоменклатурыНаКоторуюРаспотраняетсяРасределениеДопРасходов;
					КонецЕсли;
				КонецЕсли;	
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура РасчитатьДополнительнуюЦенуДляСтрокиНаКлиенте(СтрокаТовара, Знач НеМенятьСуществующие = ЛОЖЬ)
				...
				Если НеДобавлятьРучнойВыбор
					...
					ИначеЕсли Ваучер.ТипВаучера = Перечисления.ТипыВаучера.СкидочнаяКартаПроцент
						...
						Если ОтветПоВаучеру.ВаучерПодходит Тогда
							...
							// процент скидки будет расчитан позже
