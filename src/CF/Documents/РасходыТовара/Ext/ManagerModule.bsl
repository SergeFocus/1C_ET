...
Процедура Печать(ТабДокРезультат, Знач Ссылка, Знач ЭтоПечатьСчета = ЛОЖЬ, Знач НоСчетЕсть = ЛОЖЬ, Знач ЭтоВыполнениеРабот = ЛОЖЬ, Знач ЭтоТорг12 = ЛОЖЬ, Знач ЭтоЗаказНаряд = ЛОЖЬ, Знач ЗаголовокРасходнойНакладной = Неопределено, Знач ДваЭкземпляра = ЛОЖЬ, Знач ВставлятьРазделительСтраниц = ЛОЖЬ, Знач ЭтоСчетФактура = ЛОЖЬ) Экспорт
	...
	ИначеЕсли ЭтоТорг12 Тогда
		...
		ТабДок.ИмяПараметровПечати  = "Торг12" + СокрЛП(ИмяКомпьютера());
		ТабДок.КлючПараметровПечати = ТабДок.ИмяПараметровПечати;
	...
	Иначе	// регламентированный или собственный
		...
		Иначе		
			...
			Иначе  // собственынй встроенный
				
				ТабДок.ИмяПараметровПечати  = "ДокРасходВстроенный" + СокрЛП(ИмяКомпьютера());
				ТабДок.КлючПараметровПечати = ТабДок.ИмяПараметровПечати;
			...
			Пока Выборка.Следующий() Цикл
				...
				ОбщееЧислоМест = 0;
				Валюта = ?(ЗначениеЗаполнено(Выборка.ВидЦен), Выборка.ВидЦен.ВалютаЦены, ?(ЗначениеЗаполнено(Выборка.Валюта), Выборка.Валюта, ВалютаОсн));
				...
				Иначе // собственный
					...
					ДолгКлиента = ОбщийМодульСервер.ПолучитьДолгКлиентаПоставщика(Выборка.КлиентПоставщик, Выборка.Дата -1, , ИСТИНА, валютаосн, ЯзыкДокументов);
	...
	ТабДок.Вывести(ОбластьПечатиЭкземпляра);
