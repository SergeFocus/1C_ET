...
Процедура ЗаписатьНаСервере()
	...
	ЭтотОбъект = ДанныеФормыВзначение(Объект, Тип("ДокументОбъект.РасходыТовара"));
	ЭтотОбъект.Записать();
...
Процедура ОбработатьБлокировку(Знач ПриСозданииФормы = ЛОЖЬ);
	...
	Иначе
		...
		Иначе
			...
			Элементы.ТоварыВводСКоличеством1.Видимость 		= ИСТИНА;
			...
			Элементы.Товары1ПодборНоменклатуры.Видимость 		= ИСТИНА;
			Элементы.ДобавитьНовогоКлиента1.Видимость 			= ИСТИНА;
...
Процедура ПечатьРН(ТабДок, Знач ПараметрКоманды, Знач ЭтоСчет, Знач НоСчетЕсть = ЛОЖЬ, Знач ВДваЭкземпляра = ЛОЖЬ, Знач ЗаголовокРасходнойНакладной = "", Знач ВставлятьРазделительСтраниц = ЛОЖЬ)  
	...
	Документы.РасходыТовара.Печать(ТабДок, ПараметрКоманды, ЭтоСчет, НоСчетЕсть, , , , ЗаголовокРасходнойНакладной, ВДваЭкземпляра, ВставлятьРазделительСтраниц);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		ОтветственныйЗаДоставкуВидимость 			= ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("УчитыватьИнформациюОДоставкеВДокументахРасхода") И ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетЗарплатыСотрудников");
...
Функция   УкажитеВидДокументаПриИзмененииНаСервере(Знач УкажитеВидДокументаБК)
	...
	Если УкажитеВидДокументаБК = Перечисления.ВидыДокументаРасходаТовара.ВозвратПоставщику Тогда  		
		Возврат ПредопределенноеЗначение("Справочник.Поставщики.ФизическоеЛицо");
	ИначеЕсли УкажитеВидДокументаБК = Перечисления.ВидыДокументаРасходаТовара.Реализация Тогда
		Возврат ПредопределенноеЗначение("Справочник.Клиенты.ФизическоеЛицо");
	Иначе
		Возврат ПредопределенноеЗначение("Справочник.Клиенты.ПустаяСсылка");
	КонецЕсли;
