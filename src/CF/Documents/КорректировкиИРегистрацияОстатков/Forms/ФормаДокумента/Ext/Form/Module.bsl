...
Процедура СвернутьТаблицуТоваров(Команда)  
	...
	Если НЕ ДокументЗаблокирован
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СвернутьТаблицуТоваровЗавершение", ЭтаФорма), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Свернуть таблицу товаров?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
