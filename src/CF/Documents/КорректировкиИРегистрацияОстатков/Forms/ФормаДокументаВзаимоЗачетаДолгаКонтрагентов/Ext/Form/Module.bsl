// sza160308-0236 курсовые разницы
// sza150619-0025
// sza150110-0118 НЧ
// sza140701-1645
// sza140601-2203
...
Процедура ОбработатьБлокировку(Знач ПриСозданииФормы = ЛОЖЬ)
	...
	Если ОбщийМодульСервер.ОбработатьБлокировку(Объект, ЭтаФорма, ПриСозданииФормы, "000658") Тогда
		...
		Элементы.ФормаСписаниеКурсовыхРазницПоРасчетамВВалюте.Видимость = ЛОЖЬ;
	Иначе
		...
		Элементы.ФормаСписаниеКурсовыхРазницПоРасчетамВВалюте.Видимость = ИСТИНА;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		Элементы.Взаиморасчеты1.Видимость 	= ВестиУчетВалют;
...
&НаКлиенте
Процедура СписаниеКурсовыхРазницПоРасчетамВВалюте(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("СписаниеКурсовыхРазницПоРасчетамВВалютеЗавершение", ЭтотОбъект), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Заполнить таблицы за счет курсовой разницы по расчетам в валюте?"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры

&НаКлиенте
Процедура СписаниеКурсовыхРазницПоРасчетамВВалютеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт

	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		СписаниеКурсовыхРазницПоРасчетамВВалютеНаСервере();
		Объект.Комментарий = Объект.Комментарий + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Списание курсовой разницы по расчетам в валюте");
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура СписаниеКурсовыхРазницПоРасчетамВВалютеНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	-ЕСТЬNULL(РасчетыОстатки.СуммаОстаток, 0) КАК Сумма,
		|	-ЕСТЬNULL(РасчетыВВалюте.СуммаОстаток, 0) КАК СуммаВВалюте
		|ИЗ РегистрНакопления.Расчеты.Остатки(&Дата, ) КАК РасчетыОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			СУММА(РасчетыВВалютеОстатки.СуммаОстаток) КАК СуммаОстаток,
		|			РасчетыВВалютеОстатки.КлиентПоставщик КАК КлиентПоставщик,
		|			РасчетыВВалютеОстатки.Договор КАК Договор
		|		ИЗ РегистрНакопления.РасчетыВВалюте.Остатки(&Дата, ) КАК РасчетыВВалютеОстатки
		|		СГРУППИРОВАТЬ ПО
		|			РасчетыВВалютеОстатки.КлиентПоставщик,
		|			РасчетыВВалютеОстатки.Договор) КАК РасчетыВВалюте
		|		ПО (РасчетыВВалюте.КлиентПоставщик = РасчетыОстатки.Клиент)
		|			И (РасчетыВВалюте.Договор = РасчетыОстатки.Договор)
		|ГДЕ (ЕСТЬNULL(РасчетыОстатки.СуммаОстаток, 0) = 0
		|			И ЕСТЬNULL(РасчетыВВалюте.СуммаОстаток, 0) <> 0) ИЛИ (ЕСТЬNULL(РасчетыОстатки.СуммаОстаток, 0) <> 0
		|			И ЕСТЬNULL(РасчетыВВалюте.СуммаОстаток, 0) = 0)";
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
		|	РасчетыОстатки.Поставщик,
		|	РасчетыОстатки.Договор,
		|	-ЕСТЬNULL(РасчетыОстатки.СуммаОстаток, 0) КАК Сумма,
		|	-ЕСТЬNULL(РасчетыВВалюте.СуммаОстаток, 0) КАК СуммаВВалюте
		|ИЗ РегистрНакопления.РасчетыСПоставщиками.Остатки(&Дата, ) КАК РасчетыОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			СУММА(РасчетыВВалютеОстатки.СуммаОстаток) КАК СуммаОстаток,
		|			РасчетыВВалютеОстатки.КлиентПоставщик КАК КлиентПоставщик,
		|			РасчетыВВалютеОстатки.Договор КАК Договор
		...
		|			РасчетыВВалютеОстатки.КлиентПоставщик,
		|			РасчетыВВалютеОстатки.Договор) КАК РасчетыВВалюте
		|		ПО (РасчетыВВалюте.КлиентПоставщик = РасчетыОстатки.Поставщик)
		|			И (РасчетыВВалюте.Договор = РасчетыОстатки.Договор)
		|ГДЕ (ЕСТЬNULL(РасчетыОстатки.СуммаОстаток, 0) = 0
		|			И ЕСТЬNULL(РасчетыВВалюте.СуммаОстаток, 0) <> 0) ИЛИ (ЕСТЬNULL(РасчетыОстатки.СуммаОстаток, 0) <> 0
		|			И ЕСТЬNULL(РасчетыВВалюте.СуммаОстаток, 0) = 0)";
