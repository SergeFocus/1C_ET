...
Функция   ОбработатьПолученныйШКНаСервере(ТекКод, Количество = 1)
	...
	СтруктураРезультат	= ОбщийМодульТоварСервер.ПолучитьНоменклатуруПоШтрихКоду(ТекКод, Истина);
	...
	Если ЗначениеЗаполнено(номенклатура) Тогда
		...
		Если ИспользоватьСложныйМеханизмЦен  тогда
			...
			СтруктураПараметров.Вставить("Цена", ОбщийМодульСервер.ПолучитьСложнуюЦену(Номенклатура, Объект.ВидЦен, Объект.Дата));
		иначе
...
Процедура ПриИзмененииНоменклатуры(СтрокаТовара = Неопределено) 
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		СтрокаТовара.Цена 	= ЦенаНаСервере(Номенклатура);
...
&НаСервере
Функция   ЦенаНаСервере(Номенклатура)	
	Возврат ПолучитьЦенуНаСервере(Номенклатура);		
...
Функция   ПолучитьЦенуНаСервере(Номенклатура)
	...
	Если ИспользоватьСложныйМеханизмЦен  тогда
		Возврат ОбщийМодульСервер.ПолучитьСложнуюЦену(Номенклатура, Объект.ВидЦен, Объект.Дата);
	иначе
...
Процедура ЗаполнитьУчетнымиДаннымиНаСервере()
	...
	если не РезультатЗапроса.Пустой() тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			строкатовары.Количество 	= ВыборкаДетальныеЗаписи.КоличествоОстаток;
			строкатовары.цена 			= ПолучитьЦенуНаСервере(строкатовары.Номенклатура);
...
Функция   обработатьотказдвижениядлянабора(Отказ, Номенклатура, КоличествоВх, Сумма = 0, ПоведениеПрограммыПриРасходеТоваровНижеРекомендованногоМинимальногоОстатка, ПоведениеПрограммыПриРасходеТоваровБезОстатка) //для вложенных наборов
