...
Функция   ОбработатьОтказДвиженияДляНабора(Отказ, Знач Номенклатура, Знач КоличествоСтруктура, Знач Сумма = 0, Знач ПоведениеПрограммыПриРасходеТоваровНижеРекомендованногоМинимальногоОстатка, Знач ПоведениеПрограммыПриРасходеТоваровБезОстатка) // для вложенных наборов
	...
	Для Каждого СтрокаСоставаНабора Из Номенклатура.Состав Цикл
		...
		Количество = КоличествоСтруктура * СтрокаСоставаНабора.Количество;
		СуммаЭлемента = 0; // СуммаЭлемента при перемещении пренебречь

		Если ОбщийМодульПовтор.ЭтоНабор(НоменклатураСостава) Тогда
			ОбработатьОтказДвиженияДляНабора(Отказ, НоменклатураСостава, КоличествоСтруктура * СтрокаСоставаНабора.количество, СуммаЭлемента, ПоведениеПрограммыПриРасходеТоваровНижеРекомендованногоМинимальногоОстатка, ПоведениеПрограммыПриРасходеТоваровБезОстатка);
		Иначе
...
Функция   ОбработатьПолученныйШтрихкодНаСервере(Знач ТекКод, Количество = 1)
	...
	СтруктураРезультат	= ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(ТекКод, ИСТИНА, , Объект.Дата);
	...
	Если НЕ ЗначениеЗаполнено(номенклатура)
		...
		Если ЗначениеЗаполнено(Ваучер) Тогда
			Номенклатура = Ваучер.Номенклатура;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПечатьБезСерий(ДокументДляПечати, Знач ПараметрКоманды)
	...
	Документы.ПеремещенияТовара.ПечатьБезСерий(ДокументДляПечати, ПараметрКоманды);
...
Процедура ПриОткрытии(Отказ)                                         // ПРИ ОТКРЫТИИ
	...
	ТоварПеремещенБезЗадержки = НЕ ЗапретитьТоварВПути
	И (НЕ Объект.Дата = Объект.ДатаПрибытия);
	Элементы.ГруппаСкладВПути.Видимость = ТоварПеремещенБезЗадержки;
	Элементы.Перемещал.Видимость = ТоварПеремещенБезЗадержки;
...
Процедура СменитьВидОтображенияДокумента(Знач ПриОткрытии = ЛОЖЬ)
	...
	Если ПредставитьДокументДвумяКолонками Тогда
		Элементы.Переместить(Элементы.ФормаГруппаГлобальных, Элементы.ГруппаСлеваАльтернативногоВида);
		...
		Элементы.Переместить(Элементы.ГруппаКомандШапки, Элементы.ГруппаСкладОткуда);
		...
		Элементы.Переместить(Элементы.ГруппаКомментария, Элементы.ГруппаСлеваАльтернативногоВида);
		...
		Элементы.Переместить(Элементы.ГруппаПодписи, Элементы.ГруппаСлеваАльтернативногоВида);
	...
	ИначеЕсли НЕ ПриОткрытии Тогда
		Элементы.Переместить(Элементы.ФормаГруппаГлобальных, Элементы.ГруппаОбычногоВида);
		...
		Элементы.Переместить(Элементы.ГруппаПодписи, Элементы.ГруппаСлева);
		...
		Элементы.Переместить(Элементы.ГруппаКомментария, Элементы.ГруппаОбычногоВида);
