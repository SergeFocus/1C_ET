...
Процедура ВидЦенПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтарыйВидЦен = Объект.ВидЦен Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ВидЦенПриИзмененииЗавершениеЗавершение", ЭтаФорма), "Обновить цены?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ВидЦенПриИзмененииЗавершениеЗавершение(РезультатВопроса, ДополнительныеПараметры1) Экспорт
...
Процедура ВывестиИнформациюОЦенахВДругомВидеЦен(Команда)
	ПоказатьВводЗначения(Новый ОписаниеОповещения("ВывестиИнформациюОЦенахВДругомВидеЦенЗавершение", ЭтаФорма, Новый Структура("ИнформационныйВидЦен", ИнформационныйВидЦен)), ИнформационныйВидЦен, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Укажите вид цен для информационной колонки") + ": ", Тип("СправочникСсылка.ВидыЦен"));
КонецПроцедуры

&НаКлиенте
Процедура ВывестиИнформациюОЦенахВДругомВидеЦенЗавершение(Значение, ДополнительныеПараметры) Экспорт

	ИнформационныйВидЦен = ?(Значение = Неопределено, ДополнительныеПараметры.ИнформационныйВидЦен, Значение);

	Если (Значение <> Неопределено) Тогда
		Если ЗначениеЗаполнено(ИнформационныйВидЦен) Тогда
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если НЕ Объект.Товары.Количество() = 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ДатаПриИзмененииЗавершение1", ЭтаФорма), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Пересчитать цены на эту дату?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
&НаКлиенте
Процедура ДатаПриИзмененииЗавершение1(РезультатВопроса, ДополнительныеПараметры) Экспорт

	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПроверитьЗависимостьВидаЦен();
		ПроверитьВидЦенЗависимый(Новый ОписаниеОповещения("ДатаПриИзмененииЗавершение", ЭтаФорма));
	КонецЕсли;
...
Процедура ЗагрузитьШтрихКодКоличество(Знач Адрес)
	...
	Пока НЕ ПустаяСтрока(СтрокаФайла) Цикл
		...
		СтруктураПоШК = ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(СтрокаФайла, ИСТИНА, ИСТИНА, Объект.Дата);
...
Процедура ЗаписатьНаСервере()
	...
	ЭтотОбъект1 = ДанныеФормыВзначение(Объект, Тип("ДокументОбъект.УстановкиЦен"));
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если Объект.Товары.НайтиСтроки(СтруктураОтбора).Количество() = 0 Тогда
				...
				ПересчитатьСтрокуНаСервере(НоваяСтрока, Количество);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьТаблицуНаборамиИРассчитатьИмЦены(Команда)
	ЗаполнитьТаблицуНаборамиИРассчитатьИмЦеныНаСервере();
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТаблицуНаборамиИРассчитатьИмЦеныНаСервере()
...
&НаКлиенте
Процедура ОбновитьИнформационныйВидЦен(СтрокаТовара, Знач ТолькоРазница = ЛОЖЬ)
...
Функция   ОбработатьПолученныйШтрихкодНаСервере(Знач ТекКод, Знач Количество = 1)
	...
	Результат = ИСТИНА;
	РезультатОбработки = ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(ТекКод, , , Объект.Дата);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		ДляЦенПоставщика = Параметры.ДляЦенПоставщика;

		Если ДляЦенПоставщика Тогда
			КодДопуска = "00065D";
		Иначе
			КодДопуска = "002350";
		КонецЕсли;

		Отказ = ОбщийМодульСервисСервер.ПроверитьОтказДоступа(КодДопуска, ЭтаФорма, Отказ, Объект);

		Если НЕ Отказ Тогда
			Элементы.ГруппаПоставщика.Видимость 	 = ДляЦенПоставщика;
			...
			Если НЕ ЗначениеЗаполнено(ОбъектСсылка) Тогда
				...
				Объект.ДляЦенПоставщика = ДляЦенПоставщика;
			КонецЕсли;
...
Процедура ПроверитьВидЦенЗависимый(Знач Оповещение)
	...
	Если ЭтоЗависимаяЦена Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ПроверитьВидЦенЗависимыйЗавершение", ЭтаФорма, Новый Структура("Оповещение", Оповещение)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Этот Вид цен - Зависимый.") + символы.ПС + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Рекомендуется устанавливать только базовые.") + символы.ПС + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Все равно использовать?"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПроверитьЗависимостьВидаЦен()
	...
	Если ЗначениеЗаполнено(Объект.ВидЦен) Тогда
		...
		Элементы.ВидЦенФормулаРасчетаОтБазовогоВидаЦены.Видимость = НЕ ИсточниковЗависимостиМожетБытьНесколько
		И ЗначениеЗаполнено(Объект.ВидЦен.ФормулаРасчетаОтБазовогоВидаЦены);

		Элементы.ВидЦенПроцентОтБазовойЦены.Видимость = НЕ ИсточниковЗависимостиМожетБытьНесколько
		И ЗначениеЗаполнено(Объект.ВидЦен.ПроцентОтБазовойЦены);

		Элементы.ВидЦенОсновнойВидЦен.Видимость = НЕ ИсточниковЗависимостиМожетБытьНесколько
		И ЗначениеЗаполнено(Объект.ВидЦен.ОсновнойВидЦен);
...
Процедура ТоварыНоменклатураОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	ОбщийМодульКлиент.ПоискОшибкиПриВводеТекстаПользователем("Номенклатура", ДанныеВыбора, Текст, Элементы.Товары.ТекущиеДанные.Номенклатура);
...
&НаСервере
Процедура УстановитьВсемОднуЦену(Знач НоваяЦена)

	Для Каждого СтрокаЦен Из Объект.Товары Цикл
		СтрокаЦен.Цена = НоваяЦена;
		СтрокаЦен.РазницаЦены = СтрокаЦен.Цена - СтрокаЦен.СтараяЦена;
	КонецЦикла;
...
Процедура УстановитьНовуюЦенуПоСтарой(Команда)
	...
	Коэффициент = 1;
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьНовуюЦенуПоСтаройЗавершение", ЭтаФорма, Новый Структура("Коэффициент", Коэффициент)), Коэффициент, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Введите коэффициент для новой цены относительно старой (1 - равенство)?"), 12, 4);

КонецПроцедуры

&НаКлиенте
Процедура УстановитьНовуюЦенуПоСтаройЗавершение(Число, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура УстановитьОднуЦенуДляВсехСтрок(Команда)

	НоваяЦена = 0;
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьОднуЦенуДляВсехСтрокЗавершение", ЭтаФорма, Новый Структура("НоваяЦена", НоваяЦена)), НоваяЦена, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Введите общую цену для всех строк?"), 15, 2);

КонецПроцедуры

&НаКлиенте
Процедура УстановитьОднуЦенуДляВсехСтрокЗавершение(Число, ДополнительныеПараметры) Экспорт

	НоваяЦена = ?(Число = Неопределено, ДополнительныеПараметры.Коэффициент, Число);

	Если НЕ Число = Неопределено Тогда
		Состояние(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Обработка и заполнение таблицы товаров.."));
		УстановитьВсемОднуЦену(НоваяЦена);
	КонецЕсли;
