// sza150703-1528 выб серии с ост
...
Процедура ОбновитьРезультатыНаСервере()
	...
	Для Каждого СтрокаТовара Из Объект.Товары Цикл
		СтрокаТовара.ВаловаяПрибыль 	= ОбщийМодульСервер.РасчитатьОриентировочнуюВаловуюПрибыль(СтрокаТовара.номенклатура, СтрокаТовара.Количество, СтрокаТовара.Дата, СтрокаТовара.Сумма, СтрокаТовара.Валюта, СтрокаТовара.Курс, СтрокаТовара.СерияНоменклатуры);
...
Процедура ОбработатьПолученныйШКНаклиенте(Знач ТекКод)	
	...
	РезультатНакопление = ОбработатьПолученныйШКНаСервере(ТекКод);
...
Процедура ПечатьРН(ТабДок, Знач ПараметрКоманды, Знач ЭтоСчет, Знач НоСчетЕсть = ЛОЖЬ, Знач ВДваЭкземпляра = ЛОЖЬ, Знач ЗаголовокРасходнойНакладной = "", Знач ВставлятьРазделительСтраниц = ЛОЖЬ, Знач ЭтоСчетФактура = ЛОЖЬ)  
	...
	Документы.РасходыТовара.Печать(ТабДок, ПараметрКоманды, ЭтоСчет, НоСчетЕсть, ИСТИНА , , , ЗаголовокРасходнойНакладной, ВДваЭкземпляра, ВставлятьРазделительСтраниц, ЭтоСчетФактура);
