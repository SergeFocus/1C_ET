...
Процедура ОбновитьЗначениеНаправленияДеятельности()
	Объект.НаправлениеДеятельности = ОбщийМодульСервер.НайтиНаправлениеДеятельности(Объект.Клиент, Объект.Договор, Объект.Организация, Объект.Номенклатура, Объект.СкладДляОбъектаВыполненияРабот, Объект.Сотрудник);
КонецПроцедуры
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		Если ОбщийМодульПовтор.ПолучитьПараметрСеанса("ВестиУчетОтдельныхДоговоровСКонтрагентамиПС") Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(Объект.КлиентПоставщик) Тогда
				Объект.Договор = ОбщийМодульСервер.НайтиДоговорПоПараметрам(Объект.Организация, Объект.Клиент, Объект.Дата, Объект.ВидЦен);
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Процедура ПечатьГарантийногоТалонаНаСервере(ДокументДляПечати, Знач ПараметрыПечати, Знач НеОбнулятьТаблицу = ЛОЖЬ)
	...
	ОбщийМодульТоварСервер.ПечатьГарантийногоТалона(ДокументДляПечати, ПараметрыПечати, НеОбнулятьТаблицу);
...
Процедура ПечатьНалоговойНакладной(Команда)
	...
	Если НЕ ПереченьЭлементов = Неопределено Тогда
		...
		ПечатьГарантийногоТалонаНаСервере(ФормаПечати.Результат, ПараметрыПечати, ИСТИНА);
...
Процедура ПриИзмененииКонтрагента()
	...
	Если  ЗначениеЗаполнено(Объект.Клиент) Тогда
		...
		Если ОбщийМодульПовтор.ПолучитьПараметрСеанса("ВестиУчетОтдельныхДоговоровСКонтрагентамиПС")
			...
			Объект.Договор = ОбщийМодульСервер.НайтиДоговорПоПараметрам(Объект.Организация, Объект.Клиент, Объект.Дата);
		КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбъектСсылка) Тогда
			...
			Если ЗначениеЗаполнено(Параметры.КлиентПоставщик) Тогда
				Объект.Клиент = Параметры.КлиентПоставщик;
			КонецЕсли;
...
КонецФункции // проверить на отказ
...
КонецФункции // проверить учет по сериям
...
Процедура РаботыНоменклатураОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	СтрокаТовара = Элементы.Товары.ТекущиеДанные;
	ОбщийМодульКлиент.ПоискОшибкиПриВводеТекстаПользователем("Номенклатура", ДанныеВыбора, Текст, СтрокаТовара.Номенклатура, Объект.Клиент);
...
&НаКлиенте
Процедура КомплектностьПриИзменении(Элемент)
