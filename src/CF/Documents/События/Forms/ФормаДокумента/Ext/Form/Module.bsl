...
Процедура ИзменитьВидимостьПараметровСобытияНаСервере()
	...
	ЭтотОбъект1.ЗаполнитьПоВидуСобытий();
	ЗначениеВРеквизитФормы(ЭтотОбъект1, "Объект");
...
Процедура РазрешитьСменитьВидСобытия(Команда)
	...
	Если глВерсияПлатформы < 803040000  Тогда
		МожноРедактировать = Вопрос(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы уверены, что можете на этом этапе сменить вид события?"), РежимДиалогаВопрос.ДаНет, , , ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Смена вида сохраненного события")) = КодВозвратаДиалога.Да;
...
Процедура СобственныеРасходыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		СтрокаДопРасходов = Элементы.СобственныеРасходы.ТекущиеДанные;
...
Процедура ПересчитатьСуммуПрочихРасходов(Знач БезИзменений = ЛОЖЬ)
	...
	Для Каждого СтрокаДопРасходов из Объект.СобственныеРасходы Цикл
...
Процедура СоздатьДокументыРеализацииКлиентамНаСервере()
	...
	Если НЕ ЧислоДокументов = 0 Тогда
		Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Создано документов") + ": " + СокрЛП(ЧислоДокументов) + ". " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Их следует открыть и заполнить."));
	КонецЕсли;
...
Процедура КлиентОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	ОбщийМодульКлиент.ПоискОшибкиКодировки("Клиенты", ДанныеВыбора, Текст, Объект.Клиент);
КонецПроцедуры

&НаКлиенте
Процедура ПоставщикОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	ОбщийМодульКлиент.ПоискОшибкиКодировки("Поставщики", ДанныеВыбора, Текст, Объект.Поставщик);
КонецПроцедуры
