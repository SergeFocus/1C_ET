...
Функция РазложитьСтрокуВМассив(Знач Стр, Знач Разделитель = ",") Экспорт
	...
	Если Разделитель = " " Тогда
		...
		Пока 1=1 Цикл
			...
			Если Поз=0 Тогда
				СтрЭль = Справочники.ЗначенияЯчеекТаблицДанных.НайтиПоНаименованию(стр);
				Если НЕ ЗначениеЗаполнено(СтрЭль) Тогда
					СтрЭльО = Справочники.ЗначенияЯчеекТаблицДанных.СоздатьЭлемент();
					СтрЭльО.Наименование = Стр;
					СтрЭльО.Записать();
					СтрЭль = СтрЭльО.ссылка;
				КонецЕсли;
				МассивСтрок.Добавить(СтрЭль);
				
				Возврат МассивСтрок;
				
			КонецЕсли;
			стр1 = Лев(Стр,Поз-1);
			СтрЭль = Справочники.ЗначенияЯчеекТаблицДанных.НайтиПоНаименованию(стр1);
			Если НЕ ЗначениеЗаполнено(СтрЭль) Тогда
				СтрЭльО = Справочники.ЗначенияЯчеекТаблицДанных.СоздатьЭлемент();
				СтрЭльО.Наименование = Стр1;
				СтрЭльО.Записать();
				СтрЭль = СтрЭльО.ссылка;
			КонецЕсли;
			МассивСтрок.Добавить(СтрЭль);
