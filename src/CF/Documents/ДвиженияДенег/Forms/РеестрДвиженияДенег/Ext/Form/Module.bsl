		...
		Если НЕ СтрокаДенег.НельзяРедактироватьВТаблице Тогда
			...
			Если НЕ ДокументДвиженияДенег = Неопределено
				...
				Исключение 	
					...
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("В строке") + " №" + СчетчикСтрок + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("произошла ошибка") + ": " + ТекстОписаниеОшибки;
					Сообщение.Сообщить();
