// sza151011-0523 глЕдинственнаяФормаЗакрыта
// sza150603-0053
// sza140619-1559
// sza140619-1318
...
Процедура ЗаписатьВсеДвиженияНаСервере()
	...
	Для Каждого СтрокаДенег Из РеестрДвиженияДенег Цикл
		...
		Если НЕ СтрокаДенег.НельзяРедактироватьВТаблице Тогда
			...
			Если НЕ ДокументДвиженияДенег = Неопределено
				И НЕ СтрокаДенег.Сумма = 0 Тогда
				...
				Исключение
					...
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("В строке") + " №" + СчетчикСтрок + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("произошла ошибка") + ": " + ТекстОписаниеОшибки);
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если глФормаЗаданаСтрого
		ИЛИ (НЕ НетНеЗаписанных()
...
Процедура ПрочитатьЗаПериодКлиент(Знач БезВыбораПериода = ЛОЖЬ)
	...
	Если НетНеЗаписанных() Тогда
		ПрочитатьЗаПериодКлиентФрагмент(БезВыбораПериода);
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("ПрочитатьЗаПериодКлиентЗавершение", ЭтаФорма, Новый Структура("БезВыбораПериода", БезВыбораПериода)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Отменить все изменения?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ПрочитатьЗаПериодКлиентЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт

	БезВыбораПериода = ДополнительныеПараметры.БезВыбораПериода;
...
&НаКлиенте
Процедура ПрочитатьЗаПериодКлиентФрагмент(Знач БезВыбораПериода)
...
Процедура ФормаОплатыПриИзменении(Элемент)
	ХранилищеДенегПриИзмененииНаСервере(ИСТИНА);
КонецПроцедуры
...
&НаСервере
Процедура ХранилищеДенегПриИзмененииНаСервере(Знач ИзФормыОплаты = ЛОЖЬ)

	Если НЕ ИзФормыОплаты
		И ВестиУчетДенегВНесколькихХранилищах
		И ЗначениеЗаполнено(ХранилищеДенег) Тогда
...
&НаКлиенте
Процедура РеестрДвиженияДенегПриИзменении(Элемент)
