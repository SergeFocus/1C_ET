...
Процедура НаПечать(ДокументДляПечати, Ссылка) Экспорт
	...
	ДокументДляПечати.ИмяПараметровПечати  = "НоменклатурнаяГруппа" + СокрЛП(ИмяКомпьютера());
	...
	Иначе		
		...
		Запрос.Текст = "ВЫБРАТЬ НоменклатурныеГруппы.Ссылка,
		               ...
		               |	ВЫРАЗИТЬ(НоменклатурныеГруппы.Комментарий КАК СТРОКА(1000)) КАК Комментарий,
		...
		Пока Выборка.Следующий() Цикл
			...
			ЕСЛИ НЕ ВыборкаРеквизитыНоменклатуры.Количество() = 0 Тогда
				...
				ДокументДляПечати.Вывести(ОбластьРеквизитыНоменклатурыШапка);
				...
				Пока ВыборкаРеквизитыНоменклатуры.Следующий() Цикл
					...
					ДокументДляПечати.Вывести(ОбластьРеквизитыНоменклатуры, ВыборкаРеквизитыНоменклатуры.Уровень());
				КонецЦикла;	
			КонецЕсли;
