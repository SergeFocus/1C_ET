...
Функция   ОбработатьПолученныйШКНаКлиенте(ТекКод)
	...
	Если КодЭлемента <> Неопределено Тогда
		...
		ОткрытьФорму("Справочник.ПозицииДляОказанияУслугЗаВремяНомера.Форма.ФормаЭлемента", ПараметрыОткрытия);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Отказ = ОбщийМодульСервисСервер.ПроверитьОтказДоступа("001300", ЭтаФорма, Отказ, );
