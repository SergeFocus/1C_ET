	...
	Если НЕ Отказ Тогда
		...
		Если НЕ ЗначениеЗаполненоОбъектСсылка Тогда
			...
			Для Каждого ЭлементВалюта Из СписокВалют Цикл
				Элементы.ВыбратьВалютуМира.СписокВыбора.Добавить(ЭлементВалюта.Код, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке(ЭлементВалюта.Наименование));	
			КонецЦикла;
		Иначе
...
Процедура РасчитатьВсеЗависимыеЦены(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если НЕ ОТказ 
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("РасчитатьВсеЗависимыеЦеныЗавершение", ЭтаФорма), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Есть Зависимые цены по этой валюте.") + символы.пс + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Рассчитать все зависимые цены по этой валюте?"), РежимДиалогаВопрос.ДаНет);
