// sza160223-2157 составное наименование
// sza140522-0113
// sza140424-1529
...
Процедура Наименование1ПриИзменении(Элемент)
	...
	ПриИзмененииНаименованияГруппы();

КонецПроцедуры

&НаКлиенте
Процедура ПриИзмененииНаименованияГруппы()
...
Процедура СформироватьАвтоматически(Команда)
	...
	Для Каждого СтрокаТовара Из Объект.Товары Цикл
		СоставноеНаименование = ?(ПустаяСтрока(СоставноеНаименование), "" + СтрокаТовара.Номенклатура, ", " + СтрокаТовара.Номенклатура);
	КонецЦикла;

	Если НЕ ПустаяСтрока(СоставноеНаименование) Тогда
		Наименование = СоставноеНаименование;
		ПриИзмененииНаименованияГруппы();
	КонецЕсли;
