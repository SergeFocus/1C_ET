...
Процедура ОбновитьДанныеОДолгеНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ВестиУчетВалют Тогда
			...
			|ИЗ РегистрНакопления.РасчетыВВалюте.ОстаткиИОбороты(&ДатаНачала, &ДатаОкончания, , , КлиентПоставщик = &Поставщик) КАК РасчетыВВалютеОстаткиИОбороты";
			Запрос.УстановитьПараметр("ДатаНачала", '00010101000000');
			Запрос.УстановитьПараметр("ДатаОкончания", Дата);
			Запрос.УстановитьПараметр("Поставщик", Объект.Ссылка);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	СвязанныеСобытия.Параметры.УстановитьЗначениеПараметра("Поставщик", Объект.Ссылка);
	...
	Элементы.СтраницаЗаказовПоставщику.Видимость = ОбщийМодульСервер.УКонтрагентаЕстьРасчетыИлиЗаказы(Объект.Ссылка, "ЗаказыПоставщика");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		ОбъектРодитель = Объект.Родитель; // может быть задан для нового объекта
		...
		Иначе
			...
			Элементы.СтраницаДокументыПоставщика.Видимость = УКонтрагентаЕстьРасчеты;
		...
		СвязанныеСобытия.Параметры.УстановитьЗначениеПараметра("Поставщик", ОбъектСсылка);
		ВестиУчетОтдельныхДоговоровСКонтрагентамиПС = ПараметрыСеанса.ВестиУчетОтдельныхДоговоровСКонтрагентамиПС;

		Элементы.ДоговораПоставщика.Видимость 	= ВестиУчетОтдельныхДоговоровСКонтрагентамиПС
		И НЕ ОбщийМодульСервер.ЧислоДоговоровСКонтрагентом(ОбъектСсылка) = 0;

		Элементы.ЗаказыПоставщику.Видимость 	= ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетЗаказовПоставщикам");

		Элементы.СтраницаЗаказовПоставщику.Видимость = ЗначениеЗаполненоОбъектСсылка
		И ОбщийМодульСервер.УКонтрагентаЕстьРасчетыИлиЗаказы(ОбъектСсылка, "ЗаказыПоставщика");

		Элементы.СоздатьНовыйДоговор.Видимость  = ВестиУчетОтдельныхДоговоровСКонтрагентамиПС
		И ЗначениеЗаполнено(ОбъектСсылка);
...
Процедура СоздатьНовыйЗаказ(Команда)
	...
	ОткрытьФорму("Документ.ПоступленияТовара.ФормаОбъекта", ПараметрыДляФормы);
...
Процедура Взаиморасчеты(Команда)
	...
	ПараметрыОтчета = Новый Структура("ОтборПоПоставщику", Объект.Ссылка);
