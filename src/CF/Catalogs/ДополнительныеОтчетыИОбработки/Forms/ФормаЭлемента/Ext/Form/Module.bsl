...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ Отказ Тогда
	Элементы.ФормаВыпонить.Видимость = ЗначениеЗаполнено(Объект.Ссылка);	
																		КонецЕсли;
...
Функция   ЗарегистрироватьОбработку(ПараметрыРегистрации)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) И Вид <> Объект.Вид Тогда
...
Функция   ОткрытьДиалогДляЗагрузкиФайлаОбработкиОтчета(ФлагДобавить, ПараметрыРегистрации = Неопределено)
	...
	Иначе 
		...
		"2. " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Отключить конфликтующие - продолжить загрузку, отключив Публикацию отчетов (обработок), конфликтующих с загружамой по имени.") + Символы.ПС +
		"3. " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Показать конфликтующие - отменить загрузку и показать список отчетов (обработок), конфликтующих с загружамой по имени.") + Символы.ПС +
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Элементы.ФормаВыпонить.Видимость = ИСТИНА;
