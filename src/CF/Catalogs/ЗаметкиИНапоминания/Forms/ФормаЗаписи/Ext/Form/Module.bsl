...
Процедура ВыполняемыеЗадачиАдресОткрытие(Элемент, СтандартнаяОбработка)
	...
	ДиалогВыбФайла.Показать(Новый ОписаниеОповещения("ВыполняемыеЗадачиАдресОткрытиеЗавершение", ЭтаФорма, Новый Структура("ДиалогВыбФайла, ТекущиеДанные", ДиалогВыбФайла, ТекущиеДанные)));
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ Отказ
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Дата в прошлом") + ": " + формат(Запись.Дата, "ДФ='dd.MM.yy ЧЧ:мм'") + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Задайте дату в будущем.");
		Сообщение.Поле 	= "Дата";
		Сообщение.УстановитьДанные(Запись);
		Сообщение.Сообщить();
