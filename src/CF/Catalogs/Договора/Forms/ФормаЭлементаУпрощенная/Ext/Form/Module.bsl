...
Процедура АвтоНаменованиеДоговораНаСервере() 
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.ПредусматриваетСтрогиеЦены) Тогда
...
Процедура ЗаполнитьПоВидуЦенНаСервере(ВидЦен)
	...
	Для Каждого СтрокаТовара Из Объект.Товары Цикл
		СтрокаТовара.Цена = ОбщийМодульСервер.ПолучитьЦенуНаСервере(СтрокаТовара.Номенклатура, ВидЦен, Дата, ИСТИНА, , , , , Объект.Ссылка);
	КонецЦикла;
