...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Сотрудник)
		И ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетРабочегоВремениСотрудников") Тогда
		
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сотрудник обязательно должен быть указан"), Объект.Ссылка, "Сотрудник", , Отказ);
	КонецЕсли;
