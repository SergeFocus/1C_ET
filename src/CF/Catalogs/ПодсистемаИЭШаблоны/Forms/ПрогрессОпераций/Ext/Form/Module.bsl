...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ВремяОбработки" И Параметр Тогда
		...
		ВремяОбработки = СокрЛП(ВремяОбработкиЧасть) + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("с.");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если ЗначениеЗаполнено(Параметры.ДанныеПрогресса) Тогда
		...
		Если Параметры.ДанныеПрогресса.НачальноеОкно ТОгда
			...
			ЭтаФорма.Заголовок = ЭтаФорма.Заголовок + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("продолжается..");
		...
		Иначе
			...
			Элементы.ВремяПроизводства.Видимость = Параметры.ДанныеПрогресса.РассчитыватьСрокОкончанияОбмена;
			...
			ВремяОбработки = "" + Параметры.ДанныеПрогресса.ВремяОбработки + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("с.");
