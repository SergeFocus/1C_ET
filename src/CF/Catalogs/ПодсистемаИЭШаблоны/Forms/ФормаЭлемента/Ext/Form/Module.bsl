...
Функция   БыстроСоздатьНовоеПолеНаСервере(НаименованиеПоля, ТипПоля = Неопределено)
	...
	Исключение // Записи НовоеПоле
		...
		Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи справочника") + ": " + НовоеПоле + " - " + ТекстОписаниеОшибки;
...
Процедура ВыгрузитьЭтотШаблонВФайл(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		// ДиалогВыбФайла.ПолноеИмяФайла = ПодсистемаИЭ.КонстантыОтносительныйАдресФайловПолучить() + "ieUNFSH0.DBf";
...
Процедура ЗаполнитьПоРеквизитамКоманда(Команда)
	...
	Если ЗначениеЗаполнено(ТипОбъектаОбмена) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРеквизитамКомандаЗавершение", ЭтаФорма, Новый Структура("ТипМета, ТипОбъектаОбмена", ТипМета, ТипОбъектаОбмена)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оценить таблицу объекта") + " " + ТипОбъектаОбмена + "? (НЕТ - сам объект)", РежимДиалогаВопрос.ДаНет);
...
Процедура ЗаполнитьПоРеквизитамОбъектаБазыДанных(Команда)
	...
	Если ЗначениеЗаполнено(ТипОбъектаОбмена) Тогда
		...
		ТипОбъектаОбмена = Прав(ТипОбъектаОбмена, стрДлина(ТипОбъектаОбмена) - 1);
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРеквизитамОбъектаБазыДанныхЗавершение", ЭтаФорма, Новый Структура("ТипМета, ТипОбъектаОбмена", ТипМета, ТипОбъектаОбмена)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оценить таблицу объекта") + " " + ТипОбъектаОбмена + "? (НЕТ - сам объект)", РежимДиалогаВопрос.ДаНет);
...
Процедура ОценитьОбъектБазыДанных(Команда)
	...
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("ОценитьОбъектБазыДанныхЗавершение2", ЭтаФорма), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы готовы очистить поля шаблона?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОценитьОбъектБазыДанныхЗавершение2(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ОценитьОбъектБазыДанныхФрагмент()
	...
	ПараметрыФормы = Новый Структура("ТипОбъектаОбмена","");
		...
		Если ЗначениеЗаполнено(ТипОбъектаОбмена) Тогда
			...
			ТипОбъектаОбмена = Прав(ТипОбъектаОбмена, стрДлина(ТипОбъектаОбмена) - 1);
			ПоказатьВопрос(Новый ОписаниеОповещения("ОценитьОбъектБазыДанныхЗавершение1", ЭтаФорма, Новый Структура("ТипМета, ТипОбъектаОбмена", ТипМета, ТипОбъектаОбмена)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оценить таблицу объекта") + " " + ТипОбъектаОбмена + "? (НЕТ - сам объект)", РежимДиалогаВопрос.ДаНет);
		КонецЕсли;	
	
КонецПроцедуры

&НаКлиенте
Процедура ОценитьОбъектБазыДанныхЗавершение1(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыборкаОбъектов = Неопределено Тогда
		...
		Для Каждого ПолеВФайле Из Объект.ПоследовательностьПолейВФайле Цикл
			...
			Исключение 	
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка обращения к результату запроса поля") + ": " + ИмяПоляВФайле + " - " + ТекстОписаниеОшибки;
				Сообщение.УстановитьДанные(Объект);
				Сообщение.Сообщить();
