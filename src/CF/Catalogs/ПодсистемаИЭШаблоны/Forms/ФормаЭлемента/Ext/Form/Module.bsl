...
Процедура АдресФайлаБезВыбораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураДиалогФильтр = ПодсистемаИЭ.СтруктураДиалогФильтр(Объект);
	...
	ДиалогВыбораФайла1.ПолноеИмяФайла 	= ?(НЕ Объект.АдресФайла = "", Объект.АдресФайла, ПодсистемаИЭ.КонстантыОтносительныйАдресФайловПолучить()); // АДРЕС
...
Процедура БыстроСоздатьНовоеПоле(Команда)
	...
	НаименованиеПоля = "";
	ПоказатьВводСтроки(Новый ОписаниеОповещения("БыстроСоздатьНовоеПолеЗавершение", ЭтаФорма, Новый Структура("НаименованиеПоля", НаименованиеПоля)), НаименованиеПоля, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Укажите наименование нового поля") + ": ", 100);

КонецПроцедуры

&НаКлиенте
Процедура БыстроСоздатьНовоеПолеЗавершение(Строка, ДополнительныеПараметры) Экспорт

	НаименованиеПоля = ?(Строка = Неопределено, ДополнительныеПараметры.НаименованиеПоля, Строка);
...
Функция   БыстроСоздатьНовоеПолеНаСервере(НаименованиеПоля, ТипПоля = Неопределено)
	...
	НовоеПоле = Справочники.ПодсистемаИЭПоля.СоздатьЭлемент();
	...
	Исключение // Записи НовоеПоле
		...
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи справочника") + ": " + НовоеПоле + " - " + ТекстОписаниеОшибки, Объект);
...
Процедура ВидШаблонаПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ВидШаблона = ПредопределенноеЗначение("Перечисление.ПодсистемаИЭИмпортЭкспортФайловВидыШаблонов.ДляСпискаЭлементовСправочникаИлиДокументов") Тогда
		Объект.ИмяТаблицыШаблонаФайла = "";
	КонецЕсли;
...
Процедура ВыгрузитьЭтотШаблонВФайл(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ДиалогДляВыбораФайла.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выберите файл для экспорта шаблона") + ": ";
		// ДиалогДляВыбораФайла.ПолноеИмяФайла = ПодсистемаИЭ.КонстантыОтносительныйАдресФайловПолучить() + "ieUNFSH0.DBf";
		...
		Если ДиалогДляВыбораФайла.Выбрать() Тогда
			...
			ПодсистемаИЭ.ВыполнитьВыгрузкуШаблоновНаСервере(АдресФайла, Объект.Ссылка);
...
Процедура ВыполнитьВыгрузку(Команда)
	...
	Иначе
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Объект не записан!"));
	КонецЕсли;
...
Процедура ЗаполнитьПоРеквизитамКоманда(Команда)
	...
	ФормаВыбораТипа = ПолучитьФорму("Справочник.ПодсистемаИЭШаблоны.Форма.ФормаВыбораТипаОбъектаБазыДанных", ПараметрыФормы);
	...
	Если ЗначениеЗаполнено(ТипОбъектаОбмена) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРеквизитамКомандаЗавершение", ЭтаФорма, Новый Структура("ТипМета, ТипОбъектаОбмена", ТипМета, ТипОбъектаОбмена)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оценить таблицу объекта") + " " + ТипОбъектаОбмена + "? (" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("НЕТ - сам объект") + ")", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоРеквизитамКомандаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт

	ТипМета = ДополнительныеПараметры.ТипМета;
	ТипОбъектаОбмена = ДополнительныеПараметры.ТипОбъектаОбмена;
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПараметрыФормы = Новый Структура("ТипОбъектаОбмена", ТипОбъектаОбмена);
		ПараметрыФормы.Вставить("ТипМета", ТипМета);
		ФормаВыбораТипа = ПолучитьФорму("Справочник.ПодсистемаИЭШаблоны.Форма.ФормаВыбораТабличнойЧасти", ПараметрыФормы);
		ФормаВыбораТипа.ОткрытьМодально();
...
Процедура ЗаполнитьПоРеквизитамОбъектаБазыДанных(Команда)
	...
	Если ЗначениеЗаполнено(ТипОбъектаОбмена) Тогда
		...
		ТипОбъектаОбмена = Прав(ТипОбъектаОбмена, стрДлина(ТипОбъектаОбмена) - 1);
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРеквизитамОбъектаБазыДанныхЗавершение", ЭтаФорма, Новый Структура("ТипМета, ТипОбъектаОбмена", ТипМета, ТипОбъектаОбмена)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оценить таблицу объекта") + " " + ТипОбъектаОбмена + "? (" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("НЕТ - сам объект") + ")", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоРеквизитамОбъектаБазыДанныхЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьШаблонНаСервере(АдресФайла, ТипОценки)
	...
	ПодсистемаИЭ.ПроизвестиОценкуФайла(ОбъектШаблон, АдресФайла, ТипОценки);
...
Процедура ОценитьОбъектБазыДанных(Команда)
	...
	Если Объект.ПоследовательностьПолейВФайле.Количество() = 0 Тогда
		ОценитьОбъектБазыДанныхФрагмент();
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("ОценитьОбъектБазыДанныхЗавершениеДопУчасток", ЭтаФорма), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы готовы очистить поля шаблона?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОценитьОбъектБазыДанныхЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт

	ТаблицаОбъектаОбмена = ДополнительныеПараметры.ТаблицаОбъектаОбмена;
	ТипМета = ДополнительныеПараметры.ТипМета;
	ТипОбъектаОбмена = ДополнительныеПараметры.ТипОбъектаОбмена;
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
...
&НаКлиенте
Процедура ОценитьОбъектБазыДанныхЗавершениеДопУчасток(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОценитьОбъектБазыДанныхЗавершениеУчасток(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ОценитьОбъектБазыДанныхЗавершение", ЭтаФорма, Новый Структура("ТаблицаОбъектаОбмена, ТипМета, ТипОбъектаОбмена", ТаблицаОбъектаОбмена, ТипМета, ТипОбъектаОбмена)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Создавать поля шаблона, согласно данных реквизитов объекта базы данных?"), РежимДиалогаВопрос.ДаНет);

КонецПроцедуры

&НаКлиенте
Процедура ОценитьОбъектБазыДанныхФрагмент()
	...
	Если ЗначениеЗаполнено(ТипОбъектаОбмена) Тогда
		...
		ТипОбъектаОбмена = Прав(ТипОбъектаОбмена, СтрДлина(ТипОбъектаОбмена) - 1);
		ПоказатьВопрос(Новый ОписаниеОповещения("ОценитьОбъектБазыДанныхЗавершениеУчасток", ЭтаФорма, Новый Структура("ТипМета, ТипОбъектаОбмена", ТипМета, ТипОбъектаОбмена)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Оценить таблицу объекта") + " " + ТипОбъектаОбмена + "? (" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("НЕТ - сам объект") + ")", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ОценитьОбъектНаСервере(ТипМета, ТипОбъектаОбмена, ТаблицаОбъектаОбмена, СоздаватьПоляШаблона, РежимЗаполнения = 0)
	...
	ОбъектШаблон = РеквизитФормыВЗначение("Объект");
	ПодсистемаИЭ.ПроизвестиОценкуОбъектаБазыДанных(ОбъектШаблон, ТипМета, ТипОбъектаОбмена, ТаблицаОбъектаОбмена, СоздаватьПоляШаблона, РежимЗаполнения);
	ЗначениеВРеквизитФормы(ОбъектШаблон, "Объект");
...
&НаКлиенте
Процедура ОценитьФайлПоТипу(Знач ТипОценки)

	Если Объект.ПоследовательностьПолейВФайле.Количество() = 0 Тогда
		ОценитьФайлПоТипуФрагмент(ТипОценки);
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("ОценитьФайлПоТипуЗавершение", ЭтаФорма, Новый Структура("ТипОценки", ТипОценки)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы готовы удалить существующую таблицу полей шаблона?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОценитьФайлПоТипуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт

	ТипОценки = ДополнительныеПараметры.ТипОценки;
...
&НаКлиенте
Процедура ОценитьФайлПоТипуФрагмент(Знач ТипОценки)
	...
	ДиалогВыбораФайла1.ПолноеИмяФайла 	= ПодсистемаИЭ.константыПодсистемаИЭПолучить(); // АДРЕС
	ДиалогВыбораФайла1.Фильтр  			= ДиалогФильтр;
	ДиалогВыбораФайла1.Расширение  		= ДиалогРасширение;
	ДиалогВыбораФайла1.МножественныйВыбор 		= ЛОЖЬ;
	ДиалогВыбораФайла1.ПредварительныйПросмотр 	= ИСТИНА;
...
Процедура ОчиститьТаблицуОтбора(Команда)
	...
	Если Объект.ТолькоДляОбъектовСРеквизитами.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьТаблицуОтбораЗавершение", ЭтаФорма), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы готовы очистить таблицу отбора по реквизитам?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОчиститьТаблицуОтбораЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ОчиститьТаблицуРеквизитов(Команда)
	...
	Если Объект.ДополнительноеЗаполнениеРеквизитов.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьТаблицуРеквизитовЗавершение", ЭтаФорма), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы готовы очистить таблицу дополнительных реквизитов?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОчиститьТаблицуРеквизитовЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Функция   ПолеПропуска()
	...
	Возврат Справочники.ПодсистемаИЭПоля.ПропуститьДанноеПоле;
...
Процедура ПриИзмененииПоля(Обязательно = ЛОЖЬ)
	...
	Если Обязательно
		...
		Если Объект.ФорматФайла = ПредопределенноеЗначение("Перечисление.ПодсистемаИЭИмпортЭкспортФорматыФайлов.DBF") Тогда
			Элементы.ПоследовательностьПолейВФайле.ТекущиеДанные.ИмяПоляВФайле = ПодсистемаИЭ.ПеревестиСтрокуВЛатинскиеСимволы(ИзменитьИмяПоляВФайле(Элементы.ПоследовательностьПолейВФайле.ТекущиеДанные.ПолеТаблицы));
		Иначе
...
Процедура СоздатьПоляДляПроизвольноЗапросаНаСервере(Режим = 0)
	...
	Если НЕ ВзДС.Пустой() Тогда
		Если Режим = 1 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Было создано") + ": " + СчетчикПолей + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("полей в последовательности."), Объект);
		Иначе
	...
	Если НЕ ВыборкаОбъектов = Неопределено Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Всего в результате запроса") + ": " + ВыборкаОбъектов.Количество() + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("записей. Выполнен за") + " " + разницаВремени + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("сек."), Объект);
		...
		Для Каждого ПолеВФайле Из Объект.ПоследовательностьПолейВФайле Цикл
			...
			Исключение
				ТекстОписаниеОшибки = ОписаниеОшибки();
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка обращения к результату запроса поля") + ": " + ИмяПоляВФайле + " - " + ТекстОписаниеОшибки, Объект);
			КонецПопытки;

		КонецЦикла;
	ИначеЕсли Режим = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Нет результата запроса!"), Объект);
	КонецЕсли;
...
Процедура УсловияФильтраНомерПоляПриИзменении(Элемент)
	...
	Исключение
		ТекстОписаниеОшибки = ОписаниеОшибки();
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка") + ": " + ТекстОписаниеОшибки, Объект, "ИмяПоля");
	КонецПопытки;
...
Процедура УстановитьДоступностьПрогрессПроизводстваОбмена()
	...
	Если Объект.ПоказыватьПрогрессПроизводстваОбмена Тогда
		Элементы.РассчитыватьСрокОкончанияОбмена.Доступность = ИСТИНА;
		Элементы.ПоказыватьОбъектыОбмена.Доступность = ИСТИНА;
	Иначе
		Элементы.РассчитыватьСрокОкончанияОбмена.Доступность = ЛОЖЬ;
...
&НаКлиенте
Процедура ПоследовательностьПолейВФайлеПриИзменении(Элемент)
