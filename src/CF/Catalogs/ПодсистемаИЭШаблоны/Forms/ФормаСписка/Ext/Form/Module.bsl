...
// 		ШаблонФайла.НаправлениеДеятельности = Справочники.НаправленияДеятельности.НайтиПоНаименованию(СокрЛП(БазаДБФБазаШаблонов.NAPRV));
			...
			Исключение 
				...
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Устаревший формат обмена шаблонов") + ": " + ТекстОписаниеОшибки;
				Сообщение.Сообщить();
			КонецПопытки;
...
Процедура ЗагрузитьШаблоныИзФайла(Команда)
	...
	ДиалогВыбФайла.ПолноеИмяФайла 	= ПодсистемаИЭ.КонстантыОтносительныйАдресФайловПолучить() + "ieUNFSH0.DBf"; // АДРЕС
