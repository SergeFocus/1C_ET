...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Функция   ПолучитьСписокОборудования(ТипыПО = Неопределено, Идентификатор = Неопределено, РабочееМесто = Неопределено) Экспорт
		...
		Пока Выборка.Следующий() Цикл
			ОбъектМетаданных = Выборка.ТипОборудования.Метаданные();
			ИндексЗначения = Перечисления[ОбъектМетаданных.Имя].Индекс(Выборка.ТипОборудования);
			ТипОборудованияИмя = ОбъектМетаданных.ЗначенияПеречисления.Получить(ИндексЗначения).Имя;
			...
			ДанныеУстройства.Вставить("ТипОборудованияИмя"     , ТипОборудованияИмя);   
...
Процедура НаПечать(ДокументДляПечати, Ссылка) Экспорт
	...
	|	ПодключаемоеОборудование.ИдентификаторУстройства,
	|	ВЫРАЗИТЬ(ПодключаемоеОборудование.Комментарий КАК СТРОКА(1000)) КАК Комментарий,
	...
	|ГДЕ ПодключаемоеОборудование.Ссылка В(&Ссылка)";
	...
	ДокументДляПечати.ИмяПараметровПечати  = "Оборудование" + СокрЛП(ИмяКомпьютера());
