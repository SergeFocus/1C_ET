...
Функция   ОбновитьДанныеОбОбщемКоличествеНаСервере(Знач ИспользоватьСложныйМеханизмЦен, Знач ВидЦен, Знач ОтобратьОстаткиПоСкладу, Знач ВестиУчетПоСкладам)
	Возврат ОбщийМодульТоварСервер.ПолучитьСтруктураОстаткаТоваров(ИспользоватьСложныйМеханизмЦен, ВидЦен, ОтобратьОстаткиПоСкладу, ВестиУчетПоСкладам);	
КонецФункции
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		Если СложныйНабор Тогда // с остатками по набору
			...
			|			ТОГДА ЕСТЬNULL(ВложенныйЗапрос.Цена, 0)
			|		ИНАЧЕ СправочникНоменклатура.Цена
			|	КОНЕЦ * ЕСТЬNULL(ТоварыОстатки.КоличествоОстаток, 0) - ЕСТЬNULL(ТоварыОстатки.СуммаОстаток, 0) КАК ДобавочнаяСтоимость,
