...
Процедура НазначитьЭтуНоменклатурнуюГруппуВсемПодчиненным(Команда)
	...
	Если Вопрос(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Назначить номенклатурную группу всем подчиненным элементма?"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
