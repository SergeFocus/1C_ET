...
Процедура ДобавитьДопШтрихКодФрагмент()
	...
	Перем НеобходимаяФорма, ПараметрыФормы, ФормаШтрихКода;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПараметрыФормы = Новый Структура("Номенклатура", Объект.Ссылка);
		НеобходимаяФорма = "РегистрСведений.ДополнительныеШтрихКоды.Форма.ФормаЗаписи";
		ФормаШтрихКода = ПолучитьФОрму(НеобходимаяФорма, ПараметрыФормы);
...
Процедура ДобавитьИзображениеФрагмент()
	...
	Перем НеобходимаяФорма, ПараметрыФормы, формаИзображения;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПараметрыФормы = Новый Структура("СвязанныйОбъект", Объект.Ссылка);
		НеобходимаяФорма = "Справочник.Изображения.Форма.ФормаЭлемента";
		формаИзображения = ПолучитьФорму(НеобходимаяФорма, ПараметрыФормы);	
...
Процедура ДобавитьСпецификациюФрагмент()
	...
	Перем ИмяНеобходимойФормы, ПараметрыФормы, ФормаСпецификации;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПараметрыФормы = Новый Структура("Номенклатура", Объект.Ссылка);
		ИмяНеобходимойФормы = "Справочник.Спецификации.Форма.ФормаЭлемента";
		ФормаСпецификации = ПолучитьФорму(ИмяНеобходимойФормы, ПараметрыФормы);
...
Процедура ДобавитьШтрихКодФрагмент()
	...
	Перем ПараметрыФормы, ФормаШтрихКода;
...
Процедура КоличествоНаборовКотороеМожноСобратьНаТекущийМоментНаСервере(Знач ТабДок)
	...
	ТабДок.ИмяПараметровПечати  = "КоличествоНаборов" + СокрЛП(ИмяКомпьютера());
	ТабДок.КлючПараметровПечати = ТабДок.ИмяПараметровПечати;
...
Функция  НаименованиеПриИзмененииНаСервере(Знач ОбъектНаименование, Знач ОбъектРодитель)
	...
	ГруппаПоПервомуСлову = ОбщийМодульТоварСервер.НаименованиеГруппыПоПервомуСлову(ОбъектНаименование, ИСТИНА);
...
Процедура ОбработатьПолученныйШКНаклиенте(Знач ТекКод)
	...
	Иначе 
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбработатьПолученныйШКНаклиентеЗавершение", ЭтаФорма, Новый Структура("ТекКод", ТекКод)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Установить Штрих-Код основным для этой номенклатуры?"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ПредставлениеНаименованияПриИзменении(Элемент)
	...
	Если СоздаватьГруппыНоменклПоПервомуСловуНаименИОтноситьКНим тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)          // ПРИ СОЗДАНИИ НА СЕРВЕРЕ
	...
	Если НЕ Отказ Тогда
		...
		СоздаватьГруппыНоменклПоПервомуСловуНаименИОтноситьКНим = ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("СоздаватьГруппыНоменклПоПервомуСловуНаименИОтноситьКНим");
		...
		Элементы.ОбновитьШтрихКодПоФормуле.Видимость 	= ИспользоватьШтрихКоды;// И 
		//(ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьФормулыАвтоНаименованияФормированияШтрихКодаИТекстаСШтрихКодом") 
		//ИЛИ ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ФормироватьШтрихКодБезФормулыПоФорматуEAN13")
		//ИЛИ ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ФормироватьШтрихКодБезФормулыПоФорматуCode128"));
...
Процедура ДобавитьПредставлениеКонтрагентаФрагмент()
	...
	Перем ИмяНеобходимойФормы, ПараметрыФормы, формаИзображения;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПараметрыФормы = Новый Структура("Номенклатура", Объект.Ссылка);
		ИмяНеобходимойФормы = "РегистрСведений.ПредставлениеТоваровКонтрагентами.Форма.ФормаЗаписи";
		формаИзображения = ПолучитьФорму(ИмяНеобходимойФормы, ПараметрыФормы);	
		формаИзображения.Открыть();
	КонецЕсли;
...
Процедура ДобавитьЕдиницуИзмеренияФрагмент()
	...
	Перем ИмяНеобходимойФормы, ПараметрыФормы, ФормаЕдИзм;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПараметрыФормы = Новый Структура("Номенклатура", Объект.Ссылка);
		ИмяНеобходимойФормы = "Справочник.ЕдиницыИзмерения.Форма.ФормаЭлемента";
		ФормаЕдИзм = ПолучитьФорму(ИмяНеобходимойФормы, ПараметрыФормы);
		ФормаЕдИзм.Открыть();	
	КонецЕсли;
