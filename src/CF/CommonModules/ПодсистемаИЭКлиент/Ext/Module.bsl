	...
	Если НЕ ШаблонФайла = Неопределено Тогда
		...
		Если НЕ СокрЛП(АдресФайла)="" Тогда	
			...
			Если ПодсистемаИЭ.ШаблонФайлаПоказыватьПрогрессПроизводстваОбмена(ШаблонФайла) Тогда		
				...
				ПараметрыФормыПрогресса = ПодсистемаИЭ.СоздатьСтруктуруФормыПрогресса("Прогресс " + ?(СтруктураИмпортаЭкспорта.Экспортер, "экспорт", "импорт") + "а " + СокрЛП(ШаблонФайла), ШаблонФайла);
