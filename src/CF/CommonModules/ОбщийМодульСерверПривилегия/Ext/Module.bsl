...
// sza140710-1050 попытка на удаление реквизита
// sza140709-1756
// sza140707-1402  !
// sza140627-1742
// sza140624-1547
...
Процедура ВыполнитьЗадачку(Знач Напоминание) Экспорт
	...
	Для Каждого ПодЗадача Из Напоминание.ВыполняемыеЗадачи Цикл
		...
		ИнформацияРуководителю 				  = (ТипЗадачи = Перечисления.ТипыЗадач.СформироватьОтчетИОтправитьПоEmail) ИЛИ (ТипЗадачи = Перечисления.ТипыЗадач.СформироватьОтчетИСохранитьВФайлExcel);
		...
		Попытка
			...
			ИначеЕсли СформироватьОтчетИОтправитьПоEmail
				...
				Если ИнформацияРуководителю Тогда
					...
					ДокументДляПечати.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ОТЧЕТРУК" + СокрЛП(ИмяКомпьютера());
					...
					ОбщийМодульСервер.ПечатьНаСервереОтчетаРуководителю(СтруктураОтчета, ДокументДляПечати, СформироватьОтчетИОтправитьПоEmail);

					Если СформироватьОтчетИСохранитьВФайлExcel Тогда
						ПараметрыЗадания = Новый Массив;
						ПараметрыЗадания.Добавить(ДокументДляПечати);
				...
				Иначе
					...
					ДокументДляПечати = Новый ТабличныйДокумент;
					ДокументДляПечати.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ОТЧЕТ" + СокрЛП(ИмяКомпьютера());
					ДокументДляПечати.КлючПараметровПечати = ДокументДляПечати.ИмяПараметровПечати;
					...
					ОбщийМодульСервер.СформироватьПроизвольныйОтчетНаСервере(ТаблицаПараметров, ДокументДляПечати, ПодЗадача.НаименованиеОтчета, ПодЗадача.КодВариантОтчета);
			...
			ИначеЕсли резервирование Тогда
				...
				Если НЕ АдресКаталогБазыДанных = Неопределено Тогда
					...
					РезервироватьБазуДанных(ПараметрыЗадания);
...
Процедура ЗарегистрироватьИзмененияОбъектаВРегистр(Знач ОбъектБазыДанных, Знач ТекущийПользователь, Знач ДатаСобытия, Знач ТаблицаИзменений) Экспорт
	...
	Для Каждого СтрокаИзменений Из ТаблицаИзменений Цикл
		...
		РегистрВерсий
		.ОбъектБазыДанных 	= ОбъектБазыДанных.Ссылка;
...
Процедура ОбновитьДанныеПриОбновленииКонфигурации(Знач ВерсияБыла, Знач ВерсияКонфигурации) Экспорт
	...
	Попытка
		...
		// sza151223-1918
		ТекРегКод = СокрЛП(Константы.РегистрационныйНомер.Получить());
		РегистрационныйКодКУстановке = СокрЛП(ОбщийМодульСервисСервер.РегистрационныйКодКонфигурации(ИСТИНА));

		Если (ПустаяСтрока(ТекРегКод)
			ИЛИ СтрДлина(ТекРегКод) <= 6
			ИЛИ ТекРегКод = "15011401"
			ИЛИ НЕ Найти(РегистрационныйКодКУстановке, "!") = 0)
			И СтрДлина(РегистрационныйКодКУстановке) > 6 Тогда

			ТекРегКод = РегистрационныйКодКУстановке;
			Константы.РегистрационныйНомер.Установить(РегистрационныйКодКУстановке);
		КонецЕсли;
		...
		Если ВерсияБыла < 151022 Тогда
			...
			Кириллица = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Кириллица");
			...
			ЭлементСправочника.Алфавит = Кириллица;
			ЭлементСправочника.Записать();
		...
		Если ВерсияБыла < 140617 Тогда
			СтатьяДвиженияДенег = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенег.ВнутреннееПеремещениеДенег").ПолучитьОбъект();
			СтатьяДвиженияДенег.НеВключаетсяВЧистаяПрибыль = ИСТИНА;
			СтатьяДвиженияДенег.Записать();
			СтатьяДвиженияДенег = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенег.ДополнительныеРасходыПриПриобретенииТоваров").ПолучитьОбъект();
			СтатьяДвиженияДенег.НеВключаетсяВЧистаяПрибыль = ИСТИНА;
			СтатьяДвиженияДенег.Записать();
			...
			Если НЕ ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетДенегВНесколькихХранилищах")
				...
				Пока Валюты.Следующий() Цикл
					Если НЕ Валюты.Предопределенный Тогда
						...
						Если РезультатЗапроса.Пустой() Тогда
							...
							НовоеХранилищеДенег.ФормаОплаты = ФормаОплаты;
				...
				Если НЕ РезультатЗапроса.Пустой() Тогда
					...
					Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
						...
						РегистраторОбъект.ХранилищеДенег = ОбщийМодульПовтор.НайтиХранилищеДенегПоВалюте(Регистратор.Валюта, РегистраторОбъект.ХранилищеДенег);
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					Для Каждого СтрокаТ Из Документ.Товары Цикл
						...
						Если НЕ СтрокаТ.Количество = 0
							...
							СтрокаТ.ВаловаяПрибыль = ОбщийМодульСервер.РассчитатьОриентировочнуюВаловуюПрибыль(СтрокаТ.номенклатура, СтрокаТ.Количество, СтрокаТ.Дата, СтрокаТ.Сумма, СтрокаТ.Валюта, СтрокаТ.Курс, СтрокаТ.СерияНоменклатуры);
						Иначе
							СтрокаТ.ВаловаяПрибыль = ОбщийМодульСервер.РассчитатьОриентировочнуюВаловуюПрибыль(СтрокаТ.номенклатура, СтрокаТ.КоличествоПлан, СтрокаТ.ДатаПлан, СтрокаТ.План, СтрокаТ.ВалютаПлан, СтрокаТ.КурсПлан);
						КонецЕсли;
		...
		Если ВерсияБыла < 160308 // расчеты в валюте
			И ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетВалют") Тогда
			...
			|ГДЕ Расчеты.Клиент.ОсновнаяВалюта <> &ОсновнаяВалюта
			|	И Расчеты.Клиент.ОсновнаяВалюта <> &ПустаяВалюта
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|ГДЕ РасчетыСПоставщиками.Поставщик.ОсновнаяВалюта <> &ОсновнаяВалюта
			|	И РасчетыСПоставщиками.Поставщик.ОсновнаяВалюта <> &ПустаяВалюта
			|УПОРЯДОЧИТЬ ПО Дата";
			Запрос.УстановитьПараметр("ОсновнаяВалюта", ПредопределенноеЗначение("Справочник.Валюты.ОсновнаяВалюта"));
			Запрос.УстановитьПараметр("ПустаяВалюта", ПредопределенноеЗначение("Справочник.Валюты.ПустаяСсылка"));
		...
		Если ВерсияБыла < 160331 Тогда
			Константы.ИспользоватьПростуюПродажуИлиРеестром.Установить(ИСТИНА);
			Константы.ЧислоМинутПослеПоследнейОперацииКоторыеСчитаютсяРабочими.Установить(15);
		КонецЕсли;
		...
		Если ВерсияБыла < 140418 Тогда
			...
			Пока ПользователиСправочник.Следующий() Цикл
				Пользователь = ПользователиСправочник.Ссылка.ПолучитьОбъект();
		...
		Если ВерсияБыла < 150527
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					РегистрШтрихКода = РегистрыСведений.ШтрихКоды.СоздатьНаборЗаписей();
					РегистрШтрихКода.Отбор.Номенклатура.Значение = ВыборкаДетальныеЗаписи.Владелец;
					РегистрШтрихКода.Отбор.СерияНоменклатуры.Значение = ПредопределенноеЗначение("Справочник.СерииНоменклатуры.ПустаяСсылка");
					РегистрШтрихКода.Прочитать();

					Для Каждого СТрокаНабора Из РегистрШтрихКода Цикл
						СТрокаНабора.СерияНоменклатуры = ВыборкаДетальныеЗаписи.Ссылка;
					КонецЦикла;

					РегистрШтрихКода.Записать(ИСТИНА);
		...
		Если ВерсияБыла < 160101 Тогда
			ПолнотекстовыйПоиск.ОбновитьИндекс(ИСТИНА, ЛОЖЬ);
		КонецЕсли;

		Если ВерсияБыла < 160214 Тогда
			СписокВалют = Новый ТаблицаЗначений;
			СписокВалют.Колонки.Добавить("Код");
			СписокВалют.Колонки.Добавить("Наименование");
			ОбщийМодульСервер.ПолучитьСписокВалют(СписокВалют);

			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ Валюты.Ссылка
			|ИЗ Справочник.Валюты КАК Валюты
			|ГДЕ Валюты.Код = &Код";

			Для Каждого ЭлементВалюта Из СписокВалют Цикл
				Запрос.УстановитьПараметр("Код", ЭлементВалюта.Код);

				РезультатЗапроса = Запрос.Выполнить();
				ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					ВалютаОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
					ВалютаОбъект.МеждународныйКод = ЭлементВалюта.Наименование;
		...
		Если ВерсияБыла < 160223 Тогда
			Константы.ОбводитьРамкамиТекстовыеПоляВФормах.Установить(ЛОЖЬ);
		КонецЕсли;

	Исключение
		ТекстОписаниеОшибки = ОписаниеОшибки();
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при обновлении данных конфигурации") + "! " + ТекстОписаниеОшибки);
	КонецПопытки;
...
Процедура ОбновитьНаименованияПравПользователей(Знач ВНезависимостиОтЯзыка)

	Если ВНезависимостиОтЯзыка
		ИЛИ Константы.ПоддерживатьИныеЯзыкиКромеРусского.Получить() Тогда

		ПользователиБазыДанных = Справочники.Пользователи.Выбрать();

		Пока ПользователиБазыДанных.Следующий() Цикл
			ПользовательОбъект = ПользователиБазыДанных.Ссылка.ПолучитьОбъект();
			ЕстьИзменения = ЛОЖЬ;
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		Если ЗначениеЗаполнено(Пользователь.ПриЗапускеОткрывать)  Тогда
			Если Пользователь.ПриЗапускеОткрывать = Перечисления.ВариантыОкнаПриЗапускеДляПользователей.ЗапускатьДополнительнуюОбработку Тогда
				...
				КонецЕсли;
			ИначеЕсли Пользователь.ПриЗапускеОткрывать = Перечисления.ВариантыОкнаПриЗапускеДляПользователей.ОбычныеОперациии Тогда
				Результат.Форма = "Обработка.ОбычныеОперации.Форма";
				Результат.Модально = Пользователь.ЗапускатьМодально;
			ИначеЕсли Пользователь.ПриЗапускеОткрывать = Перечисления.ВариантыОкнаПриЗапускеДляПользователей.ОкноПростойПродажи Тогда
				Результат.Форма = "Обработка.ОбычныеОперации.Форма.ПростаяПродажа";
				Результат.Модально = Пользователь.ЗапускатьМодально;
			ИначеЕсли Пользователь.ПриЗапускеОткрывать = Перечисления.ВариантыОкнаПриЗапускеДляПользователей.РеестрДвиженияДенег Тогда
				Результат.Форма = "Обработка.ОбычныеОперации.Форма.РеестрПродаж";
				Результат.Модально = Пользователь.ЗапускатьМодально;
...
Функция   ОпределитьПользователя(Знач ТекущийПользователь = Неопределено) Экспорт
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ОпределенныйПользователь) Тогда
			...
			ЭтоТолькоРеализатор = ЛОЖЬ;

			Попытка
				Выполнить (" ЭтоТолькоРеализатор = НЕ (РольДоступна(""ПолныеПрава"") И НЕ РольДоступна(""АдминистраторСистемы"") И НЕ РольДоступна(""ОператорФункции"") И НЕ РольДоступна(""ОператорКассы"") И НЕ РольДоступна(""Казначей"") И (РольДоступна(""Реализация"") ИЛИ РольДоступна(""РеализацияИПриемТовара""));");
			Исключение // не полная версия
				ЭтоТолькоРеализатор = НЕ РольДоступна("ПолныеПрава");
			КонецПопытки;

			Если ЭтоТолькоРеализатор Тогда
				ОпределенныйПользовательОбъект.ПриЗапускеОткрывать = Перечисления.ВариантыОкнаПриЗапускеДляПользователей.ОбычныеОперациии;
			КонецЕсли;
	...
	Для Каждого ДокументКПерепроведению Из СписокДокументовДляПерепроведения Цикл
		...
		Если НеОстанавливатьсяПриОшибкеПроведения Тогда
			...
			Исключение
				...
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Документ не проведен") + ": " + РежимЗаписиДокумента + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("из-за ошибки") + " " + ОписаниеОшибкиТекст);
			КонецПопытки;
		Иначе
			ОбъектДокумент.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
		КонецЕсли;
...
Функция   ПолучитьСекретыПользователя(Знач ДанныеЭтогоПользователя) Экспорт
	...
	СтруктураДанныхЭтогоПользователя = Новый Структура;
	СтруктураДанныхЭтогоПользователя.Вставить("АутентификацияОС", ДанныеЭтогоПользователя.АутентификацияОС);
	СтруктураДанныхЭтогоПользователя.Вставить("АутентификацияOpenID", ДанныеЭтогоПользователя.АутентификацияOpenID);
	СтруктураДанныхЭтогоПользователя.Вставить("АутентификацияСтандартная", ДанныеЭтогоПользователя.АутентификацияСтандартная);
	СтруктураДанныхЭтогоПользователя.Вставить("ЗапрещеноИзменятьПароль", ДанныеЭтогоПользователя.ЗапрещеноИзменятьПароль);
	СтруктураДанныхЭтогоПользователя.Вставить("ОсновнойИнтерфейс", СокрЛП(ДанныеЭтогоПользователя.ОсновнойИнтерфейс));
	СтруктураДанныхЭтогоПользователя.Вставить("ПарольУстановлен", ДанныеЭтогоПользователя.ПарольУстановлен);
	СтруктураДанныхЭтогоПользователя.Вставить("ПоказыватьВСпискеВыбора", ДанныеЭтогоПользователя.ПоказыватьВСпискеВыбора);
	СтруктураДанныхЭтогоПользователя.Вставить("ПользовательОС", ДанныеЭтогоПользователя.ПользовательОС);
	СтруктураДанныхЭтогоПользователя.Вставить("РежимЗапуска", СокрЛП(ДанныеЭтогоПользователя.РежимЗапуска));

	Возврат СтруктураДанныхЭтогоПользователя;
...
Функция   ПроверитьНаличиеПропущенныхНапоминаний(Знач ТолькоЗадачи = ЛОЖЬ) Экспорт
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ ЗаметкиИНапоминанияПолучатели.Ссылка.Наименование,
	...
	|ИЗ Справочник.ЗаметкиИНапоминания.Получатели КАК ЗаметкиИНапоминанияПолучатели
	|ГДЕ (ЗаметкиИНапоминанияПолучатели.Получатель = &ПустойПолучатель
...
Процедура РезервироватьБазуДанных(Знач ПараметрыРезервирования) Экспорт
	...
	Исключение
		ТекстОписаниеОшибки = ОписаниеОшибки();
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при создании резерва") + ": " + ТекстОписаниеОшибки);
	КонецПопытки;
...
Процедура СохранитьВФайл(Знач ПараметрыСохранения) Экспорт
	...
	ДокументДляПечати = ПараметрыСохранения[0];
...
Функция   СоздатьХранилищеДенег(Знач Валюта = Неопределено, Знач ФормаОплаты = Неопределено, Знач НомерСчета = "") Экспорт
	
	ХранилищеДенег = Справочники.ХранилищаДенег.СоздатьЭлемент();
	ХранилищеДенег.Валюта = ?(ЗначениеЗаполнено(Валюта), Валюта, ПредопределенноеЗначение("Справочник.Валюты.ОсновнаяВалюта"));
	ХранилищеДенег.ФормаОплаты = ?(ЗначениеЗаполнено(ФормаОплаты), ФормаОплаты, ПредопределенноеЗначение("Перечисление.ФормыОплаты.Наличные"));
	ХранилищеДенег.НомерСчета = НомерСчета;
	ХранилищеДенег.Наименование = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Деньги в") + " " + ХранилищеДенег.Валюта + " (" + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке(ХранилищеДенег.ФормаОплаты) + ")";
	
	ХранилищеДенег.Записать();	
	
	Возврат ХранилищеДенег.Ссылка;
	
КонецФункции // Создать Хранилище Денег
