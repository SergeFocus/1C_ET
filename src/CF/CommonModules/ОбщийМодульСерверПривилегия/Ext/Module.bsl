		...
		Попытка 
			...
			ИначеЕсли СформироватьОтчетИОтправитьПоEmail
				...
				Иначе					
					...
					ОбщийМодульСервер.СформироватьПроизвольныйОтчетНаСервере(ТаблицаПараметров, ДокументДляПечати, ПодЗадача.НаименованиеОтчета);
...
Процедура ОбновитьДанныеПриОбновленииКонфигурации(Знач ВерсияБыла, Знач ВерсияКонфигурации) Экспорт
	...
	Попытка		
		...
		РегистрационныйКодКУстановке = ОбщийМодульСервисСервер.РегистрационныйКод(ИСТИНА);
