...
Функция   ВыполнитьПодстановкуСвоихЗначений(Знач ПараметрОбластиПечати, Знач ЗначениеДляЗамены) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 СвоиЗначенияПечати.НаЗначение
	...
	|УПОРЯДОЧИТЬ ПО СвоиЗначенияПечати.ДлинаЗначения ВОЗР";
...
КонецФункции // Вычислить Имя Формы
...
КонецФункции // Выяснить Подходящий Вид Объекта
...
КонецПроцедуры //Заполнить Список Выбора Единиц Измерения
...
КонецФункции // Значение Предопределенного
...
Функция   НайтиФормулуДляНадписиНаФорме(Знач ИмяФормыСНадписью) Экспорт
	Возврат ОбщийМодульСервер.НайтиФормулуДляНадписиНаФорме(ИмяФормыСНадписью);
КонецФункции
...
Функция   ПолнаяВерсияКонфигурации() Экспорт
	Возврат ОбщийМодульСервер.ПолнаяВерсияКонфигурации();
КонецФункции

&НаСервере
Функция   НайтиХранилищеДенегПоВалюте(Знач Валюта, Знач ХранилищеДенег = Неопределено) Экспорт
...
Функция   НайтиГруппуНоменклатурыПоНаименованию(Знач Наименование, Знач НеСоздавать = ЛОЖЬ) Экспорт
	Возврат ОбщийМодульТоварСервер.НайтиГруппуНоменклатурыПоНаименованию(Наименование, НеСоздавать);
КонецФункции
...
Функция   ПолучитьДополнительныеРеквизитыВидаСредств(Знач ВидСредств = Неопределено, Знач Сворачивать = ЛОЖЬ) Экспорт
	...
	|ИЗ Справочник.ВидыСредствПредприятия.ДополнительныеРеквизиты КАК ВидыСредствПредприятияДополнительныеРеквизиты";
...
КонецФункции // Получить Дополнительные Реквизиты Клиентов
...
КонецФункции // Получить Запрет На Настройки Отчета
...
&НаСервере
Функция   ПолучитьНоменклатуруЧастогоСпроса(Знач Позиция) Экспорт
...
КонецФункции // Получить Номенклатуру частого Спроса

&НаСервере
Функция   НайтиНоменклатуруПоПредставлениюКонтрагента(Знач КлиентПоставщик, Знач ПолеПредставление = Неопределено, Знач Текст, Знач НаЯзыке = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(КлиентПоставщик)
		...
		Если НЕ ЗначениеЗаполнено(ПолеПредставление = Неопределено) Тогда
			ПолеПредставление = ПредопределенноеЗначение("Перечисление.ПоляПредставленийНоменклатуры.Наименование");
		КонецЕсли;
		...
		Запрос.УстановитьПараметр("Поле", ПолеПредставление);
...
Функция   ПолучитьОписаниеВсехАналогов(Знач Номенклатура, Знач ФорматРезультата = 0) Экспорт
	...
	Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьМеханизмАналоговДляНоменклатуры") Тогда
		...
		|	ГДЕ ГруппыАналоговТовары.Номенклатура = &Номенклатура) КАК ВложенныйЗапрос
...
КонецФункции //Получить Описание Всех Аналогов
...
КонецФункции // Получить Уровень Доступа
...
КонецФункции // Получить Представление На Языке
...
Функция   ПолучитьТаблицуОбъектовДоступа() Экспорт
	...
	СтрокаДоступа.ОбъектДоступа = СтрокаОтменаПроведенияДокументов + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Корректировки и взаимозачетов с контрагентами");
	СтрокаДоступа.УровеньДоступаПоУмолчанию = Перечисления.УровниДоступа.ТолькоЧтение;
	СтрокаДоступа = ТаблицаОбъектовДоступа.Добавить();
	...
	СтрокаДоступа.ОбъектДоступа = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Изменение цен в документах реализации");
	СтрокаДоступа.УровеньДоступаПоУмолчанию = Перечисления.УровниДоступа.ПолныйДоступ;
	СтрокаДоступа = ТаблицаОбъектовДоступа.Добавить();
	СтрокаДоступа.КодПрава		= "002350";
	СтрокаДоступа.ОбъектДоступа = СтрокаДокументы + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Установки цен и карточка цены");
	СтрокаДоступа.УровеньДоступаПоУмолчанию = Перечисления.УровниДоступа.ТолькоЧтение;
	СтрокаДоступа = ТаблицаОбъектовДоступа.Добавить();
...
КонецФункции // Получить Текущий Язык Документов
...
КонецФункции // Получить ШтрихКод Номенклатуры Или Серии
...
КонецФункции // Регистрационный Код

Функция   ЭтоСпециальныйСправочник(Знач ТипСправочника) Экспорт
...
КонецФункции // Это Набор

&НаСервере
Функция   ПолучитьТипРеквизита(Знач ВыбранныйТипОбъекта, Знач НаименованиеРеквизита = "Наименование", Знач ЭтоСправочник = ЛОЖЬ, Знач ЭтоДокумент = ЛОЖЬ) Экспорт

	Результат = Новый Структура("ТипСтрокой, ТипТипом, Составной, РазделМетаданных, ТипСсылка", "", Неопределено, ЛОЖЬ, "", "");

	Если ЭтоСправочник Тогда

		Если НаименованиеРеквизита = "Наименование"
			ИЛИ НаименованиеРеквизита = "Код" Тогда

			Результат.ТипСтрокой = "Строка";
			Результат.ТипТипом = Тип("Строка");
		ИначеЕсли НаименованиеРеквизита = "ПометкаУдаления" Тогда
			Результат.ТипСтрокой = "Булево";
			Результат.ТипТипом = Тип("Булево");
		КонецЕсли;
	ИначеЕсли ЭтоДокумент Тогда
		Если НаименованиеРеквизита = "Номер" Тогда
			Результат.ТипСтрокой = "Строка";
			Результат.ТипТипом = Тип("Строка");
		ИначеЕсли НаименованиеРеквизита = "Дата" Тогда
			Результат.ТипСтрокой = "Дата";
			Результат.ТипТипом = Тип("Дата");
		ИначеЕсли НаименованиеРеквизита = "ПометкаУдаления" Тогда
			Результат.ТипСтрокой = "Булево";
			Результат.ТипТипом = Тип("Булево");
		КонецЕсли;
	ИначеЕсли НаименованиеРеквизита = "Период" Тогда
		Результат.ТипСтрокой = "Дата";
		Результат.ТипТипом = Тип("Дата");
	КонецЕсли;

	Если ЭтоСправочник Тогда
		Результат.РазделМетаданных = "Справочники";
	ИначеЕсли ЭтоДокумент Тогда
		Результат.РазделМетаданных = "Документы";
	КонецЕсли;

	Если ПустаяСтрока(Результат.ТипСтрокой) Тогда
		ЭтоРегистр = ЛОЖЬ;
		...
		Если ПустаяСтрока(Результат.РазделМетаданных) Тогда
			...
			Если ОбъектМетаданных = Неопределено Тогда
				...
				Если ОбъектМетаданных = Неопределено Тогда
					...
					Если ОбъектМетаданных = Неопределено Тогда
						...
						Результат.РазделМетаданных = "РегистрНакоплений";
					Иначе
						Результат.РазделМетаданных = "РегистрСведений";
					КонецЕсли;
				Иначе
					Результат.РазделМетаданных = "Документ";
					ЭтоДокумент = ИСТИНА;
				КонецЕсли;
			Иначе
				Результат.РазделМетаданных = "Справочник";
				ЭтоСправочник = ИСТИНА;
			КонецЕсли;
		КонецЕсли;

		Если НЕ Результат.РазделМетаданных = "" Тогда

			РеквизитМетаданных = Неопределено;
			Если ЭтоРегистр Тогда
				Попытка
					РеквизитМетаданных = Метаданные[Результат.РазделМетаданных][ВыбранныйТипОбъекта].Реквизиты[НаименованиеРеквизита];
				Исключение
					РеквизитМетаданных = Метаданные[Результат.РазделМетаданных][ВыбранныйТипОбъекта].СтандартныеРеквизиты[НаименованиеРеквизита];
				КонецПопытки;
			Иначе
				Попытка
					РеквизитМетаданных = Метаданные[Результат.РазделМетаданных][ВыбранныйТипОбъекта].Измерения[НаименованиеРеквизита];
				Исключение
					Попытка
						РеквизитМетаданных = Метаданные[Результат.РазделМетаданных][ВыбранныйТипОбъекта].Ресурсы[НаименованиеРеквизита];
					Исключение
						Попытка
							РеквизитМетаданных = Метаданные[Результат.РазделМетаданных][ВыбранныйТипОбъекта].Реквизиты[НаименованиеРеквизита];
						Исключение
			...
			Если НЕ РеквизитМетаданных = Неопределено Тогда
				...
				ТипТут = РеквизитМетаданных.Тип.Типы()[0];
				Результат.ТипТипом = ТипТут;
				...
				Иначе
					...
					Если НЕ НайденыМета = Неопределено Тогда
						Результат.ТипСтрокой = Метаданные.НайтиПоТипу(РеквизитМетаданных.Тип.Типы()[0]).Имя;

						Если Справочники.ТипВсеСсылки().СодержитТип(ТипТут) Тогда
							Результат.ТипСсылка = "Справочник";
						ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипТут) Тогда
							Результат.ТипСсылка = "Документ";
						ИначеЕсли Перечисления.ТипВсеСсылки().СодержитТип(ТипТут) Тогда
							Результат.ТипСсылка = "Перечисление";
						КонецЕсли;

					Иначе
						Результат.ТипСтрокой = "NULL";
					КонецЕсли;
				КонецЕсли;

				Если НЕ РеквизитМетаданных.Тип.Типы().Количество() = 1 Тогда
					Результат.Составной = ИСТИНА;
				КонецЕсли;
...
Функция   СравнитьНаРавенствоСКешированием(Знач ЗначениеСравнения, Знач ЭталонСравнения = Неопределено, Знач РежимСравнения = 0) Экспорт
	
	Результат = ЛОЖЬ;
	
	Если РежимСравнения = 0 Тогда
		Результат = ЗначениеСравнения = ЭталонСравнения;
		
	ИначеЕсли РежимСравнения = 1 
		И НЕ ЭталонСравнения = Неопределено Тогда //  в списке значений		
		
		Результат = НЕ ЭталонСравнения.НайтиПоЗначению(ЗначениеСравнения) = Неопределено;
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции // Сравнить На Равенство С Кешированием
