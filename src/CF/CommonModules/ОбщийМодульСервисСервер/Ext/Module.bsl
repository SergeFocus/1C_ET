...
Функция   ВывестиМакетЧтоНового(ДокументДляПечати) Экспорт
	...
	Если РегистрационныйКодКонфигурации() = "ah" Тогда
...
Функция   ДатаПервогоДокумента(Знач ТолькоВаловойПрибыли = ЛОЖЬ) Экспорт
	...
	Иначе
		...
		|ИЗ РегистрНакопления.ТоварыПереданныеНаКомиссию КАК ТоварыПереданныеНаКомиссию
		|УПОРЯДОЧИТЬ ПО Период";
	КонецЕсли;
...
Процедура ПисьмоАвтору(Знач СтруктураПисьма = Неопределено) Экспорт
	...
	Если НЕ СтруктураПисьма = Неопределено
		...
		Если НЕ Отмена Тогда
			РегистрационныйКод = ОбщийМодульСервисСервер.РегистрационныйКодКонфигурации();
			Если ПустаяСтрока(РегистрационныйКод) Тогда
			...
			// не воруйте пожалуйста - оно Вам ничего не даст. это просто способ формирования бесплатных писем автору на анонимном смтп-сервере
			Пароль = "yESei*xxdMAL7yZCyByE4w";
			//Профиль.АдресСервераSMTP 	= "smtp.mandrillapp.com";
			Пароль = СтрЗаменить(Пароль, "*", "6");
			Профиль.АдресСервераSMTP 	= "smtp.sendgrid.net";
			Профиль.ПортSMTP			= 587;
//			Профиль.Пользователь		= "PaKcfmNmQi2IXifUpyGPoA";
//			Профиль.ПользовательSMTP 	= "PaKcfmNmQi2IXifUpyGPoA";
//			"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"
			Профиль.Пользователь		= "Eltr_od_ua";
			Профиль.ПользовательSMTP 	= "Eltr_od_ua";
...
КонецФункции // Получить Версию Платформы
...
Функция   ПолучитьКрасивоеНаименованиеОбъекта(Знач НаименованиеОбъекта, Знач СПриставкой = ЛОЖЬ) Экспорт
	...
	// Не удалять - вкопия сжатого кода
	...
	Пока СчетчикСимволов < ДлинаНаименования Цикл СчетчикСимволов = СчетчикСимволов + 1; СимволСтроки = Сред(Наименование, СчетчикСимволов, 1); Если СимволСтроки = ВРег(СимволСтроки) Тогда СобираемоеНаименование = СобираемоеНаименование + " " + НРег(СимволСтроки); Иначе СобираемоеНаименование = СобираемоеНаименование + СимволСтроки; КонецЕсли; КонецЦикла;

	Возврат ОбщийМодульПовтор.ПолучитьТекстНаЯзыке(СтрЗаменить(СобираемоеНаименование, "  ", " "));
...
Функция   РегистрационныйКодКонфигурации(Знач ИзСвойств = ЛОЖЬ) Экспорт
	Возврат ОбщийМодульПовтор.РегистрационныйКодКонфигурации(ИзСвойств);
КонецФункции
...
Процедура УстановитьПараметрыСеансаЭлементарнаяТорговля(Знач ВерсияПриложения = 0, Знач Принудительно = ЛОЖЬ) Экспорт

	Если Принудительно Тогда
		ПараметрыСеансаНеУстановлены = ИСТИНА;
	Иначе
		Попытка
			ЛюбойПараметр = ПараметрыСеанса.ВерсияПриложения;
			ПараметрыСеансаНеУстановлены = ЛОЖЬ;
		Исключение
			ПараметрыСеансаНеУстановлены = ИСТИНА;
	...
	Если НЕ ВерсияПриложения = 0 Тогда
		ПараметрыСеанса.ВерсияПриложения = ВерсияПриложения;
	КонецЕсли;

	Если ПараметрыСеансаНеУстановлены Тогда
		...
		Если НЕ ДанныеТекущегоПользователя.Язык = Неопределено Тогда
			...
			КодЯзыка = Врег(СокрЛП(ДанныеТекущегоПользователя.Язык.КодЯзыка));
			Если КодЯзыка = "RU" Тогда
				ТекущийЯзыкИнтерфейсаПрограммы = ПредопределенноеЗначение("Справочник.Языки.Русский");
			Иначе
				ТекущийЯзыкИнтерфейсаПрограммы = Справочники.Языки.НайтиПоКоду(КодЯзыка);
		...
		Если НЕ ТипЗнч(Настройка) = Тип("НастройкиКлиентскогоПриложения") Тогда
			...
			Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("Интерфейс1С82") Тогда
				Настройка.ВариантИнтерфейсаКлиентскогоПриложения = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_2;
				Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВидФормВЗакладках") Тогда
			...
			Иначе
				...
				Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ОтменитьКомпактностьВыводитьЭлементыБольшими") Тогда
		...
		Иначе
			Былизм = ЛОЖЬ;
			Интерфейс82 = ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("Интерфейс1С82");
			Если Интерфейс82
				И НЕ Настройка.ВариантИнтерфейсаКлиентскогоПриложения = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_2 Тогда

				Настройка.ВариантИнтерфейсаКлиентскогоПриложения = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_2;
				Былизм = ИСТИНА;
			ИначеЕсли НЕ Интерфейс82
				И НЕ Настройка.ВариантИнтерфейсаКлиентскогоПриложения = ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда

				Настройка.ВариантИнтерфейсаКлиентскогоПриложения = ВариантИнтерфейсаКлиентскогоПриложения.Такси;
				Былизм = ИСТИНА;
			КонецЕсли;

			Если Интерфейс82 Тогда
				ВидФормВЗакладках = ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВидФормВЗакладках");

				Если ВидФормВЗакладках
					И Не Настройка.РежимОткрытияФормПриложения = РежимОткрытияФормПриложения.Закладки Тогда

					Настройка.РежимОткрытияФормПриложения = РежимОткрытияФормПриложения.Закладки;
					Былизм = ИСТИНА;
				ИначеЕсли НЕ ВидФормВЗакладках
					И НЕ Настройка.РежимОткрытияФормПриложения = РежимОткрытияФормПриложения.ОтдельныеОкна Тогда

					Настройка.РежимОткрытияФормПриложения = РежимОткрытияФормПриложения.ОтдельныеОкна;
					Былизм = ИСТИНА;
				КонецЕсли;
			Иначе
				ОтменитьКомпактностьВыводитьЭлементыБольшими = ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ОтменитьКомпактностьВыводитьЭлементыБольшими");
				Если ОтменитьКомпактностьВыводитьЭлементыБольшими
					И НЕ Настройка.ВариантМасштабаФормКлиентскогоПриложения = ВариантМасштабаФормКлиентскогоПриложения.Авто Тогда

					Настройка.ВариантМасштабаФормКлиентскогоПриложения = ВариантМасштабаФормКлиентскогоПриложения.Авто;
					Былизм = ИСТИНА;
				ИначеЕсли НЕ ОтменитьКомпактностьВыводитьЭлементыБольшими
					И НЕ Настройка.ВариантМасштабаФормКлиентскогоПриложения = ВариантМасштабаФормКлиентскогоПриложения.Компактный Тогда
			...
			Если Былизм Тогда
				ХранилищеСистемныхНастроек.Сохранить("Общее/НастройкиКлиентскогоПриложения", , Настройка, , ПользовательНаименование);
				ОбщийМодульСервисСервер.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Перезапустите 1С для сохранения настроек интерфейса."));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция   ПолучитьРеквизитыОбъектаМетаданных(РеквизитыОбъекта, Знач ВыбранныйТипОбъекта, Знач ИсключитьЖурналирование = ЛОЖЬ) Экспорт

	РезультатСтруктура = Новый Структура("РезультатИмеетОтветственного, ТипВСтроку, СправочникИерархический, ЭтоДокумент, ЭтоСправочник, УстановленныйВыбранныйТипОбъекта", ИСТИНА, "", ЛОЖЬ, ЛОЖЬ, ЛОЖЬ);
	...
	Если ОбъектМетаданных = Неопределено Тогда
		...
		Если ОбъектМетаданных = Неопределено Тогда
			...
			Если ОбъектМетаданных = Неопределено Тогда
				...
				РезультатСтруктура.ТипВСтроку = "РегистрНакоплений";
			Иначе
				РезультатСтруктура.ТипВСтроку = "РегистрСведений";
			КонецЕсли;
		Иначе
			РезультатСтруктура.ТипВСтроку = "Документ";
			РезультатСтруктура.ЭтоДокумент = ИСТИНА;
		КонецЕсли;
	Иначе
		РезультатСтруктура.ТипВСтроку = "Справочник";
		РезультатСтруктура.ЭтоСправочник = ИСТИНА;
		РезультатСтруктура.СправочникИерархический = ОбъектМетаданных.Иерархический;
	КонецЕсли;
	...
	Если НЕ ОбъектМетаданных = Неопределено Тогда
		...
		Если ЭтоРегистр Тогда
			...
			Для Каждого РеквизитОбъекта Из ОбъектМетаданных.СтандартныеРеквизиты Цикл // период
				РеквизитыОбъекта.Добавить(РеквизитОбъекта.Имя, ?(ПустаяСтрока(РеквизитОбъекта.Синоним), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке(РеквизитОбъекта.Имя), ОбщийМодульСервисСервер.ПолучитьКрасивоеНаименованиеОбъекта(РеквизитОбъекта.Синоним)), ЛОЖЬ);
			КонецЦикла;
		Иначе
			Для Каждого РеквизитОбъекта Из ОбъектМетаданных.СтандартныеРеквизиты Цикл
				Если НЕ РеквизитОбъекта.Имя = "ИмяПредопределенныхДанных"
					И НЕ РеквизитОбъекта.Имя = "Предопределенный"
					И НЕ РеквизитОбъекта.Имя = "Ссылка" Тогда
		...
		Если НЕ ИсключитьЖурналирование
			И ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьЖурналированиеСобытий") Тогда

			Если НЕ УстановленныйВыбранныйТипОбъекта = "Валюты"
				И НЕ УстановленныйВыбранныйТипОбъекта = "ВидыСобытий"
				И НЕ УстановленныйВыбранныйТипОбъекта = "ЗначенияЯчеекТаблицДанных"
				И НЕ УстановленныйВыбранныйТипОбъекта = "КатегорииКонтрагентов"
				...
				И НЕ УстановленныйВыбранныйТипОбъекта = "ВидыСредствПредприятия"
				...
				И НЕ УстановленныйВыбранныйТипОбъекта = "НаправленияДеятельности"
				...
				И НЕ УстановленныйВыбранныйТипОбъекта = "ВерсииОбъектовБазыДанных"
				И НЕ УстановленныйВыбранныйТипОбъекта = "ДополнительноОФизическихЛицах"
				...
				И НЕ УстановленныйВыбранныйТипОбъекта = "ПринадлежностьВаучеров"
				И НЕ УстановленныйВыбранныйТипОбъекта = "СвязанныеКлючиУчетаНоменклатуры"
				...
				И НЕ УстановленныйВыбранныйТипОбъекта = "ВнутренниеЗаказыТоваров"
				И НЕ УстановленныйВыбранныйТипОбъекта = "ЗадействованныеСредстваПредприятия"
				И НЕ УстановленныйВыбранныйТипОбъекта = "ВыполнениеРаботСотрудниками"
				И НЕ УстановленныйВыбранныйТипОбъекта = "НакоплениеНаВаучеры"
				И НЕ УстановленныйВыбранныйТипОбъекта = "ТоварыПереданныеНаКомиссию"
			...
			Иначе
				РезультатСтруктура.РезультатИмеетОтветственного = ЛОЖЬ;
			КонецЕсли;
		КонецЕсли;
	...
	РезультатСтруктура.Вставить("УстановленныйВыбранныйТипОбъекта", УстановленныйВыбранныйТипОбъекта);

	Возврат РезультатСтруктура;

КонецФункции

Процедура ОбработатьНовуюФорму(НоваяФорма) Экспорт
	
	Попытка
	Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВыводитьПодСпискамиВыбранныйЭлемент") Тогда
		
		ЭлементСписок = НоваяФорма.Элементы.Найти("Список");
		
		Если НЕ ЭлементСписок = Неопределено 
			И Тип(ЭлементСписок) = Тип("ТаблицаФормы") Тогда
			
			ЭлементСписокРодитель = НоваяФорма.Элементы.Добавить("ЭлементСписокРодитель", Тип("ГруппаФормы"), ЭлементСписок.Родитель);
			ЭлементСписокРодитель.Вид = ВидГруппыФормы.ОбычнаяГруппа;
			ЭлементСписокРодитель.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
			
			НоваяФорма.Элементы.Переместить(ЭлементСписок, ЭлементСписокРодитель);
			
			НовыйЭлемент = НоваяФорма.Элементы.Добавить("ВыбранныйЭлемент", Тип("ПолеФормы"), ЭлементСписокРодитель);
			НовыйЭлемент.Вид = ВидПоляФормы.ПолеНадписи;
			НовыйЭлемент.ПутьКДанным = "Элементы.Список.ТекущиеДанные.Ссылка";
			НовыйЭлемент.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Выбран");	
		КонецЕсли;	
	КонецЕсли;
	
	ЭлементКомментарий = НоваяФорма.Элементы.Найти("Комментарий");
	
	Если НЕ ЭлементКомментарий = Неопределено
		И ЭлементКомментарий.Вид = ВидПоляФормы.ПолеВвода Тогда
		
		Комментарий = НоваяФорма.Объект.Комментарий;
		Если ПустаяСтрока(Комментарий) Тогда
			ЭлементКомментарий.Видимость = ЛОЖЬ;
			
			// ГруппаСтандартныхРеквизитов = НоваяФорма.Элементы.Найти("ГруппаСтандартныхРеквизитов");
			
			НоваяКоманда = НоваяФорма.Команды.Добавить("ВвестиКомментарий");
			НоваяКоманда.Действие 	= "РеакцияНаПрочиеСобытияФормы";
			НоваяКоманда.Заголовок 	= ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Комментировать");
			НоваяКоманда.Картинка 	= БиблиотекаКартинок.КомандаКомментарийКДанным;
			НоваяКоманда.Отображение= ОтображениеКнопки.Картинка;
			НоваяКоманда.Подсказка 	= ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Добавить комментарий");
			
			НовыйЭлемент = НоваяФорма.Элементы.Добавить("ВвестиКомментарий", Тип("КнопкаФормы"), ЭлементКомментарий.Родитель); //?(ГруппаСтандартныхРеквизитов = Неопределено, ЭлементКомментарий.Родитель, ГруппаСтандартныхРеквизитов));
			НовыйЭлемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
			НовыйЭлемент.ИмяКоманды = "ВвестиКомментарий";
...
Процедура ЗавершитьПериодРаботыПользователя(Знач ТипСобытия, Знач ДатаНачала, Знач ДатаОкончания) Экспорт
	
	СотрудникПользователя = ОбщийМодульПовтор.ПолучитьТекущегоСотрудника();
	Если ЗначениеЗаполнено(СотрудникПользователя) Тогда
		
		Если ТипСобытия = 4 Тогда
			Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Завершение работы");
		ИначеЕсли ТипСобытия = 5 Тогда
			Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не отвечает");
		Иначе
			Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Перерыв");	
		КонецЕсли;
		
		ЗаписьОРаботе = РегистрыСведений.ВремяРаботыСотрудников.СоздатьМенеджерЗаписи();
		ЗаписьОРаботе.ВремяРаботы 	= ДатаОкончания - ДатаНачала;
		ЗаписьОРаботе.ДатаНачала  	= ДатаНачала;
		ЗаписьОРаботе.День  		= ДатаНачала;
		ЗаписьОРаботе.ДатаОкончания = ДатаОкончания;
		ЗаписьОРаботе.Комментарий 	= Комментарий;
		ЗаписьОРаботе.Сотрудник   	= СотрудникПользователя;
		ЗаписьОРаботе.Записать(ИСТИНА);
	КонецЕсли;
	
КонецПроцедуры

Функция   ПолучитьСписокВариантовОтчета(Знач НаименованиеОтчета) Экспорт
	
	Результат = Новый СписокЗначений;
	
	Если НЕ ПустаяСтрока(НаименованиеОтчета) Тогда
		...
		Для Каждого ВариантНастроек Из Схема.ВариантыНастроек Цикл
			Результат.Добавить(ВариантНастроек.Имя, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке(ВариантНастроек.Представление));
		КонецЦикла;
	КонецЕсли;
