...
Процедура СообщитьПользователю(Знач ТекстСообщения,
	...
	Знач ПутьКДанным = "",
	УстановитьОтказ = ЛОЖЬ,
	Знач ПисатьВЖурналЗапрещено = ЛОЖЬ,
	Знач СтатусЭтогоСообщения = Неопределено) Экспорт
	
	Если ТипЗнч(ОбъектИлиСсылка) = Тип("СтатусСообщения") Тогда
		СтатусЭтогоСообщения = ОбъектИлиСсылка;
		ОбъектИлиСсылка = Неопределено;
	КонецЕсли;
	...
	Если НЕ ПисатьВЖурналЗапрещено Тогда
		...
		Если РегистрироватьВЖурнал Тогда
			ОбщийМодульСервер.ДобавитьСобытиеЖурналаНаСервере(ТекстСообщения, 3, ОбъектИлиСсылка);
		КонецЕсли;
