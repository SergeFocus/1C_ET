...
Функция   ВыполнитьВыгрузкуВФайл(СтруктураИмпортаЭкспорта) Экспорт
	...
	Иначе
		...
		ИначеЕсли ШаблонФайлаФорматФайлаEXCEL Тогда
			...
			Исключение
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Проблема с MS excel") + ": " + ТекстОписаниеОшибки;
					...
					Пока ВыборкаОбъектов.Следующий() Цикл
						...
						Для Каждого СледующееПолеФайлу ИЗ ШаблонФайлаПоследовательностьПолейВФайле Цикл
							...
							Исключение
								ТекстОписаниеОшибки = ОписаниеОшибки();
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Неверный запрос ИЛИ имя результата") + " " + ШаблонФайла + "! " + ИмяПоляВФайле + ": " + ТекстОписаниеОшибки;
								Сообщение.УстановитьДанные(ОбъектОперацииСсылка);
								Сообщение.Сообщить();
	...
	Иначе
		...
		Исключение 	
			ТекстОписаниеОшибки = ОписаниеОшибки();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка записи") + ": " + ТекстОписаниеОшибки;
			Сообщение.УстановитьДанные(ОбъектОперацииСсылка);
			Сообщение.Сообщить();
...
Функция   ВыполнитьЗагрузкуИзФайла(СтруктураИмпортаЭкспорта) Экспорт
	...
	ИначеЕсли ШаблонФайлаФорматФайлаEXCEL Тогда		
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Проблема создания объекта таблицы MS Excel") + ": " + ТекстОписаниеОшибки;
			Сообщение.УстановитьДанные(ОбъектОперации);
			Сообщение.Сообщить();							
	...
	Если КоличествоСтрокФайла > 0 Тогда
		...
		Если ЭтоТаблица Тогда 
			...
			Исключение 	
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщение  =  Новый СообщениеПользователю;
				Сообщение.Текст  =  ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка очистки таблицы") + ": " + ТекстОписаниеОшибки;
				Сообщение.Сообщить();				
			КонецПопытки;
		КонецЕсли;
				...
				Если ЭтоЗагрузкаСписка 
					...
					Если НайденыПоляСинхронизацииВЦикле Тогда
						...
						Если НЕ ВзДС.Пустой() Тогда
							...
							Иначе
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Пропущен существующий") + " " + НайденныйОбъектОперации;
								Сообщение.Сообщить();
				...
				Если НЕ ОтказВЗаписи ТОгда
					...
					Если ЗначениеЗаполнено(ШаблонФайлаСоставлятьНаименованиеПоВыражениюИзИменПолей) 
						...
						Исключение 	
							ТекстОписаниеОшибки = ОписаниеОшибки();
							Сообщение  =  Новый СообщениеПользователю;
							Сообщение.Текст  =  ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка формирования наименования") + ": " + ТекстОписаниеОшибки;
							Сообщение.Сообщить();
						...
						Если ЗначениеЗаполнено(ИмяРодителя) Тогда			
							...
							ИначеЕсли ШаблонФайлаСоздаватьЭлементыСправочникаНоменклатураВСлучаеИхОтсутствия Тогда
								...
								Исключение 	
									ТекстОписаниеОшибки = ОписаниеОшибки();
									Сообщение = Новый СообщениеПользователю;
									Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи эл-та справочника") + ": " + РодительЭлемента + " " + ТекстОписаниеОшибки;
									Сообщение.Сообщить();
								КонецПопытки;  // записи  
							КонецЕсли;
						КонецЕсли;
					...
					Если НЕ ОтказВЗаписи Тогда
						...
						ИначеЕсли ЭтоЗагрузкаСписка 
									...
									Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений Тогда
										...
										Если ЭтоДокументНеСправочник
											...
											Исключение 
												...
												ТекстОписаниеОшибки = ОписаниеОшибки();
												Сообщение = Новый СообщениеПользователю;
												Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке проведения документа") + ": " + ОбъектОперации + " " + ТекстОписаниеОшибки;
												Сообщение.УстановитьДанные(ОбъектОперацииСсылка);
												Сообщение.Сообщить();
							...
							Исключение 	
								ТекстОписаниеОшибки = ОписаниеОшибки();
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при записи") + ": " + ТекстОписаниеОшибки;
								Сообщение.УстановитьДанные(ОбъектОперацииСсылка);
								Сообщение.Сообщить();
				...
				Если НЕ ОтказВЗаписи 
					...
					Для Каждого СтрокаРеквизита ИЗ ШаблонФайлаДополнительноеЗаполнениеРеквизитов Цикл
						...
						Исключение 	
							ТекстОписаниеОшибки = ОписаниеОшибки();
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка установки реквизита") + ": " + ТекстОписаниеОшибки;
							Сообщение.Сообщить();							
						КонецПопытки;
		...
		Если ШаблонФайла.УдалятьФайлПослеУдачногоИмпорта Тогда
			...
			Исключение 	
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка удаления файла") + ": " + ТекстОписаниеОшибки;
				Сообщение.Сообщить();
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ШаблонФайла.УсловиеДебетаПоле)
			...
			Исключение // ошибка установки условия дебета по полю
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка установки условия дебета по полю") + ": " + ШаблонФайла.УсловиеДебетаПоле + " - " + ТекстОписаниеОшибки;
				Сообщение.Сообщить();
			КонецПопытки;
		КонецЕсли;
				...
				Если ВыполненоЛиУсловиеФильтра(ЭлементПоведения.УсловиеДополнительногоПоведенияПриИмпорте, ЗначениеДанных, ЭлементПоведения.ЗначениеРеквизита) Тогда
					...
					Если ЭлементПоведения.ВыполнитьПрограммныйКод Тогда
						...
						Исключение 	
							ТекстОписаниеОшибки = ОписаниеОшибки();
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка выполнения кода поведения") + ": " + ТекстОписаниеОшибки;
							Сообщение.Сообщить();							
						КонецПопытки;  
					КонецЕсли;
...
Функция   ОбнаружитьДанныеПоВидуПоляИзСтроки(ОбъектОперации, 
		...
		Если ДанноеПолеКлючевоеДляПоискаИЗаписи Тогда
			...
			Исключение 	
				...
				Исключение 	
					ТекстОписаниеОшибки = ОписаниеОшибки();
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка") + ": " + ТекстОписаниеОшибки;
					Сообщение.Сообщить();
				КонецПопытки;
			КонецПопытки;	
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.НаименованиеКлиента Тогда	
		...
		// 	Иначе
		// 		Если НЕ ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "НомерПлатежногоДокумента", ИмяТаблицыШаблонаФайла) = "" Тогда
		// 			ОбъектОперации.НомерПлатежногоДокумента = ЭлементДанныхФайла;	
		// 		Иначе
...
Функция   ПолучитьАресФайлаШаблона(Знач ШаблонШапки) Экспорт
...
Функция   ПроверитьРавенствоИлиИзменить(Знач БылиИзменения = ЛОЖЬ, Знач ОбъектПроверкиИзменения, Знач ЗначениеПроверкиИзменения)
