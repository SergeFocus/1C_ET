// sza150514-0014
// sza150105-1154 иерарх
// sza150103-1417 фикс
// sza141201-0143 ПК
// sza141120-1240 назнпат
// sza141117-1654 массфикс
// sza141114-1432 фикс
// sza140923-1537 фикс
// sza140616-2233
// sza140228-0059
...
Процедура ВыполнитьВыгрузкуШаблоновНаСервере(АдресФайла, ВыбранШаблон = Неопределено) Экспорт
	...
	ШаблонФайла = Справочники.ПодсистемаИЭШаблоны.Выбрать();
	...
	Пока ШаблонФайла.Следующий() Цикл // УСЛОВИЕ ЗАПИСИ
		...
		Если (ЗначениеЗаполнено(ВыбранШаблон)
			...
			Если ШаблонФайла.РассчитыватьСрокОкончанияОбмена Тогда
			...
			БазаДБФБазаШаблонов.SKPO = ?(ШаблонФайла.СоздаватьКонтрагентовПоОКПО, 1, 0);
	...
	БазаДБФПоляШаблонов.поля.Добавить("OSTROKE", 	"S", 254, 0);
		...
		Если (ЗначениеЗаполнено(ВыбранШаблон)
			...
			Для Каждого ПолеШаблона Из ШаблонФайла.ПоследовательностьПолейВФайле Цикл
				Если ЗначениеЗаполнено(ПолеШаблона.ПолеТаблицы) Тогда
					...
					БазаДБФПоляШаблонов.OSTROKE  = ПолеШаблона.ОСтроке;
...
Функция   ВыполнитьДляФайлаНаСервере(СтруктураИмпортаЭкспорта) Экспорт
	...
	Иначе
		...
		// Если ШаблонФайла = Справочники.ПодсистемаИЭШаблоны.ОбменЗаказамиСПОDBF Тогда
		// 	СтруктураИмпортаЭкспорта.ИмяТаблицыШаблонаФайла = "DBFЗаказы";
		//
		// ИначеЕсли ШаблонФайла = Справочники.ПодсистемаИЭШаблоны.ОбменНоменклатурыСПОDBF
		// 	ИЛИ ШаблонФайла = Справочники.ПодсистемаИЭШаблоны.ОбменНоменклатурыСПОDBFEXCEL Тогда

		// 	СтруктураИмпортаЭкспорта.ИмяТаблицыШаблонаФайла = "DBFНоменклатура";
		//
		// ИначеЕсли ШаблонФайла = Справочники.ПодсистемаИЭШаблоны.ОбменШапкаЗаказаСПОDBF Тогда
		...
		Если ШаблонФайла.ФайлАрхив
			...
			Если МассивФайлов.Количество() > 0 Тогда
				...
				Для Каждого ФайлИзСписка Из МассивФайлов Цикл
					...
					Если НЕ ВыборАдресФайла.Пустой() Тогда
						ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Файл") + " " + АдресФайла + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("пропущен (уже загружался)."));
					Иначе
			...
			Иначе
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Файлы по маске (") + СокрЛП(ШаблонФайла.МаскаФайла) + ") " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("в указанном каталоге") + " " + СтруктураИмпортаЭкспорта.АдресФайла + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("не обнаружены!"));
...
Функция   ВыполнитьЗагрузкуИзФайла(СтруктураИмпортаЭкспорта) Экспорт
	...
	Если НЕ ПустаяСтрока(ПериодДляДокументов)
	...
	Если ШаблонФайла.ИспользоватьТолькоДляЭкспорта Тогда
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Данный шаблон только для экспорта!"), ОбъектОперацииСсылка);
	...
	Иначе
		...
		Если НЕ ФайлПоАдресу.Существует()
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Файл по адресу") + " " + АдресФайла + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("не существует или недоступен!"), ОбъектОперацииСсылка);
	...
	ИначеЕсли ЭтоПредопределенныйШаблон Тогда
		ОбъектОперации = ОбъектОперацииСсылка;
	ИначеЕсли НЕ ПустаяСтрока(ИмяТаблицыШаблонаФайла) Тогда
		Попытка
	...
	Если ЭтотШаблонНеПодходитДляОбъекта(ШаблонФайла, ОбъектОперации, ИмяТаблицыШаблонаФайла) Тогда
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Данный шаблон не подходит по реквизитам отбора!"), ОбъектОперацииСсылка);
	...
	ИначеЕсли ШаблонФайлаФорматФайлаEXCEL Тогда
		...
		Исключение
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Проблема создания объекта таблицы MS Excel") + ": " + ТекстОписаниеОшибки, ОбъектОперацииСсылка);
	...
	Если КоличествоСтрокФайла > 0 Тогда
		...
		Если ЭтоТаблица Тогда
			...
			Исключение
				ТекстОписаниеОшибки = ОписаниеОшибки();
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка очистки таблицы") + ": " + ТекстОписаниеОшибки);
			КонецПопытки;
		КонецЕсли;
		...
		Если СтруктураИмпортаЭкспорта.ПредПодготовка Тогда
			...
			Пока СчетчикСтрок < (КоличествоСтрокФайла + ЧислоСтрокСНачалаФайлаКоторыеСледуетПропустить) Цикл
				...
				ИначеЕсли ШаблонФайлаФорматФайлаEXCEL Тогда
					МассивДанныхСтрокиФайла = Новый массив;
					Если ПустаяСтрока(ТекЛист.Cells(СчетчикСтрок, НомерПоляКотороеВсегдаЗаполнено).Value) ТОгда
		...
		Если ЗначениеЗаполнено(ШаблонФайлаИспользоватьЗаголовкиБлоковДанных) Тогда
			...
			Для Каждого ЭлементЗаголовкаБлокаДанных Из ШаблонФайлаПоследовательностьПолейВЗаголовкеБлокаДанных Цикл
				Если ЗначениеЗаполнено(ЭлементЗаголовкаБлокаДанных.ПолеТаблицы) Тогда
					// И не ЭлементЗаголовкаБлокаДанных.ПолеТаблицы = Справочники.ПодсистемаИЭПоля.ПропуститьДанноеПоле Тогда
		...
		Пока СчетчикСтрок < (КоличествоСтрокФайла + ЧислоСтрокСНачалаФайлаКоторыеСледуетПропустить) Цикл
			...
			Если ШаблонФайлаИспользоватьЗаголовкиБлоковДанных Тогда
				...
				ИначеЕсли НЕ ПустаяСтрока(ШаблонФайлаУсловиеЗаголовкаБлокаДанныхНаличиеПодстроки) Тогда // ищем подстроку, надеемся что шаблон заполнен адекватно
			...
			СчетчикМассива = 0;
			ДлинаМассива = МассивДанныхСтрокиФайла.Количество();
			ВсегоКолонок = ШаблонФайлаПоследовательностьПолейВФайле.Количество();
			Пока СчетчикМассива < ДлинаМассива Цикл

				Если СчетчикМассива < ВсегоКолонок
					И ШаблонФайлаПоследовательностьПолейВФайле[ СчетчикМассива ].ЕслиНеЗаполненоЗначитПринимаетПредыдущееЗначение Тогда
			...
			ИначеЕсли ЭтоЗагрузкаСписка ТОгда
				Если ШаблонФайлаВидШаблонаДляРегистровСведений Тогда
					...
					ИначеЕсли НЕ ПустаяСтрока(ПериодДляДокументов) Тогда
				...
				Иначе
					...
					Иначе
						...
						Если ПустаяСтрока(МетаданныеОбъекта) Тогда
						...
						Иначе
							...
							Если ШаблонФайлаПредназначенДляОбменаДокументамиДС Тогда
								ОбъектОперации.ФормаОплаты = ШаблонФайла.ФормаОплаты;
								Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетДенегВНесколькихХранилищах") Тогда
									Если ЗначениеЗаполнено(ШаблонФайла.ХранилищеДенег) Тогда
										ОбъектОперации.ХранилищеДенег = ШаблонФайла.ХранилищеДенег;
									Иначе
										ОбъектОперации.ХранилищеДенег = ПредопределенноеЗначение("Справочник.ХранилищаДенег.ОсновнаяКассаВОсновнойВалюте");
									КонецЕсли;

									ОбъектОперации.ФормаОплаты = ОбъектОперации.ХранилищеДенег.ФормаОплаты;
								КонецЕсли;

								Если НЕ ЗначениеЗаполнено(ОбъектОперации.ФормаОплаты) Тогда
									ОбъектОперации.ФормаОплаты = ПредопределенноеЗначение("Перечисление.ФормыОплаты.Безналичные");
								КонецЕсли;
								...
								Если НЕ Дебет Тогда
									...
									ОбъектОперации.Статья = ШаблонФайла.СтатьяДДСПоступленияПоУмолчанию;
									Если НЕ ЗначениеЗаполнено(ОбъектОперации.Статья) Тогда
										ОбъектОперации.Статья = ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("СтатьяПоступленияДенегПоУмолчанию");
									КонецЕсли;
								Иначе
									ОбъектОперации.Статья = ШаблонФайла.СтатьяДДСРасходаПоУмолчанию;

									Если НЕ ЗначениеЗаполнено(ОбъектОперации.Статья) Тогда
										ОбъектОперации.Статья = ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("СтатьяРасходаДенегПоУмолчанию");
									КонецЕсли;
								КонецЕсли;

								ОбъектОперации.Организация = ШаблонФайла.ОрганизацияПоУмолчанию;

								Если НЕ ЗначениеЗаполнено(ОбъектОперации.Организация) Тогда
									ОбъектОперации.Организация = ПредопределенноеЗначение("Справочник.Организации.ОсновноеПредприятие");
								КонецЕсли;
							КонецЕсли;
			...
			Если НЕ ОтказВЗаписи Тогда
				...
				Если ЭтоЗагрузкаСписка
					...
					Для Каждого СледующееПолеВФайле Из ШаблонФайлаПоследовательностьПолейВФайле Цикл
						Если СледующееПолеВФайле.ДанноеПолеКлючевоеДляПоискаИЗаписи Тогда
							...
							Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, НаименованиеКлючевогоПоля, ИмяТаблицыШаблонаФайла)) Тогда
							...
							ИначеЕсли СледующееПолеВФайле.ПолеТаблицы = ТипПоля.АртикулНоменклатуры Тогда
								...
								КонецЕсли;
							ИначеЕсли СледующееПолеВФайле.ПолеТаблицы = ТипПоля.ОКПОКонтрагента
								ИЛИ СледующееПолеВФайле.ПолеТаблицы = ТипПоля.ОКПОКлиента Тогда

								Если НЕ ТекстЗапросаУжеСформирован Тогда
									ЗапДСТекст = ЗапДСТекст + ?(НайденыПоляСинхронизации, " И ", " ГДЕ") + " " + МетаданныеОбъекта + ".КлиентПоставщик.ОКПО = &ОКПОК ";
									НайденыПоляСинхронизации = ИСТИНА;
								КонецЕсли;
								...
								// 	Если НЕ ТекстЗапросаУжеСформирован Тогда
								// 		ЗапДСТекст = ЗапДСТекст + ?(НайденыПоляСинхронизации, " И ", " ГДЕ") + " " + МетаданныеОбъекта + ".КлиентПоставщик.ИНН = &ИННК ";
								// 		НайденыПоляСинхронизации = ИСТИНА;
								// 	КонецЕсли;
							...
							ИначеЕсли Лев(НаименованиеКлючевогоПоля, 3) = "ИНН"
								И ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, НаименованиеКлючевогоПоля, ИмяТаблицыШаблонаФайла)) Тогда
					...
					Если НЕ ШаблонФайлаИспользоватьВКачествеКлючевыхТолькоПоляИзТаблицы Тогда
						Если ЭтоДокументНеСправочник Тогда
							...
							Если ДатаДок = Неопределено
								И ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "ДатаВходящегоДокумента", ИмяТаблицыШаблонаФайла)) Тогда
								...
								Если НЕ ДатаВхДок = Неопределено Тогда
									ДатаДок = ДатаВхДок;
								КонецЕсли;
							КонецЕсли;
							...
							Если НЕ ПустаяСтрока(ПериодДляДокументов) Тогда
							...
							Если НомерДок = Неопределено
								И ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "НомерПлатежногоДокумента", ИмяТаблицыШаблонаФайла)) Тогда
							...
							Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "НомерПлатежногоДокумента", ИмяТаблицыШаблонаФайла)) Тогда
								...
								Если ЗначениеЗаполнено(НомерВхДок) Тогда
									ОбъектОперации.НомерПлатежногоДокумента = НомерВхДок;
									Если НЕ ТекстЗапросаУжеСформирован Тогда
										ЗапДС.Текст = ЗапДС.Текст + ?(НайденыПоляСинхронизацииВЦикле, " И ", " ГДЕ") + " " + МетаданныеОбъекта + ".НомерПлатежногоДокумента = &НомерПлатежногоДокумента ";

										НайденыПоляСинхронизацииВЦикле = ИСТИНА;
									КонецЕсли;

									ЗапДС.УстановитьПараметр("НомерПлатежногоДокумента", ОбъектОперации.НомерПлатежногоДокумента);
								КонецЕсли;
							КонецЕсли;
						...
						Иначе
							...
							Если НЕ ТекстЗапросаУжеСформирован
								И МЕтаданные.Справочники[МетаданныеОбъекта].Иерархический Тогда
							...
							Иначе
								...
								Если ПустаяСтрока(НаименованиеСпр) Тогда
					...
					Если НайденыПоляСинхронизацииВЦикле Тогда
						...
						Если НЕ ВзДС.Пустой() Тогда
							...
							Иначе
								ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Пропущен существующий") + " " + НайденныйОбъектОперации);
						...
						Если НЕ ЗначениеДанных = Неопределено Тогда
							...
							Иначе
								...
								Если ШаблонФайлаВидШаблонаДляРегистровСведений
									ИЛИ НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, ИмяПоляВФайлеИзТаблицы, ИмяТаблицыШаблонаФайла)) Тогда
									...
									КонецЕсли;
								Иначе
									ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Реквизит не обнаружен") + ": " + ИмяПоляВФайлеИзТаблицы + ", Значение потеряно: " + ЗначениеДанных);
								КонецЕсли;
							КонецЕсли;
						КонецЕсли;
				...
				Если НЕ ОтказВЗаписи ТОгда
					...
					Если ЗначениеЗаполнено(ШаблонФайлаСоставлятьНаименованиеПоВыражениюИзИменПолей)
						И ПустаяСтрока(СтруктураИмпортаЭкспорта.ИмяТаблицыШаблонаФайла) Тогда

						Попытка
							ОбъектОперации.Наименование = СоставитьНаименованиеОбъектаПоШаблонуИзИменПолей(МассивДанныхСтрокиФайла, ШаблонФайла);
						Исключение
							ТекстОписаниеОшибки = ОписаниеОшибки();
							ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка формирования наименования") + ": " + ТекстОписаниеОшибки);
						КонецПопытки;
					КонецЕсли;
					...
					Если ЗначениеЗаполнено(ШаблонФайлаСоставлятьНаименованиеГруппыРодителяИзИменПолей)
						...
						Если ЗначениеЗаполнено(ИмяРодителя) Тогда
							...
							Если ЗначениеЗаполнено(РодительЭлемента)
								И ОбщийМодульПовтор.ЭтоГруппа(РодительЭлемента) Тогда
							...
							ИначеЕсли ШаблонФайлаСоздаватьЭлементыСправочникаНоменклатураВСлучаеИхОтсутствия Тогда
								...
								Исключение
									ТекстОписаниеОшибки = ОписаниеОшибки();
									ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи эл-та справочника") + ": " + РодительЭлемента + " " + ТекстОписаниеОшибки);
								КонецПопытки;  // записи
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
					...
					Если НЕ ОтказВЗаписи Тогда
						Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений Тогда
							...
							Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "НаименованиеПолное", ИмяТаблицыШаблонаФайла))
							...
							Попытка
								...
								Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "Дата"))
						...
						ИначеЕсли ЭтоЗагрузкаСписка
							...
							Попытка
								Если НЕ СтруктураИмпортаЭкспорта.РежимТестирования ТОгда
									...
									Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений Тогда
										...
										Если ШаблонФайлаПредназначенДляОбменаДокументамиДС
											И НЕ ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик)
											И ШаблонФайла.СоздаватьКонтрагентовПоОКПО Тогда

											ОКПОКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, ТипПоля.ОКПОКонтрагента);

											Если ОКПОКонтрагента = Неопределено Тогда
												ОКПОКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, ТипПоля.ОКПОКлиента);
												Если ОКПОКонтрагента = Неопределено Тогда
													ОКПОКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("ОКПО"));

													Если ОКПОКонтрагента = Неопределено Тогда
														Если ОбъектОперации.ПриходДенег Тогда
															ОКПОКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, ТипПоля.ОКПОПлательщика);
														Иначе
															ОКПОКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, ТипПоля.ОКПОПолучателя);
														КонецЕсли;
													КонецЕсли;
												КонецЕсли;
											КонецЕсли;

											Если НЕ ОКПОКонтрагента = Неопределено Тогда
												НаименованиеКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, ТипПоля.НаименованиеКлиента);
												Если НаименованиеКонтрагента = Неопределено Тогда
													НаименованиеКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, ТипПоля.НаименованиеОбъекта);
												КонецЕсли;

												Если НЕ НаименованиеКонтрагента = Неопределено Тогда
													Если ОбъектОперации.ПриходДенег Тогда
														НовыйКонтрагент = Справочники.Клиенты.СоздатьЭлемент();
														НовыйКонтрагент.Родитель = ШаблонФайла.ГруппаНовыхКлиентов;
													Иначе
														НовыйКонтрагент = Справочники.Поставщики.СоздатьЭлемент();
														НовыйКонтрагент.Родитель = ШаблонФайла.ГруппаНовыхПоставщиков;
													КонецЕсли;

													ИНН = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("ИНН"));
													...
													НовыйКонтрагент.НаименованиеДляПечати = НаименованиеКонтрагента;
													НовыйКонтрагент.ОКПО = ОКПОКонтрагента;
													Попытка
														НовыйКонтрагент.Записать();
														ОбъектОперации.КлиентПоставщик = НовыйКонтрагент.Ссылка;
													Исключение
													КонецПопытки;
												КонецЕсли;
											КонецЕсли;
										...
										Если ЭтоДокументНеСправочник
											И ШаблонФайлаПопытатьсяПроводитьДокументы Тогда
											...
											Попытка
												...
												КонецПопытки;
												Если ШаблонФайлаПредназначенДляОбменаДокументамиДС Тогда
													ОбъектОперации.ЗадатьВидДокумента();
													ОбъектОперации.ПодготовкаКПроведению(ЛОЖЬ);
												КонецЕсли;
											...
											Исключение
												...
												ТекстОписаниеОшибки = ОписаниеОшибки();
												ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке проведения документа") + ": " + ОбъектОперации + " " + ТекстОписаниеОшибки, ОбъектОперацииСсылка);
										...
										Иначе
											...
											Если ЭтоДокументНеСправочник
												И ШаблонФайлаПредназначенДляОбменаДокументамиДС Тогда
											...
											Если ЭтоДокументНеСправочник Тогда
												ОбъектОперации.Записать(РежимЗаписиДокумента.Запись);// устар , режимПроведенияДокумента.Неоперативный);
											Иначе
							...
							Исключение
								ТекстОписаниеОшибки = ОписаниеОшибки();
								ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при записи") + ": " + ТекстОписаниеОшибки, ОбъектОперацииСсылка);
				...
				Если НЕ ОтказВЗаписи
					...
					Для Каждого СтрокаРеквизита Из ШаблонФайлаДополнительноеЗаполнениеРеквизитов Цикл
						Попытка
							Если ПустаяСтрока(СтрокаРеквизита.ИмяТаблицыРеквизита) Тогда
						...
						Исключение
							ТекстОписаниеОшибки = ОписаниеОшибки();
							ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка установки реквизита") + ": " + ТекстОписаниеОшибки);
						КонецПопытки;
		...
		Если ШаблонФайла.УдалятьФайлПослеУдачногоИмпорта Тогда
			...
			Исключение
				ТекстОписаниеОшибки = ОписаниеОшибки();
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка удаления файла") + ": " + ТекстОписаниеОшибки);
			КонецПопытки;
		КонецЕсли;
...
Функция   ВыяснитьТипПлатежногоДокумента(ШаблонФайла, МассивДанныхСтрокиФайла)
	...
	СправочникиSZИмпортЭкспортФСПоля = Справочники.ПодсистемаИЭПоля;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ШаблонФайла.УсловиеДебетаПоле)
			И НЕ ПустаяСтрока(ШаблонФайла.УсловиеДебетаПоле) Тогда
			...
			Исключение // ошибка установки условия дебета по полю
				ТекстОписаниеОшибки = ОписаниеОшибки();
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка установки условия дебета по полю") + ": " + ШаблонФайла.УсловиеДебетаПоле + " - " + ТекстОписаниеОшибки);
			КонецПопытки;
		КонецЕсли;
...
// Произведение записи в журнал импортно-экпортных операций
...
Процедура ЗаполнитьКомментарииИТипы(Обязательно = ЛОЖЬ) Экспорт
	...
	Если Обязательно
		ИЛИ ПустаяСтрока(Справочники.ПодсистемаИЭПоля.ГодДатыДокумента.Комментарий) Тогда
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Дата отгрузки");
		ПолеОпределенное.ТипТут 	= "D";
		ПолеОпределенное.ДлинаТут 	= 8;
		ПолеОпределенное.ТочностьТут= 0;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Дата поступления");
		ПолеОпределенное.ТипТут 	= "D";
		ПолеОпределенное.ДлинаТут 	= 8;
		ПолеОпределенное.ТочностьТут= 0;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Доля стоимости");
		ПолеОпределенное.ТипТут 	= "N";
		ПолеОпределенное.ДлинаТут 	= 15;
		ПолеОпределенное.ТочностьТут= 2;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Номер входящего документа");
		ПолеОпределенное.ТипТут 	= "S";
		ПолеОпределенное.ДлинаТут 	= 25;
		ПолеОпределенное.ТочностьТут= 0;
		ПолеОпределенное.ЗаголовокПоляПоУмолчанию = "НомерВходящий";
		ПолеОпределенное.Записать();
		ПолеОпределенное = ТипПоля.ОКПОКонтрагента.ПолучитьОбъект();
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ОКПО Контрагента");
		ПолеОпределенное.ТипТут 	= "S";
		ПолеОпределенное.ДлинаТут 	= 25;
		ПолеОпределенное.ТочностьТут= 0;
		ПолеОпределенное.ПолеПринадлежитОбъектуТаблицы = ИСТИНА;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сумма расходов");
		ПолеОпределенное.ТипТут 	= "N";
		ПолеОпределенное.ДлинаТут 	= 15;
		ПолеОпределенное.ТочностьТут= 2;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Штрих код номенклатуры (Товара/карточки/списка) (ключевое)");
		ПолеОпределенное.ТипТут 	= "S";
		ПолеОпределенное.ДлинаТут 	= 254;
		ПолеОпределенное.ТочностьТут= 0;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сумма документа (без НДС)");
		ПолеОпределенное.ТипТут 	= "N";
		ПолеОпределенное.ДлинаТут 	= 15;
		ПолеОпределенное.ТочностьТут= 2;
		ПолеОпределенное.ПолеПринадлежитОбъектуТаблицы=ИСТИНА;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сумма документа (с НДС)");
		ПолеОпределенное.ТипТут 	= "N";
		ПолеОпределенное.ДлинаТут 	= 15;
		ПолеОпределенное.ТочностьТут= 2;
		ПолеОпределенное.ПолеПринадлежитОбъектуТаблицы=ИСТИНА;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Поле с определенным значением 0");
		ПолеОпределенное.ТипТут 		= "N";
		...
		ПолеОпределенное.Комментарий 	= ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Поле с определенным числовым значением 1");
		ПолеОпределенное.ТипТут 		= "N";
		ПолеОпределенное.ДлинаТут 		= 1;
		ПолеОпределенное.ТочностьТут	= 0;
		...
		ПолеОпределенное.Записать();
		ПолеОпределенное = ТипПоля.РодительНоменклатуры.ПолучитьОбъект();
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Группа номенклатуры");
		ПолеОпределенное.ТипТут 	= "S";
		ПолеОпределенное.ДлинаТут 	= 100;
		ПолеОпределенное.ТочностьТут= 0;
		ПолеОпределенное.ЗаголовокПоляПоУмолчанию = "РодительНоменклатуры";
		ПолеОпределенное.Записать();
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Дата платежа");
		ПолеОпределенное.ТипТут 	= "D";
		ПолеОпределенное.ДлинаТут 	= 8;
		ПолеОпределенное.ТочностьТут= 0;
		ПолеОпределенное.ЗаголовокПоляПоУмолчанию = "Дата";
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Назначение платежа");
		ПолеОпределенное.ТипТут 	= "S";
		ПолеОпределенное.ДлинаТут 	= 254;
		ПолеОпределенное.ТочностьТут= 0;
		...
		// ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Наименование Банка Организации");
		// ПолеОпределенное.ТипТут 	= "S";
		// ПолеОпределенное.ДлинаТут 	= 120;
		// ПолеОпределенное.ТочностьТут= 0;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сумма платежа валютная");
		ПолеОпределенное.ТипТут 	= "N";
		ПолеОпределенное.ДлинаТут 	= 15;
		ПолеОпределенное.ТочностьТут= 2;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сумма платежа валютная (Приход)");
		ПолеОпределенное.ТипТут 	= "N";
		ПолеОпределенное.ДлинаТут 	= 15;
		ПолеОпределенное.ТочностьТут= 2;
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сумма платежа валютная (Расход)");
		ПолеОпределенное.ТипТут 	= "N";
		ПолеОпределенное.ДлинаТут 	= 15;
		ПолеОпределенное.ТочностьТут= 2;
		...
		ПолеОпределенное.ПолеПринадлежитОбъектуТаблицы = ИСТИНА;
		ПолеОпределенное.Записать();
		ПолеОпределенное = ТипПоля.Склад.ПолучитьОбъект();
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Склад");
		ПолеОпределенное.ТипТут 	= "S";
		ПолеОпределенное.ДлинаТут 	= 50;
		ПолеОпределенное.ТочностьТут= 0;
		ПолеОпределенное.ЗаголовокПоляПоУмолчанию = "Склад";
		ПолеОпределенное.Записать();
		ПолеОпределенное = ТипПоля.СкладКуда.ПолучитьОбъект();
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Склад куда");
		ПолеОпределенное.ТипТут 	= "S";
		ПолеОпределенное.ДлинаТут 	= 50;
		ПолеОпределенное.ТочностьТут= 0;
		ПолеОпределенное.ЗаголовокПоляПоУмолчанию = "СкладКуда";
		ПолеОпределенное.Записать();
		...
		ПолеОпределенное.Комментарий = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вес номенклатуры");
		ПолеОпределенное.ТипТут 	= "N";
		...
		// ПолеОпределенное.Комментарий = "Коэффициент для единицы измерения (только в связке с кодом ИЛИ наименованием ед.изм.)";
		// ПолеОпределенное.ТипТут 	= "N";
		// ПолеОпределенное.ДлинаТут 	= 10;
		// ПолеОпределенное.ТочностьТут= 3;
		...
		ПолеОпределенное.Записать();
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Предопределенные поля системы импорта-экспорта обновлены."));
	КонецЕсли;
			...
			Если Импорт Тогда
				...
				Если ВыполненоЛиУсловиеФильтра(ЭлементПоведения.УсловиеДополнительногоПоведенияПриИмпорте, ЗначениеДанных, ЭлементПоведения.ЗначениеРеквизита) Тогда
					...
					Если ЭлементПоведения.ВыполнитьПрограммныйКод Тогда
						...
						Исключение
							ТекстОписаниеОшибки = ОписаниеОшибки();
							ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка выполнения кода поведения") + ": " + ТекстОписаниеОшибки);
						КонецПопытки;
					КонецЕсли;
				КонецЕсли;
			Иначе
...
// Возвращаяет значение константы "относительный адрес файлов"
Функция   КонстантыПодсистемаИЭПолучить() Экспорт
...
Процедура КонстантыПодсистемаИЭУстановить(Знач АдресФайла) Экспорт
...
Функция   НайтиКлиентаПоНаименованию(Знач НаименованиеКлиента, Знач ИскатьПоПолномуНаименованию = ЛОЖЬ)
...
Функция   НайтиКлиентаПоОКПО(Знач ДанныеОКПО) Экспорт
...
Функция   НайтиОрганизациюПоОКПО(Знач ДанныеОКПО) Экспорт
...
Функция   НайтиЭлементИлиДокументПоПолю(Знач ЭтоСправочник = ИСТИНА, Знач ТипЭлемента = "Номенклатура", Знач ЭлементДанныхФайла, Знач ИмяПоля = "Наименование", Знач ОбъектОперации = Неопределено) Экспорт

	Результат = Неопределено;

	Если ТипЭлемента = "Номенклатура"
		И НЕ ОбъектОперации = Неопределено
		...
		ИначеЕсли ИмяПоля = "НаименованиеДляПечати" Тогда
			Поле = ПредопределенноеЗначение("Перечисление.ПоляПредставленийНоменклатуры.НаименованиеДляПечати");
		ИначеЕсли ИмяПоля = "Артикул" Тогда
			Поле = ПредопределенноеЗначение("Перечисление.ПоляПредставленийНоменклатуры.Артикул");
		КонецЕсли;

		Попытка
			Контрагент = ОбъектОперации.КлиентПоставщик;
		Исключение
			Попытка
				Контрагент = ОбъектОперации.Клиент;
			Исключение
			КонецПопытки;
		КонецПопытки;

		Если ЗначениеЗаполнено(Контрагент)
			И НЕ Поле = Неопределено Тогда

			Результат = ОбщийМодульТоварСервер.НайтиНоменклатуруПоПредставлениюКонтрагента(Контрагент, Поле, ЭлементДанныхФайла);
		КонецЕсли;
	КонецЕсли;
...
Функция   НеВыводитьСообщениеОЗавершенииОбмена(Знач ШаблонФайла) Экспорт
...
Функция   ОбнаружитьДанныеПоВидуПоляИзСтроки(ОбъектОперации,
	...
	Если СубРеквизит > 0 Тогда
		Если ДанноеПолеКлючевоеДляПоискаИЗаписи Тогда
			...
			Исключение
				...
				Исключение
					ТекстОписаниеОшибки = ОписаниеОшибки();
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка") + ": " + ТекстОписаниеОшибки);
				КонецПопытки;
			КонецПопытки;
	...
	ИначеЕсли НЕ ПолеТаблицы.Предопределенный Тогда
		...
		ИначеЕсли ТипТутДляПроверки = Тип("Дата") Тогда
			...
			КонецПопытки;
		ИначеЕсли ТипТутДляПроверки = Тип("Неопределено") Тогда // оно многотипное
			ЗначениеДанных = СокрЛП(ЭлементДанныхФайла); // принудительно строка
		Иначе // справочник, документ, перечисление
			...
			Иначе
				Попытка
					...
					Если ПолеТаблицы.ПолеПринадлежитОбъектуТаблицы
						ИЛИ ПустаяСтрока(ИмяТаблицыШаблонаФайла) Тогда
						...
						Исключение
							...
							Исключение
								ТекстОписаниеОшибки = ОписаниеОшибки();
								ПоказатьСообщениеТут = ИСТИНА;
								СообщениеТекст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка") + ": " + ТекстОписаниеОшибки;
							КонецПопытки;
						КонецПопытки;
				...
				исключение
					ПоказатьСообщениеТут = ИСТИНА;
					ТекстОписаниеОшибки = ОписаниеОшибки();
					СообщениеТекст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Невозможно обнаружить") + ": " + ТекстОписаниеОшибки + ": ";
				КонецПопытки;
			КонецЕсли;
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ВладелецЭлемента Тогда
		Если НЕ ПустаяСтрока(ЭлементДанныхФайла) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.РодительЭлемента Тогда
		Если НЕ ПустаяСтрока(ЭлементДанныхФайла) Тогда
			Если НЕ ПустаяСтрока(ШаблонФайла.СоставлятьНаименованиеГруппыРодителяИзИменПолей) Тогда
				ИмяРодителя = СоставитьНаименованиеОбъектаПоШаблонуИзИменПолей(МассивДанныхСтрокиФайла, ШаблонФайла, 2);
				...
				Если ЗначениеЗаполнено(ИмяРодителя) Тогда
					...
					ИначеЕсли ШаблонФайла.СоздаватьЭлементыСправочникаНоменклатураВСлучаеИхОтсутствия Тогда
						...
						Исключение
							ТекстОписаниеОшибки = ОписаниеОшибки();
							ПоказатьСообщениеТут = ИСТИНА;
							СообщениеТекст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи родителя эл-та справочника") + ": " + РодительЭлемента + " " + ТекстОписаниеОшибки;
						КонецПопытки;  // записи
					Иначе
						Попытка
			...
			Иначе
				...
				Если ЗначениеЗаполнено(родитель)
					И ОбщийМодульПовтор.ЭтоГруппа(родитель) Тогда
				...
				Иначе
					Если СоздаватьЭлементыСправочникаНоменклатураВСлучаеИхОтсутствия Тогда
						...
						родительО.записать();
						ОбъектОперации.Родитель = родительО;
					Иначе
						ПоказатьСообщениеТут = ИСТИНА;
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.Производитель Тогда
		...
		Если НЕ ЗначениеЗаполнено(Производитель)
			...
			Исключение
				ТекстОписаниеОшибки = ОписаниеОшибки();
				ПоказатьСообщениеТут = ИСТИНА;
				СообщениеТекст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи эл-та справочника") + ": " + Производитель + " " + ТекстОписаниеОшибки;
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.РодительНоменклатуры Тогда
		ЭлементДанныхФайла = СокрЛП(ЭлементДанныхФайла);
		ТутРодитель = Справочники.Номенклатура.НайтиПоНаименованию(ЭлементДанныхФайла);

		Если ЗначениеЗаполнено(ТутРодитель)
			И ОбщийМодульПовтор.ЭтоГруппа(ТутРодитель) Тогда

			ОбъектОперации.Родитель = ТутРодитель;
		КонецЕсли;
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.АртикулНоменклатуры Тогда
		Если ДанноеПолеКлючевоеДляПоискаИЗаписи Тогда
			...
			Если НЕ ЗначениеНоменклатураЗаполнено Тогда
				...
				Если НЕ ПустаяСтрока(ЭлементДанныхФайла) Тогда
					РезНайтиНоменклатуруПоАртикулу = НайтиЭлементИлиДокументПоПолю(ИСТИНА, "Номенклатура", ЭлементДанныхФайла, "Артикул", ОбъектОперации);
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ШтрихКод Тогда				// еще не нашли?
		...
		Если НЕ ЗначениеНоменклатураЗаполнено  Тогда
			...
			Запрос4.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 Номенклатура.Ссылка
		...
		Если НЕ ЗначениеНоменклатураЗаполнено
			...
			Если НЕ ПустаяСтрока(ЭлементДанныхФайла) Тогда
				...
				ЗапНоменклатураПоШК.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 ШтрихКоды.Номенклатура
				|ИЗ РегистрСведений.ШтрихКоды КАК ШтрихКоды
				|ГДЕ ШтрихКоды.ШтрихКод = &ШтрихКод"; // ВЫБРАТЬ ПЕРВЫЕ 1
				...
				Иначе 	// запрос пуст!
					...
					Иначе
						ПоказатьСообщениеТут = ИСТИНА;
						СообщениеТекст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не обнаружена номенклатура по Штрих-коду") + ": ";
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;

		Если ТипЗнч(ОбъектОперации) = Тип("СправочникОбъект.Номенклатура")
			И НЕ ПустаяСтрока(ОбъектОперации.ОсновнойШтрихКод) Тогда

			ОбъектОперации.ОсновнойШтрихКод = ЭлементДанныхФайла;
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.Номенклатура Тогда		// еще не нашли?
		Если ДанноеПолеКлючевоеДляПоискаИЗаписи Тогда
			...
			Если НЕ ЗначениеНоменклатураЗаполнено Тогда
				...
				Если НЕ ПустаяСтрока(ЭлементДанныхФайла) Тогда
					РезНоменклатураПоНаименованию = НайтиЭлементИлиДокументПоПолю(ИСТИНА, "Номенклатура", ЭлементДанныхФайла, "Наименование", ОбъектОперации);

					Если ЗначениеЗаполнено(РезНоменклатураПоНаименованию)
						И НЕ ОбщийМодульПовтор.ЭтоГруппа(РезНоменклатураПоНаименованию) Тогда
		...
		Если ДанноеПолеКлючевоеДляПоискаИЗаписи Тогда
			...
			Если НЕ ЗначениеНоменклатураЗаполнено Тогда
				...
				Если НЕ ПустаяСтрока(ЭлементДанныхФайла) Тогда
					...
					ЗапНоменклатураПоКоду = Новый Запрос;
					ЗапНоменклатураПоКоду.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 Номенклатура.Ссылка
					|ИЗ Справочник.Номенклатура КАК Номенклатура
					|ГДЕ Номенклатура.Код = &Код"; // ВЫБРАТЬ ПЕРВЫЕ 1
					ЗапНоменклатураПоКоду.УстановитьПараметр("Код", ЭлементДанныхФайла);

					ВзНоменклатураПоКоду = ЗапНоменклатураПоКоду.Выполнить();

					Если НЕ ВзНоменклатураПоКоду.Пустой() Тогда
						РезНоменклатураПоКоду = ВзНоменклатураПоКоду.Выбрать();
						РезНоменклатураПоКоду.Следующий();
						Если ЭтоЗагрузкаСписка Тогда
							Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений Тогда
								ОбъектОперации = РезНоменклатураПоКоду.ссылка.ПолучитьОбъект();
							Иначе
								ОбъектОперации.Номенклатура = РезНоменклатураПоКоду.ссылка	;
							КонецЕсли;
						Иначе
							СтрокаТаблицыИлиЭлемент.Номенклатура = РезНоменклатураПоКоду.ссылка;
						КонецЕсли;
					Иначе 	// запрос пуст!
						Если СоздаватьЭлементыСправочникаНоменклатураВСлучаеИхОтсутствия Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ДатаДокумента Тогда
		...
		попытка
			...
			Если НЕ ПериодДляДокументов = ""
				И ((ЗначениеЗаполнено(ПериодДляДокументов.ДатаОкончания) И ЭлементДанныхФайла > ПериодДляДокументов.ДатаОкончания)
		...
		КонецПопытки;
	ИначеЕсли ПолеТаблицы = ТипПоля.НазначениеПлатежа Тогда
		ОбъектОперации.Комментарий = СокрЛП(ЭлементДанныхФайла);
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОПолучателя Тогда
		...
		Иначе
			ОбъектОперации.КлиентПоставщик = ОбщийМодульСервер.НайтиКонтрагентаПоОКПО(СокрЛП(ЭлементДанныхФайла));
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОКонтрагента
		И НЕ ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик) Тогда

		ОбъектОперации.КлиентПоставщик = ОбщийМодульСервер.НайтиКонтрагентаПоОКПО(СокрЛП(ЭлементДанныхФайла));
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОКлиента
		И НЕ ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик) Тогда
		...
		Попытка
			...
			Иначе
				ОбъектОперации.КлиентПоставщик = ОбъектОперацииКлиент ;
			КонецЕсли;
		...
		КонецПопытки;
	ИначеЕсли ПолеТаблицы = ТипПоля.НаименованиеКлиента
		И НЕ ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.СуммаСНДСДокумента Тогда
		...
		Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений
			И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "СуммаДокумента", "")) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ДеньДатыДокумента Тогда
		Попытка
			...
			Если НЕ ПустаяСтрока(ПериодДляДокументов) И
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ДатаПлатежа Тогда
		...
		Попытка
			...
			Иначе
				...
				Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений
					И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "ДатаВходящегоДокумента", "")) Тогда

					ОбъектОперации.ДатаВходящегоДокумента = ДатаВходящегоДокумента;
				Иначе
					Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "ДатаПоручения", "")) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОКлиента Тогда
		Если НЕ ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик) Тогда
			ОбъектОперации.КлиентПоставщик = НайтиКлиентаПоОКПО(ЭлементДанныхФайла);
		КонецЕсли;
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.НаименованиеКлиента Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик) Тогда
			ОбъектОперации.КлиентПоставщик = НайтиКлиентаПоНаименованию(ЭлементДанныхФайла);
		КонецЕсли;
	ИначеЕсли ПолеТаблицы =  ТипПоля.НомерСчета Тогда
		ОбъектОперации.ХранилищеДенег = ОбщийМодульСервер.НайтиХранилищеПоПараметрам(, , ЭлементДанныхФайла);
	...
	ИначеЕсли ПолеТаблицы =  ТипПоля.СуммаПлатежа Тогда
		...
		ОбъектОперации.Сумма = ЭлементДанныхФайла;
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ИсполняемыйКод Тогда
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			ПоказатьСообщениеТут = ИСТИНА;
			СообщениеТекст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка исполнения кода для файла") + ": " + ТекстОписаниеОшибки;
		КонецПопытки;
	ИначеЕсли ПолеТаблицы = ТипПоля.НаименованиеОбъекта Тогда
		...
		Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений
			И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации.ссылка, "НаименованиеПолное"))
			И НЕ ЗначениеЗаполнено(ОбъектОперации.НаименованиеПолное) Тогда
	...
	Если ВзятьЧисло Тогда
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не приводится к числовому значению") + ": " + СокрЛП(ЭлементДанныхФайла));
		КонецПопытки;
	ИначеЕсли ПоказатьСообщениеТут Тогда
		ОбщегоНазначения.СообщитьПользователю(СообщениеТекст + СокрЛП(ЭлементДанныхФайла));
	КонецЕсли;
...
КонецФункции // ОбнаружитьДанныеПоВидуПоляИзСтроки
...
Функция   ОбновитьЭлементНоменклатуры(СсылкаНаЭлемент, СтрокаТаблицыИлиЭлемент, ПолеТаблицы, ШаблонФайла, МассивДанныхСтрокиФайла, СтруктураИмпортаЭкспорта, ТаблицаОбработанныхОбъектов)
	...
	ПозицияНаименование = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.Номенклатура, "ПолеТаблицы");

	Если ПозицияНаименование = Неопределено Тогда
		ПозицияНаименование = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НаименованиеОбъекта, "ПолеТаблицы");
		Если ПозицияНаименование = Неопределено Тогда
			ПозицияНаименование = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("Наименование"), "ПолеТаблицы");

			Если ПозицияНаименование = Неопределено Тогда
				ПозицияНаименование = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("Номенклатура"), "ПолеТаблицы");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.АртикулНоменклатуры, "ПолеТаблицы");
	Если НЕ ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
		...
		КонецЕсли;
		// Иначе
		// 		ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.АртикулНоменклатурыСвойПотомУКлиента, "ПолеТаблицы");
	...
	ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.ШтрихКод, "ПолеТаблицы");

	Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
		ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("ШтрихКод"), "ПолеТаблицы");
		Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
			ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("ОсновнойШтрихКод"), "ПолеТаблицы");
		КонецЕсли;
	КонецЕсли;

	ОсновнойШтрихКод = Номенклатура.ОсновнойШтрихКод;
	...
	ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.Производитель, "ПолеТаблицы");

	Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
		ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("Производитель"), "ПолеТаблицы");
	КонецЕсли;
	...
	ЭтоПоКоду = ЛОЖЬ;
	ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.РодительНоменклатуры, "ПолеТаблицы");
	Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
		ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("Родитель"), "ПолеТаблицы");

		Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
			ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.РодительЭлемента, "ПолеТаблицы");
			Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
				ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.РодителяЭлементаКод, "ПолеТаблицы");
				ЭтоПоКоду = ИСТИНА;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
		РодительТут = ?(ЭтоПоКоду, Справочники.Номенклатура.НайтиПоКоду( МассивДанныхСтрокиФайла[ ПозицияДанныхОНоменклатуреВШаблоне.НомерСтроки - 1 ]), Справочники.Номенклатура.НайтиПоНаименованию( МассивДанныхСтрокиФайла[ ПозицияДанныхОНоменклатуреВШаблоне.НомерСтроки - 1 ]));

		Если ЗначениеЗаполнено(РодительТут)
			И РодительТут.ЭтоГруппа
			И НЕ номенклатура.Родитель = РодительТут Тогда

			номенклатура.Производитель = РодительТут;
			былиизменения = ИСТИНА;
		КонецЕсли;
	КонецЕсли;

	ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НоменклатурнаяГруппаНоменклатуры, "ПолеТаблицы");

	Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
		ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("НоменклатурнаяГруппа"), "ПолеТаблицы");
	КонецЕсли;
	...
	Если былиизменения Тогда
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи номенклатуры") + ": " + ОписаниеОшибки());
...
Функция   ОбъектОперацииТоварыКоличество(ОбъектОперации) Экспорт
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("У данного объекта нет таблицы Товары!"), ОбъектОперации, "Товары");
...
Функция   ОдинШаблонФайлаЗагрузки(ЭтоЭкспорт = Неопределено) Экспорт
	...
	|	ПодсистемаИЭ.Ссылка
	|ИЗ
	|	Справочник.ПодсистемаИЭШаблоны КАК ПодсистемаИЭ
	|ГДЕ
...
Функция   ОпределитьПолеДляШаблона(СтруктураПоля) Экспорт
	...
	Поле = Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию(СтруктураПоля.Наименование);
	Если НЕ ЗначениеЗаполнено(Поле) Тогда
		Если СтруктураПоля.СоздаватьНовоеПоле Тогда
			Поле = Справочники.ПодсистемаИЭПоля.СоздатьЭлемент();

			Если СтруктураПоля.СоздаватьВПапкеСпециальные Тогда
				Поле.Родитель = Справочники.ПодсистемаИЭПоля.Специальные ;
			КонецЕсли;
			...
			Попытка Поле.Записать();
				Поле = Поле.Ссылка;
			Исключение
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи эл-та справочника") + ": " + Поле + " " + ОписаниеОшибки());

				Возврат ПредопределенноеЗначение("Справочник.ПодсистемаИЭПоля.ПустаяСсылка");
			КонецПопытки; // записи Поле
		Иначе

			Возврат Справочники.ПодсистемаИЭПоля.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли;
...
Функция   ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, ТипПоля)
	...
	Если ЗначениеЗаполнено(СтрокаШаблона) Тогда
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Проверьте число колонок в файле для") + ": " + СтрокаШаблона + " (" + ОписаниеОшибки() + ")");
		КонецПопытки;
	Иначе
...
Функция   ПараметрКомандыТоварыКоличество(ПараметрКоманды) Экспорт
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Нет таблицы Товары") + ": " + ОписаниеОшибки());
...
Функция   ПеревестиСтрокуВЛатинскиеСимволы(Знач ЗначениеПеревода, Знач ПеревестиТакжеСпецСимволыИУстановитьВерхнийРегистр = ИСТИНА, Знач ПеревестиКТакимПоНаписанию = ЛОЖЬ, ПеревестиОшибкуНабора = ЛОЖЬ) Экспорт
...
Функция   ПередатьДанныеПоВидуПоля(СтрокаТаблицыИлиЭлемент,
	...
	ИначеЕсли СубРеквизит > 0 Тогда
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка") + ": " + ОписаниеОшибки());
		КонецПопытки;
	ИначеЕсли НЕ ПолеТаблицы.Предопределенный Тогда
		Попытка
			...
			ИначеЕсли ПолеТаблицы.ПолеПринадлежитОбъектуТаблицы Тогда
				Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, ИмяПоляВФайлеИзТаблицы, ИмяТаблицыШаблонаФайла)) Тогда
					ЗначениеДанных = ОбъектОперации[ ИмяПоляВФайлеИзТаблицы ];
				КонецЕсли;
			Иначе
				Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(СтрокаТаблицыИлиЭлемент, ИмяПоляВФайлеИзТаблицы, ИмяТаблицыШаблонаФайла)) Тогда
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка") + ": " + ОписаниеОшибки(), ОбъектОперации);
		КонецПопытки;
	ИначеЕсли ПолеТаблицы = ТипПоля.НомерСчета Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.РодительНоменклатуры Тогда
		...
		Если (ЭтоЗагрузкаСписка ИЛИ ИмяТаблицыШаблонаФайла="DBFНоменклатура")
			И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "Номенклатура", ИмяТаблицыШаблонаФайла)) Тогда

			ЗначениеДанных = СокрЛП(ОбъектОперации.Номенклатура.Родитель);
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.НоменклатурнаяГруппаНоменклатуры Тогда

		Если (ЭтоЗагрузкаСписка ИЛИ ИмяТаблицыШаблонаФайла="DBFНоменклатура")
			И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "НоменклатурнаяГруппа", ИмяТаблицыШаблонаФайла)) Тогда

			ЗначениеДанных = СокрЛП(ОбъектОперации.НоменклатурнаяГруппа);
		ИначеЕсли НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(СтрокаТаблицыИлиЭлемент, "Номенклатура", ИмяТаблицыШаблонаФайла)) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.АртикулНоменклатуры Тогда
		...
		Если (ЭтоЗагрузкаСписка ИЛИ ИмяТаблицыШаблонаФайла="DBFНоменклатура")
			И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "Артикул", ИмяТаблицыШаблонаФайла)) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.Производитель Тогда
		попытка
			...
			Если (ЭтоЗагрузкаСписка ИЛИ ИмяТаблицыШаблонаФайла="DBFНоменклатура")
				И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "Производитель", ИмяТаблицыШаблонаФайла)) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.НоменклатурыКод Тогда
		...
		Если (ЭтоЗагрузкаСписка ИЛИ ИмяТаблицыШаблонаФайла="DBFНоменклатура")
			И НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "Код", ИмяТаблицыШаблонаФайла)) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОКонтрагента Тогда
		...
		Если ШаблонФайла.СообщатьОНедостаточномЗаполненииПриЭкспортеДокументовДляПОБанка
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не указан ОКПО Контрагента у") + " " + ОбъектОперации);
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОКлиента Тогда
		Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "Клиент", ИмяТаблицыШаблонаФайла)) Тогда
		...
		Если ШаблонФайла.СообщатьОНедостаточномЗаполненииПриЭкспортеДокументовДляПОБанка
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не указан ОКПО Клиента у") + " " + ОбъектОперации);
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.НаименованиеКлиента Тогда
		Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "Клиент", ИмяТаблицыШаблонаФайла)) Тогда
			Если ЗначениеЗаполнено(ОбъектОперации.Клиент) Тогда
		...
		Иначе
			Если ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик) Тогда
				...
				Если ЗначениеЗаполнено(ОбъектОперации.КлиентПоставщик.НаименованиеПолное) Тогда
					ЗначениеДанных = СокрЛП(ОбъектОперации.КлиентПоставщик.НаименованиеПолное);
				Иначе
					ЗначениеДанных = СокрЛП(ОбъектОперации.КлиентПоставщик);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ШаблонФайла.СообщатьОНедостаточномЗаполненииПриЭкспортеДокументовДляПОБанка
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не указано Наименование Клиента у") + " " + ОбъектОперации);
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.ПропуститьДанноеПоле Тогда
		// !
	ИначеЕсли ПолеТаблицы = ТипПоля.НомерДокумента Тогда

		Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "Номер", ИмяТаблицыШаблонаФайла)) Тогда
		...
		Если ШаблонФайла.СообщатьОНедостаточномЗаполненииПриЭкспортеДокументовДляПОБанка
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не указан Номер документа у") + " " + ОбъектОперации);
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.ДеньДатыДокумента Тогда
		Если НЕ ПустаяСтрока(ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, "ДатаВходящегоДокумента", ИмяТаблицыШаблонаФайла)) Тогда
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ДатаДокумента Тогда
		...
		Если ШаблонФайла.СообщатьОНедостаточномЗаполненииПриЭкспортеДокументовДляПОБанка Тогда
			Если НЕ ЗначениеЗаполнено(ЗначениеДанных) Тогда
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не указана Дата у") + " " + ОбъектОперации);
			ИначеЕсли ЗначениеДанных < НачалоДня(ОбщийМодульСервисСервер.ПользователяТекущаяДата()) Тогда
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Дата у") + " " + СокрЛП(ОбъектОперации) + " - " + ЗначениеДанных);
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.ШтрихКод Тогда

		ЗапШК = Новый Запрос;
		ЗапШК.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 ШтрихКоды.ШтрихКод
		|ИЗ РегистрСведений.ШтрихКоды КАК ШтрихКоды
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ДатаПлатежа Тогда
		...
		Если ШаблонФайла.СообщатьОНедостаточномЗаполненииПриЭкспортеДокументовДляПОБанка Тогда
			Если НЕ ЗначениеЗаполнено(ЗначениеДанных) Тогда
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Не указана Дата платежа у") + " " + ОбъектОперации);
			ИначеЕсли ЗначениеДанных < НачалоДня(ОбщийМодульСервисСервер.ПользователяТекущаяДата()) Тогда
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Дата платежа у") + " " + СокрЛП(ОбъектОперации) + " - " + ЗначениеДанных);
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОКлиента Тогда
...
Функция   ВыполнитьВыгрузкуВФайл(СтруктураИмпортаЭкспорта) Экспорт
	...
	Если ШаблонФайла.ИспользоватьТолькоДляИмпорта Тогда
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Данный шаблон только для импорта!"), ОбъектОперацииСсылка);
	...
	ИначеЕсли НЕ ЭтоПредопределенныйШаблон
		И НЕ ПустаяСтрока(ИмяТаблицыШаблонаФайла) Тогда
	...
	Если ПриЭкспортеПередЗаписьюВыводитьОкноСПеречнемВыгрузкиДляОтбора Тогда
		...
		Для Каждого СледующееПолеВФайле Из ШаблонФайла.ПоследовательностьПолейВФайле Цикл
			...
			Если СледующееПолеВФайле.ПолеТаблицы = Справочники.ПодсистемаИЭПоля.ПропуститьДанноеПоле
	...
	Иначе
		...
		ИначеЕсли ШаблонФайлаФорматФайлаEXCEL Тогда
			...
			Исключение
				ТекстОписаниеОшибки = ОписаниеОшибки();
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Проблема с MS excel") + ": " + ТекстОписаниеОшибки, ОбъектОперацииСсылка);
	...
	Иначе
		...
		СуммироватьДанныеНомерПлатежногоДокумента = "";
		...
		ИначеЕсли ЭтоЗагрузкаСписка Тогда
				...
				Если НЕ ВыборкаОбъектов = Неопределено Тогда
					...
					Пока ВыборкаОбъектов.Следующий() Цикл
						...
						Для Каждого СледующееПолеФайлу Из ШаблонФайлаПоследовательностьПолейВФайле Цикл
							...
							Исключение
								ТекстОписаниеОшибки = ОписаниеОшибки();
								ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Неверный запрос ИЛИ имя результата") + " " + ШаблонФайла + "! " + ИмяПоляВФайле + ": " + ТекстОписаниеОшибки, ОбъектОперацииСсылка);
							КонецПопытки;
					...
					КонецЦикла; // выборка
				Иначе
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Нет результата запроса для выгрузки по") + " " + ШаблонФайла + "!", ОбъектОперацииСсылка);
			...
			Иначе
				...
				Если (ПустаяСтрока(ТипМетаОбъектаОбмена) И МетаДанные.Справочники.Найти(ТипОбъектаОперации) = Неопределено)
				...
				Если НЕ ВыборкаОбъектов = Неопределено Тогда
					...
					КонецЦикла; // выборка
				Иначе
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Нет объектов для выгрузки по") + " " + ШаблонФайла + ", " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("согласно условиям!"), ОбъектОперацииСсылка);
		...
		Если НЕ ПодсистемаИЭИмпортЭкспортОграничитьЧислоСтрокЗагрузки = 0
			И КоличествоСтрокДанныхДляФайла > ПодсистемаИЭИмпортЭкспортОграничитьЧислоСтрокЗагрузки Тогда
			...
			Если СуммироватьДанныеДокументовРасходДСПланПриВыгрузкеДляОдногоКлиентаОтОднойОрганизацииВТеченииСуток Тогда
				Если НЕ СуммироватьДанныеБанковскийСчет = Неопределено Тогда
					...
					Если СуммироватьДанныеБанковскийСчет = ОбъектОперации.БанковскийСчет
						...
						ИЛИ СуммироватьДанныеНомерПлатежногоДокумента = ОбъектОперации.НомерПлатежногоДокумента) Тогда
						...
						Если стрдлина(СуммироватьДанныеНазначение) > 250 Тогда
							...
							Если ШаблонФайлаСообщатьОНедостаточномЗаполненииПриЭкспортеДокументовДляПОБанка Тогда
								ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Назначение обрезано до 250 символов."));
							КонецЕсли;
						КонецЕсли;
				...
				Если ПустаяСтрока(СуммироватьДанныеНазначение) Тогда
				...
				СуммироватьДанныеНомерПлатежногоДокумента = ОбъектОперации.НомерПлатежногоДокумента;
			КонецЕсли;
	...
	Иначе
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка записи") + ": " + ТекстОписаниеОшибки, ОбъектОперацииСсылка);
		КонецПопытки;
...
Функция   ПолучитьАдресФайлаШаблона(Знач ШаблонШапки) Экспорт

	Возврат ШаблонШапки.АдресФайла;

КонецФункции // ПолучитьАдресФайлаШаблона(Объект.ШаблонШапки);

// Превратить значение в числовой тип
Функция   ПревратитьВЧисло(Знач ЗначениеДанных, Знач ШаблонФайла, Знач МожетБытьОтрицательным = ЛОЖЬ)
	...
	ОбщийМодульСервер.УдалитьНечисловыеСимволыИзТекста(ЗначениеДанных, ШаблонФайла.РазделительЧисел, МожетБытьОтрицательным);
...
Функция   ПривестиИзЧисла(Знач ЗначениеДанных, Знач ШаблонФайла)
	...
	Исключение
		ОбщийМодульСервер.УдалитьНечисловыеСимволыИзТекста(ЗначениеДанных, СимволРазделителяЧислаНашейБазы);
		Попытка
			ЗначениеДанных = Число(ЗначениеДанных);
		Исключение
			ЗначениеДанных = 0;
...
Функция   ПривестиКСимволамНаРусском(Знач ЭлементДанныхФайла, Знач ПеревестиОшибкуНабора = ЛОЖЬ, Знач ПеревестиКТакимПоНаписанию = ЛОЖЬ) Экспорт
	...
	Если ПеревестиОшибкуНабора Тогда
		...
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "~","Ё");
	ИначеЕсли ПеревестиКТакимПоНаписанию Тогда
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "q","к");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "w","в");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "e","е");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "r","р");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "t","т");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "y","у");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "u","ю");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "i","и");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "o","о");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "p","п");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "a","а");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "s","с");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "d","д");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "f","ф");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "g","г");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "h","х");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "j","ж");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "k","к");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "l","л");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "z","з");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "x","с");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "c","с");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "v","в");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "b","б");
		ЭлементДанныхФайла = стрзаменить(ЭлементДанныхФайла, "n","н");
...
Функция   ПроверитьДатаЛи(Знач ЭлементДанныхФайла, Знач РазделительДаты = " ")
...
Функция   ПроверкаНаличияРеквизитаУОбъектаОперации(ОбъектОперации, ИмяРеквизита, ИмяТаблицыШаблонаФайла = "")
	...
	Исключение
		...
		Исключение
			...
			Исключение
				ОбщегоНазначения.СообщитьПользователю("! " + ОбъектОперации + ": " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Нет реквизита") + " " + ИмяРеквизита + " " + ИмяТаблицыШаблонаФайла + ": " + ОписаниеОшибки());
			КонецПопытки;
		КонецПопытки;
	КонецПопытки;
...
Процедура ПроизвестиОценкуОбъектаБазыДанных(ОбъектШаблон, ТипМета, ТипОбъектаОперации, ТаблицаОбъектаОбмена = "", СоздаватьПоляШаблона = ЛОЖЬ, Режим = 0) Экспорт
	...
	Если СоздаватьПоляШаблона Тогда
		ГруппаПолейШаблонаПоОбъекту = Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию(ТипОбъектаОперации);

		Если НЕ ЗначениеЗаполнено(ГруппаПолейШаблонаПоОбъекту)
			ИЛИ НЕ ОбщийМодульПовтор.ЭтоГруппа(ГруппаПолейШаблонаПоОбъекту) Тогда

			ГруппаПолейШаблонаПоОбъекту = Справочники.ПодсистемаИЭПоля.СоздатьГруппу();
			...
			Исключение
				ГруппаПолейШаблонаПоОбъекту = Справочники.ПодсистемаИЭПоля.ПустаяСсылка();
				ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи группы справочника") + ": " + ГруппаПолейШаблонаПоОбъекту + " " + ОписаниеОшибки());
	...
	Иначе
		...
		Если СоздаватьПоляШаблона Тогда
			ГруппаПолейШаблонаПоТаблицеОбъекта = Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию(ТипОбъектаОперации + "-" + ТаблицаОбъектаОбмена, , ГруппаПолейШаблонаПоОбъекту);

			Если НЕ ЗначениеЗаполнено(ГруппаПолейШаблонаПоТаблицеОбъекта) ИЛИ НЕ ГруппаПолейШаблонаПоТаблицеОбъекта.ЭтоГруппа Тогда
				ГруппаПолейШаблонаПоТаблицеОбъекта= Справочники.ПодсистемаИЭПоля.СоздатьГруппу();
				...
				Исключение
					ГруппаПолейШаблонаПоТаблицеОбъекта = Справочники.ПодсистемаИЭПоля.ПустаяСсылка();
					ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи группы справочника") + ": " + ГруппаПолейШаблонаПоТаблицеОбъекта + " " + ОписаниеОшибки());
				КонецПопытки; // записи ГруппаПолейШаблонаПоОбъекту
			КонецЕсли;
		...
		Если Режим = 0 Тогда
			...
			Если ТипМета = "Справочники" Тогда
				НовоеПолеШаблона.ПолеТаблицы 	 = Справочники.ПодсистемаИЭПоля.НаименованиеВладельцаКарточки;
				НовоеПолеШаблона.ИмяПоляВФайле 	 = Справочники.ПодсистемаИЭПоля.НаименованиеВладельцаКарточки.ЗаголовокПоляПоУмолчанию;
				НовоеПолеШаблона.КомментарийПоля = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Наименование") + " " + ТипОбъектаОперации;
				НовоеПолеШаблона.ПолеТаблицы 	 = Справочники.ПодсистемаИЭПоля.КодВладельцаКарточки;
				НовоеПолеШаблона.ИмяПоляВФайле 	 = Справочники.ПодсистемаИЭПоля.КодВладельцаКарточки.ЗаголовокПоляПоУмолчанию;
				НовоеПолеШаблона.КомментарийПоля = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Код") + " " + ТипОбъектаОперации;
			Иначе
				НовоеПолеШаблона.ПолеТаблицы 	 = Справочники.ПодсистемаИЭПоля.НомерВладельцаШапки;
				НовоеПолеШаблона.ИмяПоляВФайле 	 = Справочники.ПодсистемаИЭПоля.НомерВладельцаШапки.ЗаголовокПоляПоУмолчанию;
	...
	Для Каждого РеквизитОбъекта Из ПереченьСтандартныхРеквизитов Цикл
		...
		Иначе // если режим = 0 Тогда
			...
			Если СоздаватьПоляШаблона Тогда
				ПолеДляШаблона = Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию(РеквизитОбъектаИмя, , ГруппаРодитель);
				Если НЕ ЗначениеЗаполнено(ПолеДляШаблона) Тогда
					ПолеДляШаблона = Справочники.ПодсистемаИЭПоля.СоздатьЭлемент();
					...
					Исключение
						ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи эл-та справочника") + ": " + ПолеДляШаблона + " " + ОписаниеОшибки());
					КонецПопытки; // записи ПолеДляШаблона
				Иначе
					НовоеПолеШаблона.ПолеТаблицы = ПолеДляШаблона;
				КонецЕсли;
			Иначе
				НовоеПолеШаблона.ПолеТаблицы = Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию(РеквизитОбъектаИмя);

				Если НЕ ЗначениеЗаполнено(НовоеПолеШаблона.ПолеТаблицы) Тогда
					НовоеПолеШаблона.ПолеТаблицы = Справочники.ПодсистемаИЭПоля.ПропуститьДанноеПоле ;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроизвестиОценкуФайла(ШаблонФайла, АдресФайла, ТипОценки) Экспорт
	...
	Если ШаблонФайлаФорматФайлаDBF Тогда
		...
		КонецЦикла;
	ИначеЕсли ШаблонФайлаФорматФайлаEXCEL Тогда
		ЭксельФайлИмпортаЭкспорта = Неопределено;
		попытка
			ЭксельФайлИмпортаЭкспорта = Новый COMОбъект("Excel.Application");
			ЭксельФайлИмпортаЭкспорта.Visible       = Ложь;
			ЭксельФайлИмпортаЭкспорта.DisplayAlerts = Ложь;
			Книга = ЭксельФайлИмпортаЭкспорта.Application.Workbooks.Open(АдресФайла);
			ТекЛист = Книга.ActiveSheet;// WorkSheets[0];
			СчетчикСтрок = ЧислоСтрокСНачалаФайлаКоторыеСледуетПропустить;
		Исключение
			Попытка
				ЭксельФайлИмпортаЭкспорта.Application.Quit();
			Исключение
			КонецПопытки;
			ЭксельФайлИмпортаЭкспорта = Неопределено;
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка работы с excel") + ": " + ОписаниеОшибки());
	...
	Попытка
		...
		СправочникиПодсистемаИЭПоля = Справочники.ПодсистемаИЭПоля;
		СчетчикПолей = 0;
		...
		Пока СчетчикПолей < КоличествоПолейФайла Цикл
						...
						Если УжеБылРеквизит1 Тогда
							Если УжеБылРеквизит4 Тогда
								Если НЕ УжеБылРеквизит8 Тогда
									// 									СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.СуммаНДС;
									УжеБылРеквизит8 = ИСТИНА;
								Иначе
									СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
								КонецЕсли;
							Иначе
								СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.Сумма;
								УжеБылРеквизит4 = ИСТИНА;
							КонецЕсли;
						Иначе
							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.Цена;
						...
						Если УжеБылРеквизит5 Тогда
							...
							Если УжеБылРеквизит2 ИЛИ Значение >99999 Тогда
								Если НЕ УжеБылРеквизит9 Тогда
									СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.АртикулНоменклатуры;
								...
								КонецЕсли;
							Иначе
								СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.Количество;
								УжеБылРеквизит2 = ИСТИНА;
							КонецЕсли;
						Иначе
							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ШтрихКод;
				...
				Исключение
					...
					Попытка
						...
						Если УжеБылРеквизит6 Тогда
							Если НЕ УжеБылРеквизит3 Тогда
								СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ДатаПоступления;
								УжеБылРеквизит3 = ИСТИНА;
							Иначе
								СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
							КонецЕсли;
						Иначе
							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ДатаДокумента;
					...
					Исключение
						Если НЕ УжеБылРеквизит7 Тогда
							...
							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.Номенклатура;
			...
			ИначеЕсли ШаблонФайлаПредназначенДляОбменаДокументамиДС(ШаблонФайла) Тогда
				...
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ВалютыДокументаКод;
				// 				УжеБылРеквизит1 = ИСТИНА;
				// 			Иначе
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
				// 			КонецЕсли;
				...
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ВалютаДокумента;
				// 				УжеБылРеквизит2 = ИСТИНА;
				// 			Иначе
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
				// 			КонецЕсли;
				...
				// 			Если НЕ УжеБылРеквизит3 Тогда
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ОКПООрганизации;
				// 				УжеБылРеквизит3 = ИСТИНА;
				// 			Иначе
				// 				Если НЕ УжеБылРеквизит4 Тогда

				// 					СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ОКПОКлиента;
				// 					УжеБылРеквизит4 = ИСТИНА;
				// 				Иначе
				// 					Если НЕ УжеБылРеквизит5 Тогда

				// 						СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ИННКлиента;
				// 						УжеБылРеквизит5 = ИСТИНА;
				// 					Иначе
				// 						Если НЕ УжеБылРеквизит6 Тогда

				// 							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ИННОрганизации;
				// 							УжеБылРеквизит6 = ИСТИНА;
				// 						Иначе
				// 							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
				// 						КонецЕсли;
				...
				// 			Если НЕ УжеБылРеквизит8 Тогда
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.МФОБанкаОрганизации;
				// 				УжеБылРеквизит8 = ИСТИНА;
				// 			Иначе
				// 				Если НЕ УжеБылРеквизит9 Тогда

				// 					СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.МФОБанкаКлиента;
				// 					УжеБылРеквизит9 = ИСТИНА;
				// 				Иначе
				// 					СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
				// 				КонецЕсли;
				...
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.НомерСчетаОрганизации;
				// 				УжеБылРеквизит0 = ИСТИНА;
				// 			Иначе
				// 				Если НЕ УжеБылРеквизит10 Тогда

				// 					СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.НомерСчетаКлиента;
				...
				// 			СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.НазначениеПлатежа;
				...
				// 			Если Значение <> цел(Значение ) Тогда
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.СуммаПлатежа;
				// 			ИначеЕсли НЕ УжеБылРеквизит7 Тогда
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.НомерПлатежногоДокумента;
				// 				УжеБылРеквизит7 = ИСТИНА;
				// 			Иначе
				// 				СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
				// 			КонецЕсли;
				...
				// 					СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ДатаПлатежа;
				...
				// 					СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.НаименованиеКлиента;
				// 					УжеБылоКлючевое = ИСТИНА;
				// 				Иначе
				// 					СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
				// 				КонецЕсли;
			...
			Иначе
				Попытка
					...
					Если Значение <> цел(Значение ) Тогда
						СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.Сумма;
					ИначеЕсли НЕ УжеБылРеквизит7 Тогда
						СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.НомерДокумента;
						УжеБылРеквизит7 = ИСТИНА;
					Иначе
						СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
					КонецЕсли;
				...
				Исключение
					...
					Исключение
						Если СокрЛП(ЗначениеВСтроке) = "" Тогда
							...
							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
						ИначеЕсли НЕ УжеБылоКлючевое Тогда
							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.НаименованиеОбъекта;
							УжеБылоКлючевое = ИСТИНА;
						Иначе
							СтрокаТаблицыШаблона.ПолеТаблицы = СправочникиПодсистемаИЭПоля.ПропуститьДанноеПоле;
						КонецЕсли;
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка") + ": " + ОписаниеОшибки(), ШаблонФайла);
	КонецПопытки;
...
Функция   СоздатьСтруктуруДляИмпортаЭкспорта(ОбъектОперации)
	...
	СтруктураИмпортаЭкспорта.Вставить("ШаблонФайла", Справочники.ПодсистемаИЭШаблоны.ПустаяСсылка());
...
Функция   СоздатьСтруктуруФормыПрогресса(Знач ЗаголовокФормы = "", Знач ШаблонФайла) Экспорт
	...
	СтруктураФормыПрогресса.Вставить("РассчитыватьСрокОкончанияОбмена", ШаблонФайла.РассчитыватьСрокОкончанияОбмена);
...
Функция   СоздатьЭлементНоменклатуры(СтрокаТаблицыИлиЭлемент, ПолеТаблицы, ШаблонФайла, МассивДанныхСтрокиФайла, СтруктураИмпортаЭкспорта, ТаблицаОбработанныхОбъектов)
	...
	Номенклатура.НоменклатурнаяГруппа 		= ШаблонФайла.НоменклатурнаяГруппа;
	Номенклатура.Родитель	= ШаблонФайла.ГруппаДляНовойНоменклатуры;
	ПозицияНаименование = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.Номенклатура, "ПолеТаблицы");
	...
	Иначе
		...
		Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
			ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭПоля.НайтиПоНаименованию("Родитель Код"), "ПолеТаблицы");
		КонецЕсли;
...
Функция   СтруктураДиалогФильтр(Знач ШаблонФайла, Знач ВсеФайлы = ЛОЖЬ) Экспорт
	...
	Если НЕ ВсеФайлы
		...
		СтруктураДиалогФильтр.Вставить("ДиалогФильтр", ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Файл") + " (*.DBf)|*.DBf");
	...
	ИначеЕсли НЕ ВсеФайлы
		...
		СтруктураДиалогФильтр.Вставить("ДиалогФильтр", ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Файл") + " (*.XLs)|*.XLs");
		СтруктураДиалогФильтр.Вставить("ДиалогРасширение", "XLs");
	Иначе // txt // ВсеФайлы
		СтруктураДиалогФильтр.Вставить("ДиалогФильтр", ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Файл текста") + " (*.*)|*.*");
		СтруктураДиалогФильтр.Вставить("ДиалогРасширение", "*");
	КонецЕсли;
...
Функция   УбратьНеподходящиеСимволы(Знач ТекстДляЗамен, Знач ЗаменятьНа = "") Экспорт
...
Функция   УбратьНеразрывныйПробел(Знач СтрокаСПробелом)
