									...
									Если НЕ ШаблонФайлаВидШаблонаДляРегистровСведений Тогда
										...
										Если ШаблонФайлаПредназначенДляОбменаДокументамиДС 
											...
											Если ОКПОКонтрагента = Неопределено Тогда
												...
												Если ОКПОКонтрагента = Неопределено Тогда
													ОКПОКонтрагента = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, Справочники.ПодсистемаИЭИмпортЭкспортФСПоля.НайтиПоНаименованию("ОКПО"));
													Если ОКПОКонтрагента = Неопределено Тогда
											...
											Если НЕ ОКПОКонтрагента = Неопределено Тогда
												...
												Если НЕ НаименованиеКонтрагента = Неопределено Тогда
													...
													ИНН = ОтобратьИзМассиваДанныхСтрокиПоПолю(МассивДанныхСтрокиФайла, ШаблонФайла, Справочники.ПодсистемаИЭИмпортЭкспортФСПоля.НайтиПоНаименованию("ИНН"));
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ШаблонФайла.УсловиеДебетаПоле)
			...
			Исключение //ошибка установки условия дебета по полю
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка установки условия дебета по полю") + ": " + ШаблонФайла.УсловиеДебетаПоле + " " + ОписаниеОшибки();
				Сообщение.Сообщить();
			КонецПопытки;
		КонецЕсли;
...
Процедура ЗаполнитьКомментарииИТипы(Обязательно = ЛОЖЬ) Экспорт	
	...
	Если Обязательно 
		ИЛИ ПустаяСтрока(Справочники.ПодсистемаИЭИмпортЭкспортФСПоля.ГодДатыДокумента.Комментарий) Тогда
...
Функция   ОбновитьЭлементНоменклатуры(СсылкаНаЭлемент, СтрокаТаблицыИлиЭлемент, ПолеТаблицы, ШаблонФайла, МассивДанныхСтрокиФайла, СтруктураИмпортаЭкспорта, ТаблицаОбработанныхОбъектов)
	...
	ЭтоПоКоду = ЛОЖЬ;
	ПозицияДанныхОНоменклатуреВШаблоне = ШаблонФайла.ПоследовательностьПолейВФайле.Найти(Справочники.ПодсистемаИЭИмпортЭкспортФСПоля.РодительНоменклатуры, "ПолеТаблицы");
	Если ПозицияДанныхОНоменклатуреВШаблоне = Неопределено Тогда
	...
	Если НЕ ЗначениеЗаполнено(Поле) Тогда
		Если СтруктураПоля.СоздаватьНовоеПоле Тогда
			...
			Исключение 	Сообщение = Новый СообщениеПользователю;
				...
				Возврат ПредопределенноеЗначение("Справочник.ПодсистемаИЭИмпортЭкспортФСПоля.ПустаяСсылка");	
