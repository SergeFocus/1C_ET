		...
		Пока СчетчикСтрок < (КоличествоСтрокФайла + ЧислоСтрокСНачалаФайлаКоторыеСледуетПропустить) Цикл
			...
			Если НЕ ОтказВЗаписи Тогда
				...
				Если НЕ ОтказВЗаписи ТОгда
					...
					Если ЗначениеЗаполнено(ШаблонФайлаСоставлятьНаименованиеГруппыРодителяИзИменПолей) Тогда
...
Процедура КонстантыПодсистемаИЭИмпортЭкспортФСУстановить(АдресФайла) Экспорт	
...
Функция   ОбнаружитьДанныеПоВидуПоляИзСтроки(ОбъектОперации, 
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.Производитель Тогда
		...
		Попытка
			ОбъектОперации.Производитель = Справочники.Производители.НайтиПоНаименованию(ЭлементДанныхФайла) ;//.SZМаркиНоменклатуры.НайтиПоНаименованию(ЭлементДанныхФайла) ;
			
			Если НЕ ЗначениеЗаполнено(ОбъектОперации.Производитель) И ЗначениеЗаполнено(ЭлементДанныхФайла) И шаблонФайла.СоздаватьМаркиНоменклатуры Тогда
				...
				Попытка 
					Если Не СтруктураИмпортаЭкспорта.РежимТестирования Тогда
						...
						ОбъектОперации.Производитель = Производитель.Ссылка;	
					КонецЕсли;
				Исключение 	
					ПоказатьСообщениеТут = Истина;
	...
	ИначеЕсли ПолеТаблицы = ТипПоля.ОКПОКлиента Тогда
		...
		//	ОбъектОперации.ОКПОПолучателя = ЭлементДанныхФайла; 		
		//КонецЕсли;
...
Функция   ПроверитьРавенствоИлиИзменить(БылиИзменения = Ложь, ОбъектПроверкиИзменения, ЗначениеПроверкиИзменения)
