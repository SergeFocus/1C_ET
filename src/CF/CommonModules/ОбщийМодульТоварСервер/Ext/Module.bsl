...
Процедура ЗарегистрироватьШтрихКод(Знач ШтрихКод, Знач Номенклатура, Знач ТипШтрихКодов = Неопределено, Знач СерияНоменклатуры = Неопределено) Экспорт
	...
	Если НЕ ПустаяСтрока(ШтрихКод) Тогда
		...
		Исключение 
			...
			ОбщегоНазначения.СообщитьПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи штрих-кода") + ": " + ШтрихКод + " - " + ТекстОписаниеОшибки);
		КонецПопытки;	
	КонецЕсли;
...
КонецФункции // НаименованиеЭтойНоменклатурнойГруппыНазначается
...
Процедура ПечатьАктаВыполненныхРабот(ТабДокРезультат, Знач СсылкаНаОбъектПечати, Знач ПараметрыПечати, Знач ВставлятьРазделительСтраниц = ЛОЖЬ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ОбъектКлиента) Тогда
			...
			ДокументДляПечати.Вывести(ОбъектРабот, Выборка.Уровень());
		КонецЕсли;
		...
		ДокументДляПечати.Вывести(ЗаголовокРабот);
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			ДокументДляПечати.Вывести(СтрокаРабот, ВыборкаТовары.Уровень());
		...
		ДокументДляПечати.Вывести(ПодвалРабот);
...
Процедура ПечатьНалоговойНакладной(ТабДокРезультат, Знач ПараметрыПечати, Знач ВставлятьРазделительСтраниц = ЛОЖЬ) Экспорт
	...
	Пока Выборка.Следующий() Цикл		
		...
		ОбщийМодульСервисСервер.ЗаменитьСвоиЗначенияПараметровПечати(Шапка);
		НалоговаяНакладная.Вывести(Шапка);
...
Функция   ПолучитьНоменклатуруПоШтрихКоду(Знач ДанныеШтрихкода, Знач ВСтруктуре = ЛОЖЬ, Знач ИспользоватьДополнительныеУстаревшиеШтрихКодыНоменклатуры = Неопределено, Знач ДатаДокумента = Неопределено) Экспорт
	...
	Иначе
		...
		Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьЕдиницыИзмеренияНоменклатуры") Тогда
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |ИЗ
			               |	РегистрНакопления.Товары.Остатки(&Дата, %1%) КАК ТоварыОстатки
			               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			               |			ЦеныСрезПоследних.Цена КАК Цена,
			               |			ЦеныСрезПоследних.Номенклатура КАК НоменклатураЦены
			               |		ИЗ
			               |			РегистрСведений.Цены.СрезПоследних(, ВидЦен = &ВидЦен) КАК ЦеныСрезПоследних
			               |		ГДЕ
			               |			ЦеныСрезПоследних.ЕдиницаИзмерения = &ПустаяЕдиницаИзмерения) КАК ВложенныйЗапрос
		...
		Иначе
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |			ЦеныСрезПоследних.Номенклатура КАК НоменклатураЦены
			               |		ИЗ
			               |			РегистрСведений.Цены.СрезПоследних(, ВидЦен = &ВидЦен) КАК ЦеныСрезПоследних) КАК ВложенныйЗапрос
			               |		ПО ТоварыОстатки.Номенклатура = ВложенныйЗапрос.НоменклатураЦены";
