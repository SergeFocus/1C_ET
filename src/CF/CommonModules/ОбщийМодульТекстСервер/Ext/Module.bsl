...
Процедура ЗагрузитьТекстыЯзыкаИзМакета(Знач ИмяМакета, Знач Язык, Знач УжеМонопольный = ЛОЖЬ, Знач ПроверятьНеобходимостьЗагрузить = ЛОЖЬ, Знач Перезаписать = Неопределено, Знач ПроверятьНаличиеВообще = ЛОЖЬ) Экспорт
	...
	Если НЕ НеЗагружать Тогда
		...
		Счетчик = 1;
		Пусто 	= 0;
		//	НачатьТранзакцию(); //из-за внутренней ошибки - запрета перезаписи. прочие алгоритмы медленные
		...
		Пока КоличествоСтрок > Счетчик Цикл
			...
			Если НЕ ПустаяСтрока(КодЯзыкаСтроки) Тогда				
				Если ВРег(СокрЛП(КодЯзыкаСтроки)) = КодЯзыка Тогда
					...
					ТЯ = РегистрыСведений.ТекстыНаДругихЯзыках.СоздатьМенеджерЗаписи();
					ТЯ.Активность 	  = ИСТИНА;   						
					ТЯ.ЭтоЭлементИнтерфейса = ИСТИНА;
					ТЯ.Язык 		  = Язык;
					ТЯ.ТекстНаРусском = ТекстНаРусском;
					ТЯ.МашинныйПеревод= ЛОЖЬ; // установлены автором или за счет пользователей ручной перевод
					ТЯ.ТекстНаЯзыке   = Макет.Область("R" + R + "C3").Текст;
...
КонецФункции // ПолучитьОбщееИННИлиОКПО
...
Функция   СтрокаТипаРеквизита(Знач Объектик, Знач реквизитэлементаимя) Экспорт
	...
	Иначе
		...
		Если НЕ НайденыМета = Неопределено Тогда
			//Если ТолькоКороткоеИмя Тогда
			//	РеквизитЭлементаТип = НайденыМета.Имя;
			//Иначе	
...
Функция   ПолучитьПереченьДляПроизвольнойФормы(ВходящаяТаблицаПеречня = Неопределено, Знач ВидПечатнойФормы, Знач ДляПеречняПараметров = ЛОЖЬ, Знач ДляТаблицы = "", Знач РазрядМетаданных = "", Знач ИмяМетаданных = "", Знач ДляДемонстрации = ЛОЖЬ) Экспорт
	...
	Иначе // для получения реквизитов - добавляются типовые		
		...
		Если ПустаяСтрока(ДляТаблицы) Тогда
			...
			Исключение //объект метаданных не предусматривает (напр. обраторка, отчет)
			КонецПопытки;	
		КонецЕсли;
			...
			Если ПустаяСтрока(ДляТаблицы)
				...
				Для Каждого РеквизитМетаданных Из ТаблицаОбъекта.Реквизиты Цикл
					...
					Если ВРег(ТипРеквизитаТаблицы) = "ЧИСЛО" Тогда // итоговые
						...
						СтрокаПеречня.ИмяТаблицы   	 = ""; // для объекта вцелом
						СтрокаПеречня.Программно   	 = ЛОЖЬ;
	...
	//	ТаблицаПеречня.Сортировать("НеРоднойРеквизит, ИмяТаблицы");
	
	//	ТаблицаПеречня.Свернуть("ИмяПрограммное, Описание, ИмяТаблицы, Программно, ТипПараметра, Программа, ЗначениеПараметра");
...
КонецФункции //ПолучитьПереченьДляПроизвольнойФормы
...
Функция   ОпределитьПроизвольнуюПечатнуюФорму(Знач ВидПечатнойФормы = Неопределено, Знач ДатаПечати = Неопределено, Знач СсылкаНаЭлемент = Неопределено, РазрядМетаданных = "", ИмяМетаданных = "") Экспорт
	...
	|ИЗ
	|	РегистрСведений.ЗаданныеПроизвольныеПечатныеФормы КАК ЗаданныеПроизвольныеПечатныеФормы
	|ГДЕ
	|	ЗаданныеПроизвольныеПечатныеФормы.ВидПечатнойФормы = &ВидПечатнойФормы
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЗаданныеПроизвольныеПечатныеФормы.Пользователь";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Иначе
				...
				Исключение 	
					...
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка выполнения проверки условия произвольной формы")+ ": " + ТекстОписаниеОшибки;
					Сообщение.Сообщить();					
				КонецПопытки;
			КонецЕсли;
	...
	Если Результат = Неопределено Тогда
		...
		|ИЗ
		|	Справочник.ПроизвольныеПечатныеФормы КАК ПроизвольныеПечатныеФормы
		|ГДЕ
		|	ПроизвольныеПечатныеФормы.ПечатнаяФорма = &ВидПечатнойФормы
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПроизвольныеПечатныеФормы.ПорядокВСпискеВыбора";
...
КонецФункции //ОпределитьПроизвольнуюПечатнуюФорму
...
Функция   ПолучитьВсеПараметрыПечатиОбъекта(Знач ОбъектДляПечати, Знач РазрядМетаданных, Знач ИмяМетаданных) Экспорт
	...
	Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьПроизвольныеПечатныеФормы") Тогда
		...
		Иначе		
			...
			Исключение //объект метаданных не предусматривает (напр. обработорка, отчет)
			КонецПопытки;
...
Функция   СформироватьДокументПоПроизвольнойФорме(Знач ПроизвольнаяПечатнаяФорма, Знач ВПП = Неопределено, Знач ТабДок = Неопределено, Знач ОбъектИлиСсылкаПечати = Неопределено, Знач РазрядМетаданных = "", Знач ИмяМетаданных = "", Знач СтруктураТаблиц = Неопределено, Знач МассивИменОтобранныхОбластейПечати = Неопределено, знач ФорматСтрокСоздан = ЛОЖЬ, Знач ЗапретитьСозданиеФорматаКолонок = ЛОЖЬ) Экспорт
	...
	Если НЕ Макет = Неопределено Тогда
			...
			Для Каждого ПараметрОбластейПечати Из ПараметрыОбластейПечати Цикл
				Если НЕ ПараметрОбластейПечати.Программно
					...
					Если НЕ ПараметрОбластейПечати.Программа = "ВПП." + ПараметрОбластейПечати.ИмяПрограммное Тогда
						Попытка // создаём переменную с таким же именем чтобы пользователь мог её использовать в программируемых параметрах
						...
						Исключение 	
							...
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка определения значения параметра области печати") + ": " + ТекстОписаниеОшибки + Символы.ПС + ПараметрОбластейПечати.Программа;
							Сообщение.Сообщить();							
						КонецПопытки;	
					КонецЕсли;
				КонецЕсли;
			...
			Если ИспользоватьПрограммируемыеПараметры Тогда
				...
				Для Каждого НужнаяПрограмма Из НужныеПрограммы Цикл
					Если НЕ ПустаяСтрока(НужнаяПрограмма.Программа) Тогда						
						...
						Исключение 	
							ТекстОписаниеОшибки = ОписаниеОшибки();
							Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка программы до макета") + ": " + ТекстОписаниеОшибки);
						КонецПопытки;	
					КонецЕсли;
				КонецЦикла;			
				...
				Для Каждого ПрограммируемыйПараметр Из ПроизвольнаяПечатнаяФорма.ПрограммируемыеПараметры Цикл
					Если ПустаяСтрока(ПрограммируемыйПараметр.ИмяТаблицы) Тогда
						...
						Исключение 	
							...
							Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка выполнения программного значения параметра области печати") + ": " + ОписаниеОшибкиТекст + Символы.ПС + ПрограммаПараметра);
						КонецПопытки;
				...
				Для Каждого НужнаяПрограмма Из НужныеПрограммы Цикл
					Если НЕ ПустаяСтрока(НужнаяПрограмма.Программа) Тогда						
						...
						Исключение 	
							...
							Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка программы очереди макета") + ": " + ОписаниеОшибкиТут);
						КонецПопытки;	
					КонецЕсли;
				КонецЦикла;
				...
				Если НЕ ПереченьЗаявленныхОбластей.Найти(ВРег(ОбластьМакета.ИмяПрограммное)) = Неопределено Тогда
					...
					Для Каждого НужнаяПрограмма Из НужныеПрограммы Цикл
						Если НЕ ПустаяСтрока(НужнаяПрограмма.Программа) Тогда						
							...
							Исключение 	
								ОписаниеОшибкиТекст = ОписаниеОшибки();
								Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка программы до вывода области") + " " + ОбщийМодульСервисСервер.ПолучитьКрасивоеНаименованиеОбъекта(ОбластьМакета.ИмяПрограммное) + ": " + ОписаниеОшибкиТекст);
							КонецПопытки;	
						КонецЕсли;
					КонецЦикла;
					...
					Иначе						
						...
						Если ПустаяСтрока(ОбластьМакета.ИмяТаблицы) Тогда
							...
							Если Лев(ВРег(ОбластьМакета.ИмяПрограммное), 5) = "ШАПКА" Тогда // шапка таблицы выйдет только в присутсвии таблицы
								...
								Если НЕ ОбластьПечатиТаблицы = Неопределено 
									...
									ТабДок.Вывести(ОбластьПечатиТаблицы);
									ОбластьПечатиТаблицы = Неопределено;
								КонецЕсли;
						...
						Иначе // печать таблицы
							...
							Если НЕ ТаблицаОбъектаПечати = Неопределено Тогда
								...
								Для Каждого СтрокаТаблицыОбъекта Из ТаблицаОбъектаПечати Цикл
										...
										Если НЕ ПараметрОбластейПечати.Программно Тогда
											...
											Если НЕ ПараметрОбластейПечати.Программа = "ВПП." + ПараметрОбластейПечати.ИмяПрограммное Тогда
												...
												Исключение 	
													...
													// Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка определения значения параметра области печати") + ": " + ОписаниеОшибки());
												КонецПопытки;
											КонецЕсли;
										КонецЕсли;
									КонецЦикла;
									...
									СтруктураРачетныхПолейТаблицы = Новый Структура;
									Для Каждого ПараметрПечати Из ПараметрыОбластейПечати Цикл
										Если ЗначениеЗаполнено(ПараметрПечати.ЗначениеПараметра) Тогда
											СтруктураРачетныхПолейТаблицы.Вставить(ПараметрПечати.ИмяПрограммное, ПараметрПечати.ЗначениеПараметра);
										КонецЕсли;
									КонецЦикла;
									
									ОбластьПечати.Параметры.Заполнить(СтруктураРачетныхПолейТаблицы);	
						...
						Если ИспользоватьПрограммируемыеПараметры Тогда
							...
							Для Каждого НужнаяПрограмма Из НужныеПрограммы Цикл
								Если НЕ ПустаяСтрока(НужнаяПрограмма.Программа) Тогда						
									...
									Исключение 	
										ОписаниеОшибкиТекст = ОписаниеОшибки();
										Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка программы после вывода области") + " " + ОбщийМодульСервисСервер.ПолучитьКрасивоеНаименованиеОбъекта(ОбластьМакета.ИмяПрограммное) + ": " + ОписаниеОшибкиТекст);
									КонецПопытки;	
								КонецЕсли;
							КонецЦикла;	
						КонецЕсли;
		...
		ИначеЕсли МассивИменОтобранныхОбластейПечати = Неопределено Тогда
			Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка! Области в макете не обнаружены."));
		КонецЕсли;
		...
		Если ИспользоватьПрограммируемыеПараметры Тогда
			...
			Для Каждого НужнаяПрограмма Из НужныеПрограммы Цикл
				Если НЕ ПустаяСтрока(НужнаяПрограмма.Программа) Тогда						
					...
					Исключение 	
						ОписаниеОшибкиТут = ОписаниеОшибки();
						Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка программы после макета") + ": " + ОписаниеОшибкиТут);
					КонецПопытки;	
				КонецЕсли;
			КонецЦикла;	
		КонецЕсли;
	...
	Если ПроизвольнаяПечатнаяФорма.ИспользоватьОтступыМасштабИПринтер Тогда
		УстановитьПараметрыПечатиДляПроизвольнойПечатнойФормы(ТабДок, ПроизвольнаяПечатнаяФорма);
	КонецЕсли;	
	
	Возврат ТабДок;
	
КонецФункции // СформироватьДокументПоПроизвольнойФорме
...
Процедура ДобавитьТиповыеПараметрыОбластейПечати(Знач ОбъектИлиСсылкаПечати, КлиентЗаголовок, Контрагент, ПредставлениеКонтрагента, ПараметрыОбластейПечати) Экспорт
	...
	ПараметрОбластейПечати.ЗначениеПараметра = СтруктураДополнительныПараметровМакета.ТекстВШапкеДокументовПриПечати;
	ПараметрОбластейПечати.Программно		 = ИСТИНА;
	ПараметрОбластейПечати = ПараметрыОбластейПечати.Добавить();
	ПараметрОбластейПечати.ИмяПрограммное 	 = "ТекстВПодвалеДокументовПриПечати";			
	ПараметрОбластейПечати.ЗначениеПараметра = СтруктураДополнительныПараметровМакета.ТекстВПодвалеДокументовПриПечати;
	ПараметрОбластейПечати.Программно		 = ИСТИНА;
...
КонецПроцедуры //СформироватьОбластьШтрихКода

Функция   ПреобразоватьВЧисло(Знач ЗначениеДляПреобразования = 0) Экспорт
	
	Попытка  //убарать попытку - выяснить состав
...
Функция   ЗаменитьПеременныеНаВПП(Знач ПрограммаПараметра = "", Знач ВПП, Знач ПрефиксИмяСтруктуры = "ВПП") Экспорт
	...
	Иначе
		Результат = ПрограммаПараметра; // встречается ВПП - задавал автор, всё учёл
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции //ЗаменитьПеременныеНаВПП
...
КонецФункции //ЭтотСимволКонецСлова

Функция   ПолучитьПереводДляЭлементаНаЯзыке(Знач ТекстНаРусском, Знач ОпределенныйЯзык = Неопределено) Экспорт
