// sza160314-0207 вычислить состояние
// sza151011-0228
// sza150429-1813
// sza150212-2115
// sza150209-1429
...
Процедура ПодобратьЗначениеПоВведенномуТексту(Элемент,
	Текст,
	ДанныеВыбора,
	Ожидание,
		...
		Пока СчетчикСтрок < Результат.СчетчикСтрок Цикл
			...
			Для Каждого КолонкаТаблицы Из СписокКолонокТаблицы Цикл
				...
				ИначеЕсли ТипВКолонке = Тип("Число") Тогда
					...
					СтроковоеЗначение = СтрЗаменить(ЗначениеЯчейки, Символы.НПП, "");
					ОбщийМодульСервер.УдалитьНечисловыеСимволыИзТекста(СтроковоеЗначение, , ИСТИНА);

					Попытка
						ЗначениеЯчейки = Число(ЗначениеЯчейки);
					Исключение
						УдалитьНечисловыеСимволыИзТекста(ЗначениеЯчейки, СимволРазделителяЧислаНашейБазы);
						Попытка
							ЗначениеЯчейки = Число(ЗначениеЯчейки);
						Исключение
							ЗначениеЯчейки = 0;
				...
				ИначеЕсли ТипВКолонке = Тип("СправочникСсылка.Номенклатура") Тогда
					...
					ИначеЕсли НоменклатураПредставлена = "ШтрихКод" Тогда
						Номенклатура = ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(ЗначениеЯчейки, ЛОЖЬ, ИСТИНА);
...
КонецФункции // Единица принадлежит номенклатуре

Функция   ЕстьНоваяВерсияКонфигурации()
	Возврат ОбщийМодульСервер.ЕстьНоваяВерсияКонфигурацииЭТ();
КонецФункции // Есть новая версия
...
КонецФункции // Использовать подключаемое оборудование
...
Функция   КамераУстановить() Экспорт
	...
	ПутьВнешнейКомпоненты = "ОбщийМакет.ДрайверWebCam";
	ИмяВнешнейКомпоненты  = "Video";
	ТипКомпоненты  	= ТипВнешнейКомпоненты.Native;
	ИмяОбъекта  	= "AddIn.Video.AddInNativeExtension";

	Если глВэбКамера <> Неопределено Тогда
		Возврат ИСТИНА;
	КонецЕсли;

	Если НЕ ПодключитьВнешнююКомпоненту(ПутьВнешнейКомпоненты, ИмяВнешнейКомпоненты ,ТипКомпоненты) Тогда
		УстановитьВнешнююКомпоненту(ПутьВнешнейКомпоненты);
		Если НЕ ПодключитьВнешнююКомпоненту(ПутьВнешнейКомпоненты, ИмяВнешнейКомпоненты, ТипКомпоненты) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура НайтиРекурсивно(КоллекцияЭлементов, МассивЭлементов, СпособПоиска, ЗначениеПоиска)
	...
	Для Каждого ЭлементОтбора Из КоллекцияЭлементов Цикл
		...
		Иначе
			...
			Если СпособПоиска = 2
				И ЭлементОтбора.Представление = ЗначениеПоиска Тогда
...
Функция   ОбновиласьВерсияКонфигурации()
	...
	Если НЕ ПустаяСтрока(НоваяВерсия) Тогда
		...
		Возврат ОбщийМодульСервисСервер.ОбновиласьВерсияКонфигурации();
	Иначе
...
Процедура ОкруглитьДанныеВТаблицеТовары(Документ,
	Знач Коэффициент = 0,
	Знач ОкруглятьТолькоВверх = ЛОЖЬ,
	Знач НаименованиеТаблицы = "Товары") Экспорт

	ОбщийМодульТоварСервер.ОкруглитьДанныеВТаблицеТовары(Документ, Коэффициент, ОкруглятьТолькоВверх, НаименованиеТаблицы);
КонецПроцедуры
...
Процедура ОтправитьПоEMAIL(Результат, Знач ИмяДокумента, Знач Емейл = "") Экспорт
	ПоказатьВопрос(Новый ОписаниеОповещения("ОтправитьПоEMAILСогласиеСОтправкой", ОбщийМодульКлиент, Новый Структура("ИмяДокумента, Результат", ИмяДокумента, Результат)), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вы уверены, что следует отправить отчет?"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура ОтправитьПоEMAILСогласиеСОтправкой(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ОчиститьТаблицуТовары(Документ) Экспорт
	ОбщийМодульТоварСервер.ОчиститьТаблицуТоварыНаСервере(Документ);
...
КонецФункции // Перевести текст на язык

Процедура ПересчитатьТаблицуТовары(Документ,
	Знач Коэффициент = 0,
...
Процедура ПоискОшибкиПриВводеТекстаПользователем(Знач ИмяСправочника,
	ДанныеВыбора,
	ТекстПользователя,
	...
	Если ДанныеВыбора = Неопределено Тогда
		ДанныеВыбора = ОбщийМодульСервер.НайтиЭлементПоИмени(ИмяСправочника, ТекстПользователя);
		Если НЕ ЗначениеЗаполнено(ДанныеВыбора) Тогда
			ТекстРус = ОбщийМодульКлиент.ПеревестиТекстНаЯзык(ТекстПользователя, 0);
			ДанныеВыбора = ОбщийМодульСервер.НайтиЭлементПоИмени(ИмяСправочника, ТекстРус);

			Если НЕ ЗначениеЗаполнено(ДанныеВыбора) Тогда
				ТекстАнг = ОбщийМодульКлиент.ПеревестиТекстНаЯзык(ТекстПользователя, 1);
				ДанныеВыбора = ОбщийМодульСервер.НайтиЭлементПоИмени(ИмяСправочника, ТекстАнг);

				Если ЗначениеЗаполнено(ДанныеВыбора) Тогда
					ТекстПользователя = ТекстАнг;
				Иначе
					Если ИмяСправочника = "НОМЕНКЛАТУРА" Тогда
						...
						Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьРеквизитАртикулНоменклатуры") Тогда
							ДанныеВыбора = ОбщийМодульТоварСервер.НайтиНоменклатуруПоАртикулу(ТекстПользователя, ЛОЖЬ);
						КонецЕсли;
						...
						Если НЕ ЗначениеЗаполнено(ДанныеВыбора)
							...
							ДанныеВыбора = ОбщийМодульТоварСервер.НайтиНоменклатуруПоШтрихКоду(ТекстПользователя, ЛОЖЬ);
						КонецЕсли;
						...
						Если НЕ ЗначениеЗаполнено(ДанныеВыбора)
							...
							ДанныеВыбора = ОбщийМодульТоварСервер.НайтиНоменклатуруПоПредставлениюКонтрагента(КлиентПоставщик, ПредопределенноеЗначение("Перечисление.ПоляПредставленийНоменклатуры.Наименование"), ТекстПользователя);

							Если НЕ ЗначениеЗаполнено(ДанныеВыбора) Тогда
								ДанныеВыбора = ОбщийМодульТоварСервер.НайтиНоменклатуруПоПредставлениюКонтрагента(КлиентПоставщик, ПредопределенноеЗначение("Перечисление.ПоляПредставленийНоменклатуры.НаименованиеДляПечати"), ТекстПользователя);

								Если НЕ ЗначениеЗаполнено(ДанныеВыбора)
									И ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьРеквизитАртикулНоменклатуры") Тогда

									ДанныеВыбора = ОбщийМодульТоварСервер.НайтиНоменклатуруПоПредставлениюКонтрагента(КлиентПоставщик, ПредопределенноеЗначение("Перечисление.ПоляПредставленийНоменклатуры.Артикул"), ТекстПользователя);
								КонецЕсли;
							КонецЕсли;
						КонецЕсли;
					...
					ИначеЕсли ИмяСправочника = "ПОСТАВЩИКИ"
						...
						ДанныеВыбора = ОбщийМодульСервер.НайтиПоТелефонуАдресуИЕмейл(ТекстПользователя, ИмяСправочника);
					КонецЕсли;
				КонецЕсли;
			Иначе
				ТекстПользователя = ТекстРус;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
КонецФункции // получить значение реквизита
...
КонецФункции // Получить список из метаданных

Функция   ПользователяТекущаяДата() Экспорт
	Возврат ОбщийМодульСервисСервер.ПользователяТекущаяДата();
КонецФункции // Пользователя текущая дата

Процедура ПриНачалеРаботыПользователяСБазойДанных() Экспорт
	...
	Если СтруктураОтветовСервераОНастройках.ПервыйЗапуск Тогда
		...
		ФормаПервогоЗапуска.ОткрытьМодально();
		ОбновиласьВерсияКонфигурации();
	Иначе
		Если ОбновиласьВерсияКонфигурации() Тогда
			ВывестиМакетЧтоНового();
		Иначе
			НоваяВерсия = ЕстьНоваяВерсияКонфигурации();
...
Процедура СобытиеФормы(ФормаСобытия, Знач ТипСобытия = 0, Знач СсылкаНаОбъект = Неопределено, Знач ЭтоФормаОбъекта = ЛОЖЬ) Экспорт
	...
	Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ВестиУчетРабочегоВремениСотрудников") Тогда
		ОбщийМодульКлиент.ПользовательНаРабочемМесте(ТипСобытия);
	КонецЕсли;
		...
		#Если НЕ ТолстыйКлиентОбычноеПриложение Тогда
			...
			Если СсылкаНаОбъект = Неопределено
				...
				Попытка
					...
					ИначеЕсли ПоложениеСтрокиПоискаЗначение = 4 Тогда
			...
			Если НЕ РамкиТекстовыхПолейВФормахРазличногоЦвета
				...
				Попытка
					...
					Для Каждого ЭлементФормы Из ФормаСобытия.Элементы Цикл ТипЭлементФормы = Тип(ЭлементФормы); Если ТипЭлементФормы = ТипПолеФормы Тогда ВидПоляФормыПереключатель = ЭлементФормы.Вид = ВидПоляФормы.ПолеПереключателя; Если НЕ ВидПоляФормыПереключатель Тогда ЭлементФормы.ЦветРамки = ЦветРамки; КонецЕсли; КонецЕсли; КонецЦикла;
				Исключение // 8,2,18
				КонецПопытки;
			КонецЕсли;
...
Функция   СохранитьДанныеПроизвольнойФормы(Знач Модифицированность, Отказ = Неопределено) Экспорт
...
Функция   СформироватьАдресМестонахождения(Знач Регион,
	Знач Локация,
	Знач ПочтовыйИндекс = "",
...
Функция   УдалитьНечисловыеСимволыИзТекста(ТекстВОбработке,
	...
	ТекстВОбработке = СокрЛП(ТекстВОбработке);
	ТекстВОбработке = СтрЗаменить(ТекстВОбработке, НеверныйСимволРазделителяЧисла, РазрешенныйСимволРазделитель);
	ТекстВОбработке = СтрЗаменить(ТекстВОбработке, Символы.НПП, "");
	ТекстВОбработке = СтрЗаменить(ТекстВОбработке, Символы.Таб, "");
	ИтоговаяСтрока = "";
	ЧислоСимволов  = СтрДлина(ТекстВОбработке);
	СчетчикСимволов= 1;

	Пока СчетчикСимволов <= ЧислоСимволов Цикл
		СимволСтроки = Сред(ТекстВОбработке, СчетчикСимволов, 1);
	...
	Возврат ТекстВОбработке;

КонецФункции // Удалить нечисловые символы
...
КонецФункции //Установить компоненту WebCam

Процедура УстановитьПараметрыСеансаЭлементарнаяТорговля(Знач ВерсияПлатформы = "", Знач Принудительно = ЛОЖЬ)
	ОбщийМодульСервисСервер.УстановитьПараметрыСеансаЭлементарнаяТорговля(ВерсияПлатформы, Принудительно);
КонецПроцедуры // Установить параметры сеанса
...
Функция   ВычислитьВозраст(Знач Дата, Знач ВернутьЦелыеГоды = ИСТИНА) Экспорт

	Возраст = 0;

	Если НЕ Дата = '00010101' Тогда
		Если ВернутьЦелыеГоды Тогда
			Возраст = Цел((ОбщийМодульСервисСервер.ПользователяТекущаяДата() - Дата) / 31556925.9936);
		Иначе
			Возраст = (ОбщийМодульСервисСервер.ПользователяТекущаяДата() - Дата) / 31556925.9936;
		КонецЕсли;
	КонецЕсли;

	Возврат Возраст;

КонецФункции //Вычислить возраст

Процедура ВычислитьСостояние(Знач ТекстСостояния = "", Знач ТекстЗаголовка = "", Знач НаименованиеКартинки = "", Знач ВсегоСтрок = 0, Знач СчетчикСтрок = 0, Знач ДатаНачалаСостояния = '00010101000000', Знач ДатаПоследнегоСостояния ='00010101000000', Знач ПоследнийПроцент = -1, Знач ВыводитьВТекстеНомерСтроки = ЛОЖЬ) Экспорт

	ПользователяТекущаяДата = ОбщийМодульКлиент.ПользователяТекущаяДата();

	Если НЕ ПользователяТекущаяДата = ДатаПоследнегоСостояния Тогда
		Если НЕ ВсегоСтрок = 0 Тогда
			Процент = ВсегоСтрок / 100;
			ТекущийПроцент = СчетчикСтрок / Процент;
			ПрогрессСостояния = Окр(ТекущийПроцент);

			Если ВыводитьВТекстеНомерСтроки
				ИЛИ НЕ ПоследнийПроцент = ПрогрессСостояния Тогда

				ПоследнийПроцент = ПрогрессСостояния;
				ТекстСостояния = ?(ПустаяСтрока(ТекстСостояния), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Идет процесс.."), ТекстСостояния);
				Если ВыводитьВТекстеНомерСтроки Тогда
					ТекстСостояния = ТекстСостояния + " " + Формат(СчетчикСтрок, "ЧРГ=' '; ЧГ=0") + "/" + Формат(ВсегоСтрок, "ЧРГ=' '; ЧГ=0")
				КонецЕсли;
				Состояние(, ПрогрессСостояния, ?(ПустаяСтрока(ТекстЗаголовка), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ждите.."), ТекстЗаголовка), ?(ПустаяСтрока(НаименованиеКартинки), БиблиотекаКартинок.АнимацияСерая, БиблиотекаКартинок[НаименованиеКартинки]));
			КонецЕсли;
		КонецЕсли;
		ДатаПоследнегоСостояния = ПользователяТекущаяДата;
	КонецЕсли;

КонецПроцедуры

Процедура РеакцияНаПрочиеСобытияФормы(Знач КомандаСобытияФормы, ФормаССобытием) Экспорт
	
	Если КомандаСобытияФормы.Имя = "ВвестиКомментарий" Тогда
		ФормаССобытием.Элементы.ВвестиКомментарий.Видимость = ЛОЖЬ;
		ЭлементКомментарий = ФормаССобытием.Элементы.Найти("Комментарий");
		
		Если НЕ ЭлементКомментарий = Неопределено
			И ЭлементКомментарий.Вид = ВидПоляФормы.ПолеВвода Тогда
			
			ЭлементКомментарий.Видимость = ИСТИНА;
			ФормаССобытием.ТекущийЭлемент = ЭлементКомментарий;	
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Процедура ПользовательНаРабочемМесте(Знач ТипСобытия) Экспорт
	...
	Если НЕ глВремяПоследнегоСобытия = '00010101000000'  Тогда
		...
		Если ПериодМолчания > ЧислоСекунд Тогда
			ОбщийМодульСервисСервер.ЗавершитьПериодРаботыПользователя(ТипСобытия, глВремяНачалаРаботыПользователя, глВремяПоследнегоСобытия + ЧислоСекунд); // Есть завершенный период
			глВремяНачалаРаботыПользователя = ТекущаяДатаСейчас;
		КонецЕсли;
		
		глВремяПоследнегоСобытия = ТекущаяДатаСейчас;
	Иначе
		Если глВремяНачалаРаботыПользователя = '00010101000000' Тогда
			глВремяНачалаРаботыПользователя = ТекущаяДатаСейчас;
		Иначе
