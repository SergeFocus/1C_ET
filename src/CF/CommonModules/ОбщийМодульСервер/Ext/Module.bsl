...
&НаСервере
Функция   РассчитатьНовуюЦену(Знач Номенклатура, Знач СтараяЦена, Знач ВидЦен, Знач Дата, Знач Ссылка = Неопределено, Знач ЕдиницаИзмерения = Неопределено) Экспорт
	
	Цена = СтараяЦена;	
	
	Если ЗначениеЗаполнено(ВидЦен) 
		и ВидЦен.Зависимая 
		И ЗначениеЗаполнено(ВидЦен.ОсновнойВидЦен) Тогда
		...
		Если ОбщийМодульПовтор.получитьЗначениеНастройкиИлиКонстанты("НазначатьРазличныеСпособыФормированияЦеныДляНоменклатурныхГрупп")
			...
			СтрокаНоменклатурнойГруппы = ВидЦен.СпособыФормированияЦеныДляНоменклатурныхГрупп.Найти(НоменклатурнаяГруппа, "НоменклатурнаяГруппа");
			Если НЕ СтрокаНоменклатурнойГруппы = Неопределено Тогда
		...
		Если СтрокаНоменклатурнойГруппы = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВидЦенФормулаРасчетаОтБазовогоВидаЦены) Тогда
			Попытка 
				Цена = ПолучитьСложнуюЦену(Номенклатура, ВидЦен.ОсновнойВидЦен, Дата, Истина, , , , , ЕдиницаИзмерения);
...
&НаСервере
Процедура УстановитьЦенуИВсеЗависимые(Знач ВидЦен, Знач СтрокаТовара, Знач Ссылка = Неопределено, Знач Комментарий, Знач Дата, Знач ТолькоЗависимые = Ложь, Знач МассивВидовЦен = Неопределено, Знач наУдаление = Ложь, Знач первыйУровень = истина, Знач БезЗависимых = Ложь, Знач ЕдиницаИзмерения = Неопределено) Экспорт
			...
			если НЕ РезультатЗапроса.Пустой() тогда
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					если наУдаление тогда
						РасчетнаяЦена = 0;
					иначе
						РасчетнаяЦена = ОбщийМодульСервер.РассчитатьНовуюЦену(Номенклатура, Цена, ЗависимыйВидЦен, Дата, Ссылка, ЕдиницаИзмерения);	
					КонецЕсли;
					...
					если НЕ ОбщийМодульПовторВТеченииСервера.ЦенаУжеСтоит(ЗависимыйВидЦен, Номенклатура, Дата, РасчетнаяЦена, Ложь, Ссылка, ЕдиницаИзмерения) тогда
						Попытка 					
							...
							КонецЕсли;
							УстановитьЦенуИВсеЗависимые(ЗависимыйВидЦен, СтрокаТовара, Ссылка, Комментарий, Дата, Истина, МассивВидовЦен, наУдаление, ЛОЖЬ, , ЕдиницаИзмерения) ; //рекурсия для всех зависимых
...
Процедура ПроизвестиПересчетВсехЦен(Знач ВидЦен) Экспорт
	...
	|	ЦеныСрезПоследних.Номенклатура,
	|	ЦеныСрезПоследних.ЕдиницаИзмерения
	|ИЗ РегистрСведений.Цены.СрезПоследних(&ТекущаяДата, ВидЦен = &ВидЦен) КАК ЦеныСрезПоследних ";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ОбщийМодульСервер.УстановитьЦенуИВсеЗависимые(ВидЦен.ОсновнойВидЦен.Ссылка, СтрокаТовара, , ВидЦен.Комментарий, ВыборкаДетальныеЗаписи.Период, Истина, , , , , ВыборкаДетальныеЗаписи.ЕдиницаИзмерения);
...
Функция   ПолучитьСтоимостьВыполненныхРабот(Знач ДатаНачала, Знач ДатаОкончания, Знач Сотрудник = Неопределено, Знач ЗаКомплектацию = Ложь, Знач ЗаОказанныеУслугиЗаВремя = Ложь, Знач ПроцентЗаОказанныеУслугиЗаВремя = ложь) Экспорт
	...
	Если ПроцентЗаОказанныеУслугиЗаВремя Тогда
		Если ЗначениеЗаполнено(Сотрудник) Тогда
			...
			|ГДЕ
			|	ОказанияУслугЗаВремя.Ссылка.Проведен = ИСТИНА
	...
	ИначеЕсли ЗаКомплектацию Тогда
		Если ЗначениеЗаполнено(Сотрудник) Тогда
			...
			|ИЗ
			|	Документ.Комплектация.Работы КАК КомплектацияРаботы
			|ГДЕ
			|	КомплектацияРаботы.Ссылка.Проведен = ИСТИНА
	...
	Иначе
		Если ЗначениеЗаполнено(Сотрудник) Тогда
			...
			|ИЗ
			|	Документ.ВыполненияРабот.Товары КАК ВыполненияРаботРаботы
			|ГДЕ
			|	ВыполненияРаботРаботы.Ссылка.Проведен = ИСТИНА
...
&НаСервере
Функция   ПолучитьТаблицыЦенСписанияИОстатков(Знач ссылка = Неопределено, Знач склад, Знач дата, Знач ОстаткиСледуетПроверять, Знач Номенклатура = Неопределено, Знач ИмяТаблицы = "Товары") Экспорт
...
Функция   ДобавитьШапкуОрганизации(ТабДок, Знач Организация) Экспорт
	...
	Если ОбщийМодульПовтор.ПлатнаяВерсия() Тогда
		...
		Если ЗначениеЗаполнено(Организация.КорреспондентскийСчет) Тогда
...
Функция   СформироватьОтчетРуководителюНаСервере(Знач СтруктураУсловий) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаНачалаОтчета) 
		...
		Если ОбщийМодульПовтор.ПолучитьПараметрСеанса("ИспользоватьСложныйМеханизмЦенПС") Тогда
			...
			|ИЗ РегистрНакопления.ВаловаяПрибыль.Обороты(&ДатаНачалаОтчета, &ДатаОкончанияОтчета, , Склад = &Склад) КАК ВаловаяПрибыльОбороты";
...
Функция   ПолучитьПустоеЗначениеДополнительногоРеквизита(Знач ТипДополнительногоРеквизита, Знач ЭтоОбъект = Ложь) Экспорт
	...
	ИначеЕсли ТипДополнительногоРеквизита = Перечисления.ТипыДополнительныхРеквизитов.ЗначениеИзСправочника Тогда
		Результат = Справочники.НаборЗначенийДополнительныхРеквизитов.ПустаяСсылка();
...
Функция   получитьЗначениеРеквизита(Знач Элемент, Знач ИмяРеквизита) Экспорт
...
Функция   НайтиХранилищеПоНомеруСчета(Знач НомерСчета = "") Экспорт
	
	РезультатНайтиХранилищеПоНомеруСчета = ПредопределенноеЗначение("Справочник.ХранилищаДенег.ПустаяСсылка");
	...
	|ГДЕ ХранилищаДенег.НомерСчета = &НомерСчета
	...
	если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			РезультатНайтиХранилищеПоНомеруСчета = ВыборкаДетальныеЗаписи.ссылка;
		КонецЦикла;
		
	КонецЕсли;	
	
	Возврат РезультатНайтиХранилищеПоНомеруСчета;
	
КонецФункции //НайтиХранилищеПоНомеруСчета
...
Процедура ОбеспечитьСписокОтборов(СписокФормы, Знач ДопТип = "") Экспорт
	
	СписокОтборов = ОбщийМодульПовтор.ПолучитьОтборыДоступа(СписокФормы.ОсновнаяТаблица, , ДопТип);
...
Функция   ПечатьТоварногоЧека(Знач Документ, Знач ЭтоВозврат = Ложь, Знач Клиент = Неопределено, Знач ИмяТаблицы = "Товары", Знач ВидОплаты = Неопределено, Знач ТаблицаОплаты = "", Знач Кассир = Неопределено) Экспорт
	...
	Если НЕ СтрогоеИмяНефискальногоПринтераЧеков = "" Тогда
		...
		Если ЭтоВозврат Тогда
			...
			ОблЗаголовок.Параметры.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Чек ВОЗВРАТА");
			ОблПодвал = МакетЧека.ПолучитьОбласть("ОблПодвалВозврат");			
			
		Иначе
			ОблЗаголовок = МакетЧека.ПолучитьОбласть("ОблЗаголовок");
			ОблЗаголовок.Параметры.Заголовок = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ЧЕК");
			Если ВидОплаты = Перечисления.ФормыОплаты.Наличные Тогда
				ОблЗаголовок.Параметры.ФормаОплаты = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("НАЛИЧНЫМИ");
			ИначеЕсли ВидОплаты = Перечисления.ФормыОплаты.ПлатежнаяКарта Тогда
				ОблЗаголовок.Параметры.ФормаОплаты = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ПЛАТ.КАРТОЙ");
			ИначеЕсли ВидОплаты = Перечисления.ФормыОплаты.Кредит Тогда
				ОблЗаголовок.Параметры.ФормаОплаты = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("КРЕДИТОМ");
			ИначеЕсли ВидОплаты = Перечисления.ФормыОплаты.Сертификат Тогда
				ОблЗаголовок.Параметры.ФормаОплаты = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("СЕРТИФИКАТОМ");	
			КонецЕсли;
		...
		ОблЗаголовок.Параметры.Организация 		= Организация;
		ОблЗаголовок.Параметры.Номер 			= Документ.Номер;
		ОблЗаголовок.Параметры.Контрагент 		= Клиент;
		...
		Если НЕ ЭтоВозврат Тогда
			ОблПодвал.Параметры.ИтогоСтрока = ОбщийМодульТекстСервер.СформироватьСуммуПрописью(Сумма, Документ.Валюта);
		КонецЕсли;
	...
	Иначе		
		Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ТОВАРНЫЙ ЧЕК") + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("от") + " " + Формат(Дата, "ДЛФ=ДД") + Символы.ПС + "--------------------------------" + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Организация") + ": " + Организация + Символы.ПС +
		ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Вид операции") + ": " + ?(ЭтоВозврат, ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Возврат"), ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Продажа")) + Символы.ПС;
		
		Если ЗначениеЗаполнено(Клиент) Тогда
			Текст = Текст + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Клиент") + ": " + Клиент + Символы.ПС;
		КонецЕсли;
		...
		Если Не ИмяТаблицы = "" Тогда
			...
			Для Каждого СтрокаТЧ ИЗ Товары Цикл
				...
				СтрокаТовара = "" + СтрокаТЧ.Номенклатура + Символы.ПС + СтрокаТЧ.Количество + " х " + СтрокаТЧ.Цена + " = " + СтрокаТЧ.Сумма + Символы.ПС;
		...
		Иначе
			...
			СтрокаТовара 	= "" + Документ.Номенклатура + Символы.ПС + Документ.ТовараВКоличестве + " х " + Документ.Цена + " = " + ТовараНаСумму + Символы.ПС;
			Текст = Текст + СтрокаТовара;
		...
		Текст = Текст + "         " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ИТОГ") + " = " + ТовараНаСумму + Символы.ПС +
		"       " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Скидка") + " = " + СуммаСкидки + Символы.ПС +
		"    " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Сумма НДС") + " = " + НДС + Символы.ПС;
		
		Текст = Текст + "--------------------------------" + Символы.ПС;
		
		Если ВидОплаты = Перечисления.ФормыОплаты.Наличные Тогда
			Текст = Текст + "    " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("НАЛИЧНЫМИ") + " = " + СуммаОплаты + Символы.ПС;
		ИначеЕсли ВидОплаты = Перечисления.ФормыОплаты.ПлатежнаяКарта Тогда
			Текст = Текст + "  " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("ПЛАТ.КАРТОЙ") + " = " + СуммаОплаты + Символы.ПС;
		ИначеЕсли ВидОплаты = Перечисления.ФормыОплаты.Кредит Тогда
			Текст = Текст + "     " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("КРЕДИТОМ") + " = " + СуммаОплаты + Символы.ПС;
		ИначеЕсли ВидОплаты = Перечисления.ФормыОплаты.Сертификат Тогда
			Текст = Текст + " " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("СЕРТИФИКАТОМ") + " = " + СуммаОплаты + Символы.ПС;	
		КонецЕсли;
		
		Текст = Текст + "--------------------------------" + Символы.ПС;	
		Текст = Текст + "        " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("СДАЧА") + " = " + Сдача + Символы.ПС;
		
		Текст = Текст + "--------------------------------" + Символы.ПС;
		Текст = Текст + "       " + ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Кассир") + ":  " + Кассир;	
	КонецЕсли;
