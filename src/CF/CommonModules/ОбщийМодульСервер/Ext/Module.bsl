...
Функция   ЗапросКСерверуПриНачалеРаботы() Экспорт
	...
	Результат.Вставить("ПолнаяВерсия", ОбщийМодульПовтор.ПолнаяВерсия());
...
КонецФункции //ЗапросКСерверуПриНачалеРаботы
...
Функция   ПечатьДнейРожденияКлиентов(ДокументДляПечати = Неопределено) Экспорт
	...
	Иначе			
		...
		Если НЕ РезультатЗапроса .Пустой() Тогда
			...
			СтрокаИзМакета = Макет.ПолучитьОбласть("С");
...
Процедура ПечатьНаСервереОтчетаРуководителю(Знач СтруктураОтчета, ДокументДляПечати, Знач НаЕмейл = ЛОЖЬ) Экспорт
	...
	ДокументДляПечати.ИмяПараметровПечати  = "ОтчетРуководителю" + СокрЛП(ИмяКомпьютера());
	ДокументДляПечати.КлючПараметровПечати = ДокументДляПечати.ИмяПараметровПечати;
...
Функция   ПолучитьКонтрагентаПоШтрихКоду(Знач ДанныеШтрихкода, Знач ЭтоТочноКлиент = ЛОЖЬ, Знач ЭтоТочноПоставщик = ЛОЖЬ, Знач ЭтоОрганизация = ЛОЖЬ, Знач ЭтоПозиция = ЛОЖЬ) Экспорт
...
КонецФункции //ПолучитьТаблицуОстатковВнутреннихЗаказов
...
КонецФункции // РассчитатьОриентировочнуюВаловуюПрибыль
...
Функция   УбратьНечисловыеСимволы(Знач ЗначениеДанных, Знач РазрешенныйСимволРазделитель = ".", Знач МожетБытьОтрицательным = ЛОЖЬ) Экспорт
