...
Процедура ДобавитьСобытиеЖурналаНаСервере(Знач ФормаСобытияТ, Знач ТипСобытия, Знач Ссылка = Неопределено) Экспорт
	...
	Если ОбщийМодульПовтор.ПолучитьПараметрСеанса("ВестиЗаписьВЖурналВсехОбычныхДействий") Тогда
		...
		Если ТипСобытия = 0 Тогда
			...
			ЖурналСобытий.Описание = "" + ОбщийМодульСервисСервер.ПолучитьКрасивоеНаименованиеОбъекта(ФормаСобытияТ);
		...
		Исключение // Записи в регистр сведений 
			Если ОбщийМодульПовторВТеченииСервера.ВыводитьСообщенияМожно() Тогда
				...
				ОбщийМодульСервисСервер.ДобавитьСообщениеПользователю(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи в журнал") + ": " + ЖурналСобытий + " " + ТекстОписаниеОшибки, , Ссылка);
			КонецЕсли;
...
Функция   ДобавитьШапкуОрганизации(ТабДок, Знач Организация, Знач ЯзыкДокументов = Неопределено) Экспорт
	
	Если ОбщийМодульПовтор.ПолнаяВерсия() Тогда
...
Процедура ОбеспечитьСписокОтборов(СписокФормы, Знач ДопТип = "", Знач СвоеНаименованиеОбъектаОтбора = "") Экспорт
	...
	Если НЕ СписокОтборов = Неопределено Тогда
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка установки правила отбора") + ": " + ТекстОписаниеОшибки);
		КонецПопытки;
	КонецЕсли;
...
Функция   ПечатьДнейРожденияКлиентов(ТабДок = Неопределено) Экспорт
	...
	Если ТабДок = Неопределено Тогда // сегодня
	...
	Иначе			
		...
		ТабДок.ИмяПараметровПечати  = "ДниРождения" + СокрЛП(ИмяКомпьютера());
...
Процедура ПечатьНаСервереОтчетаРуководителю(Знач СтруктураОтчета, ТабДок, Знач НаЕмейл = ЛОЖЬ) Экспорт
	...
	ТабДок.ИмяПараметровПечати  = "ОтчетРуководителю" + СокрЛП(ИмяКомпьютера());
	ТабДок.КлючПараметровПечати = ТабДок.ИмяПараметровПечати;
	...
	РазрядМетаданных = "Отчеты"; ИмяМетаданных = "ОтчетРуководителю";
	ПроизвольнаяПечатнаяФорма = ОбщийМодульТекстСервер.ОпределитьПроизвольнуюПечатнуюФорму(Перечисления.ВидыПечатныхФорм.ОтчетРуководителю, , СсылкаНаОбъектПечати, РазрядМетаданных, ИмяМетаданных);
...
Функция   ПечатьТоварногоЧека(Знач Документ, Знач ЭтоВозврат = ЛОЖЬ, Знач Клиент = Неопределено, Знач ИмяТаблицы = "Товары", Знач ВидОплаты = Неопределено, Знач ТаблицаОплаты = "", Знач Кассир = Неопределено) Экспорт
	...
	Если НЕ ПустаяСтрока(СтрогоеИмяНефискальногоПринтераЧеков) Тогда
		...
		// Попытка
		// 	ДокументЧекаОбъект = Документ.ПолучитьОбъект();
		// 	ДокументЧекаОбъект.ПКОНапечатан = ИСТИНА;
		// 	ДокументЧекаОбъект.Записать(?(Документ.Проведен, РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.Запись));
		// Исключение 
		// КонецПопытки;
...
Функция   ПоКурсу(Знач Сумма, Знач Валюта = Неопределено, Знач ИзВалюты = Неопределено, Знач НаДату = Неопределено, Знач ОпределенныйКурс = 0) Экспорт
	...
	Иначе		
		...
		ЗначениеЗаполненоИзВалюты = ЗначениеЗаполнено(ИзВалюты)
		И НЕ извалюты = ОсновнаяВалюта;
		
		ЗначениеЗаполненоВалюта   = ЗначениеЗаполнено(Валюта)
		И НЕ валюта = ОсновнаяВалюта;
...
Функция   ПолучитьДолгКлиентаПоставщика(Знач КлиентПоставщик, Знач Дата, Знач Договор = Неопределено, Знач ВернутьСтрокуДляРеализации = ЛОЖЬ, Знач валютаосн = Неопределено, Знач ЯзыкДокументов = Неопределено) Экспорт
	...
	Если ВернутьСтрокуДляРеализации Тогда
		...
		ИначеЕсли Долг > 0 Тогда
			Результат = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Долг до продажи составляет", ЯзыкДокументов) + ": " + Долг + " " + валютаосн;
		Иначе
			Результат = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Переплата до продажи составила", ЯзыкДокументов) + ": " + СокрЛП(-Долг) + " " + валютаосн;
		КонецЕсли;
	Иначе
		результат = Долг;	
	КонецЕсли;
	...
	Если ОбщийМодульПовтор.ПолучитьПараметрСеанса("ИспользоватьСкидкиПС") Тогда
		...
		Иначе
			...
			Если ОбщийМодульПовтор.ПолучитьЗначениеНастройкиИлиКонстанты("ИспользоватьСкидкиПериода") Тогда
				ПроцентСкидкиПериода = ОбщийМодульТоварСервер.ПолучитьПроцентСкилкиПериода(ОбъектСкидки, Дата, Склад);
...
Функция   УстановитьСостояниеЗаказа(Знач Заказ, Знач Дата = Неопределено, Знач НовоеСостояние = Неопределено) Экспорт
	...
	Если НЕ НовоеСостояние = Неопределено Тогда
		...
		Исключение // Записи в регистр сведений 
			ТекстОписаниеОшибки = ОписаниеОшибки();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка при попытке записи состояния заказа") + " " + ТекстОписаниеОшибки;
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Иначе 
			Если ПолучитьСреднююВИстории Тогда
				...
				Иначе
					Запрос.Текст = Запрос.Текст + " И Товары.Номенклатура = &Номенклатураа ";
				КонецЕсли;
...
Процедура ПроверитьФормулу(Знач Формула, Знач родитель = Неопределено) Экспорт
	...
	Если НЕ пропустить тогда
		...
		Исключение 	
			Если ОбщийМодульПовторВТеченииСервера.ВыводитьСообщенияМожно() Тогда
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщить(ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка в формуле") + ": " + ТекстОписаниеОшибки);
			КонецЕсли;			
		КонецПопытки;
	КонецЕсли;	
...
Процедура УстановитьЦенуИВсеЗависимые(Знач ВидЦен, Знач СтрокаТовара, Знач Ссылка = Неопределено, Знач Комментарий, Знач Дата, Знач ТолькоЗависимые = ЛОЖЬ, Знач МассивВидовЦен = Неопределено, Знач наУдаление = ЛОЖЬ, Знач первыйУровень = ИСТИНА, Знач БезЗависимых = ЛОЖЬ, Знач ЕдиницаИзмерения = Неопределено, Знач РежимУстановкиЦен = 1, Знач НеВыводитьСообщенияОбОшибке = ЛОЖЬ, Знач ГлобальныйПересчет = ЛОЖЬ) Экспорт
	...
	Попытка 
		...
		Если НЕ БезЗависимых
			...
			РезультатЗапроса = ОбщийМодульПовторВТеченииСервера.ВыяснитьПереченьВидовЦен(ВидЦен);
...
Функция   РасчитатьОриентировочнуюВаловуюПрибыль(Знач Номенклатура, Знач Количество = 1, Знач Дата, Знач Сумма, Знач Валюта = Неопределено, Знач Курс = 1, Знач СерияНоменклатуры = Неопределено) Экспорт
...
Процедура ПечатьДокументаПКО(ТабДок, Знач Ссылка) Экспорт
	...
	ИначеЕсли ТипЗнч(СсылкаНаДокументДляПКО) = Тип("ДокументСсылка.ОтчетыКомиссионеров") Тогда
		...
		|	ОтчетыКомиссионеровДеньгиЗаТовар.Ссылка.Комментарий,
		|	"""" КАК ПредметСделки,
	...
	ТабДок.ИмяПараметровПечати  = "ПКО" + СокрЛП(ИмяКомпьютера());
	ТабДок.КлючПараметровПечати = ТабДок.ИмяПараметровПечати;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(СсылкаНаДокументДляПКО) = Тип("ДокументСсылка.ВыполненияРабот") Тогда
			Шапка.Параметры.ПредметСделки = СсылкаНаДокументДляПКО.ПредметСделки;
		КонецЕсли;
		...
		Шапка.Параметры.КонтрагентПолноеНаименование = ПредставлениеПлательщика;
		...
		Шапка.Параметры.НаименованиеПредприятия = ПредставлениеОрганизации ;
		...
		Шапка.Параметры.Сумма = Сумма;
		Шапка.Параметры.СуммаСтрокой = Формат(Сумма, "ЧЦ=15; ЧДЦ=2; ЧРГ=' '; ЧГ=3,0") + " " + СокрЛП(Выборка.Валюта);
		Шапка.Параметры.СуммаТекстом = ОбщийМодульТекстСервер.СформироватьСуммуПрописью(Сумма, Выборка.Валюта, ЯзыкДокументов);
...
Процедура ЗадатьНаименованияОбъектамНаДругомЯзыке(Знач НасильноВернутьРусский = ЛОЖЬ) Экспорт
	...
	Если ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы(ИСТИНА) < 803020000
		ИЛИ ОбщийМодульСервисСервер.ПолучитьВерсиюПлатформы() < 803020000 Тогда // не поддерживается своё представление
...
Функция   НайтиПоТелефонуАдресуИЕмейл(Знач СтрокаПоиска, Знач ИмяСправочника = "Клиенты", Знач ТочноеСоотвествие = ЛОЖЬ) Экспорт
	
	УсловиеПоиска = ?(ТочноеСоотвествие, " = ", " ПОДОБНО ");
...
КонецФункции // НайтиПоТелефонуАдресуИЕмейл
...
Функция   СоздатьДокументСторно(Знач ОбъектСсылка, Знач Причина) Экспорт
	...
	Если НЕ Отказ Тогда
		...
		//ТипЗнчОбъектСсылка = Тип("ДокументСсылка.Инвентаризации")
		...
		//ИЛИ ТипЗнчОбъектСсылка = Тип("ДокументСсылка.ОтчетыКомиссионеров");
		...
		Попытка 
			...
			Исключение // Записи в регистр сведений 
				ТекстОписаниеОшибки = ОписаниеОшибки();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка регистрации причины") + ": " + ТекстОписаниеОшибки;
				Сообщение.Сообщить();
		...
		Исключение 	
			ТекстОписаниеОшибки = ОписаниеОшибки();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Ошибка создания документа сторно") + ": " + ТекстОписаниеОшибки;
...
Функция   ОпределитьНаправлениеДеятельности(Знач Контрагент = Неопределено, Знач Договор = Неопределено, Знач Организация = Неопределено, Знач Номенклатура = Неопределено, Знач Склад = Неопределено, Знач Сотрудник = Неопределено) Экспорт
...
Функция   ПеревестиРеквизитыФормы(ФормаСобытия, НаличиеПользовательскихПереводов = ЛОЖЬ, Знач РодительскийРеквизит = "", Уровень = 0) Экспорт
	...
	Если Уровень < 3 Тогда
		...
		Исключение
			ТекстОписаниеОшибки = ОписаниеОшибки();
			сообщить("" + РодительскийРеквизит + " " + ТекстОписаниеОшибки);
		КонецПопытки;	
	КонецЕсли;
...
Функция   ПроверитьИперевестиРеквизитИнтерфейса(РеквизитФормы, Знач ИПодсказку = ЛОЖЬ, ФормаСобытия = Неопределено, Знач ПереводитьЭлементыПоРеквизитам = ЛОЖЬ) Экспорт
	...
	Исключение
		ТекстОписаниеОшибки = ОписаниеОшибки();
		Сообщить("" + тип(РеквизитФормы) + " " + РеквизитФормы.Заголовок + " " + ТекстОписаниеОшибки);
	КонецПопытки;	
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Если НЕ ПустаяСтрока(ВыборкаДетальныеЗаписи.УсловиеПрограмма) Тогда
					...
					Исключение 	
						ТекстОписаниеОшибки = ОписаниеОшибки();
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка проверки условия события") + ": " + ТекстОписаниеОшибки;
						Сообщение.Сообщить();
							...
							Если РеакцииНаСобытие.СпособРеакции = Перечисления.СпособыРеакции.ВыполнитьПрограмму Тогда
								...
								Если ЗначениеЗаполнено(РеакцииНаСобытие.РеакцияПрограммаИзСправочника) Тогда
									...
									Исключение 	
										ТекстОписаниеОшибки = ОписаниеОшибки();
										Сообщение = Новый СообщениеПользователю;
										Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка выполнения программы из справочника реакции подписки") + ": " + ТекстОписаниеОшибки;
										Сообщение.Сообщить();
								...
								Если НЕ ПустаяСтрока(РеакцииНаСобытие.РеакцияПрограмма) Тогда									
									...
									Исключение 	
										ТекстОписаниеОшибки = ОписаниеОшибки();
										Сообщение = Новый СообщениеПользователю;
										Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка выполнения программы реакции подписки") + ": " + ТекстОписаниеОшибки;
										Сообщение.Сообщить();
...
КонецФункции //ВычислитьИтогДляТаблицы
		...
		Если НЕ СтруктураВерсионирования.РежимВерсионирования = Перечисления.ВидыВерсионирования.НеВерсионировать Тогда
			...
			Если СтруктураВерсионирования.РежимВерсионирования = Перечисления.ВидыВерсионирования.ПриУсловии Тогда
				...
				Исключение 	
					...
					Сообщение = Новый СообщениеПользователю;				
					Сообщение.Текст = ОбщийМодульПовтор.ПолучитьТекстНаЯзыке("Произошла ошибка вычисления условия версионирования") + ": " + ОписаниеОшибкиТекст;
					Сообщение.Сообщить();
...
Функция   УбратьНечисловыеСимволы(Знач ЗначениеДанных, Знач РазрешенныйСимволРаделитель = ".", Знач МожетБытьОтрицательным = ЛОЖЬ) Экспорт
...
КонецФункции //ПолучитьСотрудниковКлиента
...
Функция   ПолучитьТаблицуОстатковКнутреннихЗаказов(Знач Ссылка, Знач Склад, Знач Дата, Знач ИмяТаблицы = "Товары") Экспорт
...
КонецФункции //ПолучитьТаблицуОстатковКнутреннихЗаказов
